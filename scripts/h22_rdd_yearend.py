...
ax_a.set_xlabel('월', fontsize=10)
ax_a.set_ylabel('활동 평균 일집행액 (십억원)', fontsize=10)
ax_a.set_title('A. 월별 활동 평균 일집행액 (2015~2025)', fontsize=11, fontweight='bold')
ax_a.set_xticks([10, 11, 12])
ax_a.set_xticklabels(['10월', '11월', '12월'])
ax_a.legend(fontsize=6, ncol=6, loc='upper left', framealpha=0.7)
ax_a.grid(alpha=0.3)
...
ax_b.set_xlabel('연도', fontsize=9)
ax_b.set_ylabel('log 일집행액 jump\n(12월-11월 중앙값)', fontsize=9)
ax_b.set_title('B. 연도별 12월 점프\n(활동 중앙값)', fontsize=10, fontweight='bold')
ax_b.set_xticks(yr_list)
ax_b.set_xticklabels([str(y)[2:] for y in yr_list], fontsize=8)
...
if not field_df.empty:
    ...
    ax_c.set_yticks(ys)
    ax_c.set_yticklabels(field_df['label'], fontsize=8.5)
    ...
    ax_c.legend(handles=handles, fontsize=8, loc='lower right')
    ax_c.set_xlabel('β (log 일집행액 점프, 95% CI)', fontsize=9)
    ax_c.set_title('C. 분야별 12월 집행 점프 (RDD β, bw=±1월)', fontsize=10, fontweight='bold')
...
if clust_results:
    ...
    ax_d.set_yticks(ys)
    ax_d.set_yticklabels(cl_df['label'], fontsize=9.5)
...
for idx, fld in enumerate(plot_list):
    ...
    if not r_row.empty:
        ...
        ax.set_title(f'{fld}\nβ={b:.2f}{sig}  ({m:.1f}x)', fontsize=9, fontweight='bold')
    else:
        ax.set_title(fld, fontsize=9, fontweight='bold')

    ax.set_xticks([10, 11, 12])
    ax.set_xticklabels(['10월','11월','12월'], fontsize=7.5)
