"""H3: 활동(ACTV) 단위 게임화 시그니처 임베딩 + 클러스터링.
...
OUT_DIR = os.path.join(ROOT, 'data', 'figs', 'h3_embed')
...
csv_path = os.path.join(RES_DIR, 'H3_activity_embedding.csv')
df_out.to_csv(csv_path, index=False, encoding='utf-8-sig')
print(f'  활동 임베딩: {csv_path} ({len(df_out)} rows)')

prof_path = os.path.join(RES_DIR, 'H3_cluster_profile.csv')
zprof.to_csv(prof_path, encoding='utf-8-sig')
print(f'  클러스터 프로파일: {prof_path}')

ct_path = os.path.join(RES_DIR, 'H3_field_x_cluster.csv')
...
ofc_comm_df.to_csv(os.path.join(RES_DIR, 'H3_ministry_community.csv'),
...
df_c2[key_cols + ['subcluster','v1','v2']].to_csv(
    os.path.join(RES_DIR, 'H3_cluster2_subembedding.csv'),
    index=False, encoding='utf-8-sig')
...
pd.DataFrame([(o, ofc_comm2[o]) for o in ofc_comm2],
    ...
    os.path.join(RES_DIR, 'H3_ministry_community_v2.csv'),
    index=False, encoding='utf-8-sig')
...
ks_df.to_csv(os.path.join(RES_DIR, 'H3_field_ks_amp12m.csv'),
             index=False, encoding='utf-8-sig')
