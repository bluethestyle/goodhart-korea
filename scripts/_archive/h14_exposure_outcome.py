"""H14: 부처 굿하트 노출 점수 × outcome 디커플링 결합 분석.

H5 부처 노출 점수(51부처) + H6.1 부처별 outcome 차분 상관을 결합하여
...
입력
  ...
  data/warehouse.duckdb                  (indicator_panel — outcome 12분야)

출력
  scripts/h14_exposure_outcome.py
  data/results/H14_ministry_outcome_combined.csv
  data/figs/h14/H14_exposure_vs_outcome.png
  data/figs/h14/H14_risk_quadrant.png
...
OUT_DIR = os.path.join(ROOT, 'data', 'figs', 'h14')
...
merged = ofc_agg.merge(
    expo[['OFFC_NM', 'exposure_score', 'co_cluster',
          'pct_chooyeon', 'pct_sub05', 'pct_sub01']],
    on='OFFC_NM', how='inner'
)
...
# ── 결과 저장 ─────────────────────────────────────────────────────────────
out_csv = os.path.join(RES_DIR, 'H14_ministry_outcome_combined.csv')
...
print('\n[Fig 1] H14_exposure_vs_outcome.png')
...
ax.set_title('H14 — 부처 굿하트 노출 × outcome 디커플링\n'
...
fig_save(fig, os.path.join(OUT_DIR, 'H14_exposure_vs_outcome.png'))

# ── Figure 2: 4분면 위험 분류도 ───────────────────────────────────────────
print('[Fig 2] H14_risk_quadrant.png')
...
ax.set_xlabel('굿하트 노출 점수 (H5 exposure_score)', fontsize=12)
ax.set_ylabel('부처 가중 outcome 차분 상관 (w_corr_diff)', fontsize=12)
ax.set_title('H14 — 굿하트 위험 4분면 분류\n'
             '(X: 노출 점수, Y: outcome 디커플링, 기준선: 노출=0.3, corr=0)',
             fontsize=13)
ax.grid(alpha=0.2, zorder=0)

fig_save(fig, os.path.join(OUT_DIR, 'H14_risk_quadrant.png'))
