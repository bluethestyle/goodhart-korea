"""Figure 11 (h27_psd) — EN version. 4 archetype x 6 freq bin average PSD.

source: scripts/h27_power_spectrum_coherence.py (A) PSD plot
...
freqs = list(range(7))
labels_freq = ['DC (k=0)', '12m', '6m', '4m', '3m (quarterly)', '2.4m', '2m']
...
ax.set_xlabel('Frequency bin (period)')
ax.legend(loc='upper right', frameon=True)
ax.grid(alpha=0.3)

# Grant-Transfer 12m emphasis — top-left annotation
chooyeon_12m = float(psd_avg[psd_avg['archetype'] == 'C2_chooyeon']['psdnorm_k1'].values[0])
ax.text(0.02, 0.96,
        f'Grant-Transfer 12m: {chooyeon_12m:.3f}\n(~2× other archetypes)',
