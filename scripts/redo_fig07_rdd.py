"""그림 7 분리 — (A) 월별 line / (B) 연도별 점프를 별개 figure로.

source: scripts/h22_rdd_yearend.py
A4 본문 폭 6.3 inch 1:1
출력:
  paper/figures/_preview/h22_rdd_monthly.png  — (A) 월별 활동 평균 일집행액
  paper/figures/_preview/h22_rdd_yearly.png   — (B) 연도별 11→12월 점프
"""
...
""").df()
con.close()
print(f'raw rows: {len(df_raw):,}  활동: {df_raw["ACTV_CD"].nunique():,}')
...
# ── (A) 월별 활동 평균 일집행액 — viridis cmap + colorbar로 연도 의미 표시
