"""Figure 3 (h9_persistence) — 1x3 condensed → three separate figures.

Three figures (each A4 body width 6.3in 1:1 mapping):
  paper/figures_en/_preview/h9_pd.png        (Persistence Diagram)
  paper/figures_en/_preview/h9_barcode.png   (H1 Barcode top 30)
  paper/figures_en/_preview/h9_bootstrap.png (Bootstrap H1 max histogram)

source: scripts/h9_v2_11y.py plot code separated + A4 guideline applied
...
print(f'Activity N = {len(df):,}, PH pairs = {len(ph)}')
...
# ── Bootstrap (same as H9 v2: n=200, RNG=42, 50 iterations)
SUB_N = 200
RNG = np.random.default_rng(42)
print('Bootstrap 50 iterations ...')
boot = []
for i in range(50):
...
ax.axvline(0.65, color='steelblue', lw=1.4, linestyle='--',
           label='5y reference (median 0.65)')
ax.set_xlabel('H1 max persistence (bootstrap, n=200, 50 iterations)')
