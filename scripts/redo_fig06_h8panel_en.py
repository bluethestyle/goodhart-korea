"""Figure 6 (h8_panel) — English label version.

Copied from scripts/redo_fig06_h8panel.py; changes:
...
# Archetype beta labels (from source script execution)
beta_labels = [
    'A0_personnel: β=3.72',
...
# English bar labels
labels = ['A: Base\n(Δamp only)', 'B: +Field FE', 'C: +Archetype×Δamp']
...
ax.set_ylabel('R²')
ax.set_title(
    f'ΔR² (B→C, Archetype added) = {dr2_arch:+.3f}   '
...
# Top-left: archetype betas
ax.text(0.02, 0.97, '\n'.join(beta_labels),
        transform=ax.transAxes, va='top', fontsize=9,
        bbox=dict(boxstyle='round,pad=0.4', facecolor='#fff8e1',
                  ec='#daa520', alpha=0.92))

# Bottom-right: proxy replacement info (translated)
ax.text(0.98, 0.03,
        'Proxies: S&T→patent / Tourism→foreigners\n'
        'Admin.→fiscal autonomy / ICT→broadband',
