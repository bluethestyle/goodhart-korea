...
## §6.1/Ⅴ.1 분야별 12월 비중 — 데이터 오류 정정 (2026-06, conference_v1 동기화)

- **검증**: `warehouse.duckdb::monthly_exec`(2015~2025)에서 활동×연도 12월 share=Dec/연간(0≤share≤1) → 분야(FLD_NM) 중앙값·IQR 재현.
...
## conference_v1 제출 보강 (재평가 후속) — 2026-06-04

- **+554% 3구간 평균** (코로나 이전 2015–2018 **0.265** / 코로나기 2019–2021 **1.049** / 코로나 이후 2022–2025 **1.375**): 원천 `H28_wavelet_12m_evolution.csv` → main_v2 §6.8 표. **코로나 이후가 전 구간 최고**(이전의 약 5배) → "+554%는 일시 충격 아닌 ratchet(비가역) 고착"의 근거. conference_v1 §Ⅴ.6 인용. (사용자 지적 반영: 코로나 이후 지속성)
- **결과변수 교체 6건 audit**: 원천 `docs/limitations.md` §6 + `docs/index.md` + git `558878c`(H18~H21 outcome 6개 교체). 과기(R&D총투자→특허·자기인과)·관광(표본수→입국자)·행정(지방세→재정자립도·징세≠성과)·통신(ICT부가→가입률·자기인과)·교육(사교육→IMD·역지표)·국방(영업이익률→제외). **방어 논리**: 제거된 4개는 상관을 *부풀리는* 지표 → 교체는 결과 *보수화*(cherry-picking과 반대). conference_v1 §Ⅲ(축약) + main_v2 데이터절(전체) 인용.
- **RDD 게임화 vs 정상회계 caveat**: main_v2 §8.2(L1314) → conference_v1 §Ⅴ.2 이식. **제목** "사업원형별 인과 식별"→"**집행 측** 인과 식별"(영문 Execution-Side) 한정. **국방 모순**(15개 포함 vs 제외) 서술 정정. **생략 3곳** "지면 관계상 생략"→"기술보고서 수록"(목적지 명시).
