...
#caption(
  "FIG-5.3",
  [출연금형 +554% 강화 vs 인건비형 −1.4%(통제) — 극단 비대칭의 시간 진화.],
)[
  사업 유형별 12개월 cycle 진폭의 *시간 진화* (2015–2025). 출연금형(주황)이 *+554% 강화*,
  정상사업 +314.3%, 자산취득형 +174.1%. *인건비형은 −1.4%로 변화 없음* — 매월 균등 지급이라
...
#block(breakable: false)[
  #table(
    ...
    [인건비형 (통제)], [0.007], [0.007], text(fill: soft)[−1.4% (변화 없음)],
...
#callout(label: "§ 5 정리", color: ink, bg: chip-bg)[
  ...
  *인건비형 통제*: −1.4%(시점 조정 자체가 불가능한 사업)와의 극단 비대칭은 도구가 진짜 동적
...
= 학계는 30년째 이름을 붙여왔다 #h1sub[발견과 학술 frame 일괄 매칭]

§ 2부터 § 5까지 본 분석은 *일상 어휘*로 세 발견을 풀어왔다 — (a) 12월에 쏠림, (b) 분야는
균질한 단위가 아니고 사업 형태가 갈라낸다, (c) 출연금형은 분기말 정산 시점에 일제히
집행된다. 본 챕터는 이 세 패턴이 *학계가 이미 30년 가까이 이름을 붙여 온 현상의 한국
사례*임을 한눈에 매칭한다.
...
#block(breakable: false)[
  #table(
    ...
    table.header[*학계 이름 (이론 frame)*][*핵심 명제*][*본 분석의 한국 대응 발견*],
    [*굿하트-캠벨 법칙* (Goodhart 1975 / Campbell 1979)],
    [측정 가능한 양이 *평가 목표*가 되면 측정값과 본래 의도가 멀어진다.],
    [§ 2 관찰 1·3 — 집행률이 95–100%에 67% 압축되는 *천장 효과*는 측정 양만 좁아진 모양이고, 같은 분야 안에서 결과 지표는 *광범위 분산*된다.],
    [*다업무 계약 이론* (Holmstrom-Milgrom 1991)],
    [측정 쉬운 업무($e_t$)와 어려운 업무($e_q$)가 공존할 때 *측정 쉬운 쪽에 노력이 쏠린다*. 측정성 격차가 크면 어떤 인센티브 강도로도 1차 최적이 불가능 (impossibility).],
    [§ 3 12월 점프 — 자산취득형 ×3.42 격차는 *시점 조정 노력 $e_t$*의 한계비용이 낮은 사업원형에서 발현. § 4 archetype별 RDD 격차가 *비용 함수 이질성*의 정량 증거.],
    [*연성 예산 제약* (Kornai 1980 / Kornai-Maskin 2003)],
    [모기관·정부가 적자를 메워줄 기대가 있는 환경에서 *자율 규율*이 약해진다. 사회주의 국유기업 분석에서 출발했으나 공공기관·출연기관에 동일 적용.],
    [§ 5 출연금형 사이클 — 활동 간 동시 피크 coherence 0.54는 *기관 자율 의사결정*이 작동하는 환경에서 나오기 어려운 모양. 모기관 정산 사이클이 기관 내부 의사결정보다 우위에 있다는 단서.],
  )
]

== frame별 풀이

*굿하트-캠벨 — 측정 적응의 일반 패턴*: § 2 관찰 3의 *집행률 95–100% 천장 압축*은 측정값이
"채워졌는가"만 묻고 "사업이 무엇을 달성했는가"는 묻지 않는 평가 구조의 흔적이다. 측정값이
좁아진 만큼 결과 지표는 자유롭게 흩어진다 — 같은 집행률을 보고하는 분야 사이에서 결과
지표가 광범위 분산한다는 § 2 관찰 3이 그 정량적 표현. 굿하트는 *측정값이 평가 목표가 되면*
이 패턴이 일반적이라고 일찍 지적했으며, 본 분석은 한국 중앙정부 재정에서 그 정량 흔적을
보고한다.

*다업무 계약 — 한계비용 이질성의 직접 귀결*: Holmstrom-Milgrom은 *측정 쉬운 업무와 어려운
업무가 공존할 때 측정 쉬운 쪽에 합리적 행위자가 쏠린다*는 모형을 제시했다. 본 분석의
§ 3·§ 4 결과는 이 모형의 직접 한국 실증이다 — 같은 회계 마감 압력 아래서도 *시점 조정의
한계비용이 낮은 사업원형* (자산취득형 = 공정률 마감이 회계 마감과 결합)에서 점프가
가장 크게 발현(×3.42), *시점 조정 자체가 구조상 불가능한 사업원형*(인건비형 = 매월 균등
지급)에서는 점프가 거의 없다(×1.12). 같은 압력이 사업 형태에 따라 *3배 격차*로 발현되는
모양은 모형이 예측하는 *비용 함수 이질성*의 정량 패턴.

*연성 예산 제약 — 자율 규율의 약화*: Kornai의 핵심 통찰은 *연성*이 단순한 자원 부족이
아니라 *거버넌스 구조*에서 온다는 점이다. § 5의 출연금형 사이클 — 활동 간 동시 피크
coherence 0.54 — 는 *기관이 자율적으로 집행 계획을 짜는 환경*에서는 나오기 어려운 모양이다.
같은 달에 일제히 피크하는 패턴은 *외부 정산 일정*이 기관 내부 의사결정보다 우위에 있음을
시사한다. 출연기관은 직접 사업 부처와 달리 *시점 조정의 자유도*가 높고, 다음 회계연도
예산이 깎이더라도 모기관 보전 가능성이 시장 규율을 약화시킨다. 본 분석의 정량 흔적은
한국 공공기관에서 이 메커니즘이 *현재 진행형*임을 보여준다.

== 세 frame이 서로 어떻게 보완하는가

세 frame은 *서로 다른 추상 층위*를 다룬다. 굿하트는 *측정 일반*의 패턴, Holmstrom-Milgrom은
*개별 행위자의 비용 함수*, Kornai는 *기관 거버넌스 구조*를 본다. 본 분석의 세 발견은 이
셋이 *동일 사업 구조에 동시에 작용*한다는 것을 보여준다 — 굿하트가 진단한 *측정 적응*의
한국 형태가, Holmstrom-Milgrom의 *비용 이질성*으로 사업원형별로 갈리고, 출연금형의 경우
Kornai의 *연성 예산*이 추가로 작동해 *동기화된 사이클*까지 만들어내는 구조다. 한 frame이
아니라 *셋의 결합 적용*이 한국 사례에 맞다.
...
본 분석의 발견을 *현장 점검 자원 배분*에 쓸 수 있도록 세 가지 산출물로 정리한다.

== 산출물 1 #h2sub[부처 × 결과변수 4분면 — 점검 우선순위 도구]

51개 부처를 (a) 시점 집중 노출도(예산 가중 dec_pct)와 (b) 결과변수와의 부호 상관 두 축으로
4분면에 배치한다. *Q2 (노출 + 결과 음 상관)*에 위치한 부처가 점검 우선순위 상단이다. 진단
도구의 *해석 주의*는 § 7 결론과 같다 — 부처 라벨 단위는 1차 진단이고, 그 부처 *내부*에서
어떤 사업 형태(archetype)가 노출-결과 불일치를 만드는지가 2차 진단이다. § 4의 사업 단위
4분면 보조 차트가 이 2차 진단의 시각 자료다.

== 산출물 2 #h2sub[archetype별 점검 우선 활동 — top 사례 카드]

자산취득형 99개·출연금형 154개 활동 중 시점 압력 강도가 큰 *상위 사례*를 카드 형식으로
정리한다 (App-A·App-B 부록). 각 카드는 (a) 활동명·소속 부처·분야, (b) 11년 평균 12월 비중,
(c) 집행률 천장 도달 여부, (d) 비교 baseline 활동을 포함한다. 발표 Q&A 및 후속 점검에서
*특정 사업 즉답 자료*로 활용 가능. 본 카드는 *우선순위 데이터*이지 *문제 사업 낙인*이
아니다 — 시점 집중 자체가 위법·부정의 증거가 아닌 *추가 분석이 필요한 우선 대상* 표시다.

== 산출물 3 #h2sub[자동 모니터링 알고리즘 — 상시 운영 dashboard]

본 분석의 식별 알고리즘(amp_12m_norm 상위 + RDD 점프 임계 + cluster archetype 매칭)을
*상시 운영 가능한 dashboard*로 외부화한다. 매년 새 회계 마감 데이터가 dBrain+에 입수되면
자동으로 (a) 시점 집중 패턴 조기 식별, (b) archetype별 우선 점검 리스트 갱신, (c) 분야
4분면 위치 변화 추적이 가능하다. 감사원·국회 예산정책처와의 협력 운영을 권장한다.

= 정책 시사점 #h1sub[모형 균형점 이동의 6개 행정 액션]

본 분석은 정책 처방을 *임의로 나열*하지 않고 § 6의 학술 frame이 제시하는 *비교정역학
레버 3개*에서 직접 도출한 *권고 3개*와, 실증 발견에서 파생된 *보강 권고 3개*를 구분해
제시한다.

== 모형 직접 도출 — 권고 1~3 (Holmstrom-Milgrom 레버 1:1)

*권고 1. 다년도 회계 도입 확대 (시점 압력 가중치 ↓)*: 현행 *단년도 예산 마감* 구조는
시점 압력을 극단적으로 키운다. 중기재정운용계획(MTEF, 2009 도입)을 *5년 framework에서
실제 회계 단위*로 격상하면, 12월 절단점이 *실질적*에서 *명목적*으로 옮겨가 시점 압력이
크게 감소한다.

*권고 2. 출연기관 평가 지표 전환 (시점 가중치 ↓ + 품질 가중치 ↑)*: 출연금형의 동시 피크
사이클은 모기관-위탁기관의 12월 일률 정산 압력의 직접 증거다. 출연기관 경영평가에서
*집행률 100%*의 비중을 축소하고 *사업 품질 평가*(피인용·산출물 평가) 가중을 확대하면
두 레버가 동시에 작동한다. 단, 품질 측정성 한계로 효과는 일부에 그친다.

*권고 3. 출연기관 정산 시점 분산 (시점 조정 한계비용 ↑)*: 위탁계약별 정산 주기를
*분기 또는 반기*로 분산하면 12월 직전 시점 조정 한계비용이 증가해 사이클이 평탄해진다.
출연금형의 분야별 cycle coherence가 *분산형*으로 바뀌면 우연 alignment 외 효과를 추가로
식별 가능.

== 실증 발견 파생 — 권고 4~6 (전제·운영·시간 차원)

*권고 4. 데이터 인프라 강화 (모형 품질 가중치의 전제 조건)*: 현 *월별 집행* 자료 한계로
*12월 첫 주 단위 점프*를 직접 관측 불가. 열린재정 API의 *주별·일별 granularity 추가*는
RDD 식별력 향상 외에 사업 성과 측정 인프라의 1차 자료가 된다.

*권고 5. 자동 모니터링 시스템 (시점 조정 한계비용 ↑의 운영 보강)*: 산출물 3의 dashboard를
*상시 운영*하면 시점 집중 패턴의 *조기 식별 가시성*이 높아져 시점 조정 노력의 한계비용이
간접적으로 증가한다. 감사원·국회 예산정책처와 협력해 *실시간 패턴 모니터링 인프라*를
구축한다.

*권고 6. 시간 가중 점검 (H6 실증 발견 직접 적용)*: 출연금형 시간 강화 +554%는 *최근 3년
자료에 가중*한 점검이 필요함을 의미한다. 11년 평균 지표는 강화 추세를 희석한다 — 정책
점검 주기 자체를 *최근 시점 우대*로 재설계.

== Holmstrom-Milgrom impossibility의 한계 인정

본 6 권고는 *균형점 이동*을 목표로 하지 *게임화 완전 제거*를 약속하지 않는다. 측정성
격차가 본질적인 한, 어떤 인센티브 설계도 *1차 최적*이 될 수 없다는 Holmstrom-Milgrom
impossibility는 정직히 명시할 사항이다.

= 한계와 향후 과제 #h1sub[정직하게 짚어두면]

본 분석의 한계는 *데이터 한계*, *분석 모형 한계*, *해석 한계* 세 층위로 정리한다.

== 데이터 한계

*월 단위 집행 자료*: 본 분석은 *월별 집행*(dBrain+ VWFOEM)을 사용한다. 미국 Liebman-Mahoney
2017이 *주별 자료*로 ×4.9 점프를 보고한 것과 달리, 본 분석은 12월 *주 단위*의 정확한
점프 패턴을 관측하지 못한다. ×1.91이라는 한국 추정치의 일부는 *자료 granularity 한계*에서
온다 — 부록 E.2의 보정 시나리오 범위(×1.85~×2.87)도 미국 ×4.9에 도달하지 못함은 한계와
함께 *제도 구조 차이*의 정량 증거다.

*표본 크기 제약*: 매개분석에서 분야 단위 시계열 길이가 짧다(사회복지 n=6, 환경 n=4 등).
14분야 중 5개 분야만 매개 추정 가능했으며, 농림수산 단일 유의 결과(z=−2.897)는 *분포의
extremal 시그너처*이지 *분야별 광범위 매개*를 일반화할 근거는 아니다.

*외부 거버넌스 데이터 부재*: 출연금형 사이클이 *모기관 정산 일정*에 묶여 있다는 § 5
해석은 정량 패턴(coherence 0.54)에서 추론한 것이지 *직접 거버넌스 데이터*로 입증한 것은
아니다. 위탁계약 정산 시점 데이터가 부처 단위로 통합 공개되면 직접 검증 가능.

== 분석 모형 한계

*Stationary 가정의 한계*: H6 wavelet 분석은 +554% *시간 강화*를 보이지만, NeuralProphet
AR-Net forecast (부록 D)는 *현행 평가 제도가 stationary*하다는 가정 하에 외삽한다. 본
권고 1~3이 실현되면 forecast는 *반사실적*으로 재산출되어야 한다.

*클러스터 한계*: H3 UMAP+HDBSCAN의 입력은 *12개 스칼라 피처* (편성목·FFT 요약·HHI·규모·
추세·CV·12월 비중)이지 *12개월 시점 시퀀스 자체*가 아니다. 따라서 4 archetype은 *편성목
유형 클러스터*이며 월별 시점 패턴은 사후 서술이다 (silhouette: 편성목 공간 0.240,
시점 공간 −0.179). C2 출연금형 내부의 bimodal sub-structure 등 post-archetype 분석은
후속 과제.

*측도 의존성*: 사회복지 r=−0.86은 *FFT 측도 기반*이며 STL 측도로는 신호가 소멸 (§7).
본 분석은 *세 측도 모두 보고*하고 단일 측도 결론을 피했으나, 사회복지 fortuitous alignment의
*정확한 강도*는 도구 의존이라 정직히 보고한다.

== 향후 과제

(a) *주별·일별 RDD 정밀화* — 권고 4 데이터 인프라 구축 후 가능, (b) *자연 실험
calibration* — 다년도 회계 시범 부처 vs 일반 부처 비교, (c) *국제 비교* (미국·일본·EU)
— 평가 제도 차이의 균형 효과 측정, (d) *post-archetype sub-cluster 분석* — C2 출연금형
내부 bimodal 구조의 정량 분리, (e) *원형별 매개 분석 분해* — pooled p=0.481을 archetype별
매개 강도로 분해.

= 부록 #h1sub[용어·데이터·재현]

본 분석의 상세 부록은 별도 자료 묶음으로 정리한다. 발표 슬라이드 부록(App-A~App-WAV)과
학술 논문 버전(main_v2.typ 부록 A~G)을 *상호 보완 자료*로 활용한다.

- *부록 A — 용어 사전*: 본 분석에서 사용한 통계·방법론 용어 일괄 정의 (FFT·STL·NP·UMAP·
  HDBSCAN·TDA·PH·RDD·Sobel·Bootstrap·Wavelet 등). 학술 논문 부록 A·B 참조.
- *부록 B — 데이터 출처와 라이선스*: dBrain+ 월별 집행(2015~2025) + 14분야 outcome
  변수(KOSIS·ECOS·공공데이터포털·GIR·OECD DAC) + CPI 외생 통제변수. App-DATA 카드에
  정밀 매핑 표 수록.
- *부록 C — 방법론 디테일*: 수식·알고리즘 골자·대안 비교는 학술 논문 부록 C.1~C.13에
  정리. 슬라이드 App-RDD·App-MED·App-TDA·App-WAV 카드는 발표 활용 시나리오 중심.
- *부록 D — 재현 코드*: GitHub `bluethestyle/goodhart-korea` 저장소에 (a) 데이터 fetch
  스크립트(`scripts/fetch_*.py`), (b) 분석 스크립트(`scripts/h*.py` 시리즈), (c) 차트 생성
  (`scripts/build_*.py`) 일괄 공개. `data/warehouse.duckdb` (56MB)도 git에 포함되어
  clone 직후 즉시 재현 가능.
- *부록 E — 참고자료*: 학술 문헌(Holmstrom-Milgrom 1991, Kornai 1980, Goodhart 1975,
  Liebman-Mahoney 2017 등) + 한국 선행 공식 진단(FIS 22-03, 국회예산정책처 보고) + 미디어
  인용(한국재정정보원 인터뷰·서울경제·머니투데이 등) 일괄 정리.
