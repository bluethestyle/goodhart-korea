...
#caption(
  ...
  *큰 사업 분야*(사회복지·산업·중소기업) 는 13%대에 그친다.
...
#caption(
  ...
  사업 유형별 12월 점프 배수 (11월 마지막 주 → 12월 첫 주 일평균 비율). *자산취득형이 3.42배로 가장 강함*,
...
#block(breakable: false)[
  #grid(
    columns: (1fr, 1fr),
    column-gutter: 10pt,
    kpi("자산취득형", "× 3.42", sub: "공정률 마감 + 회계 마감"),
...
#callout(label: "관찰 — 행정 시점 압력은 단일 절단점이 아니다")[
  본 분석에서 12월 RDD를 1차 검증 도구로 보고하되, 3·6·9월 부수 점프의 존재는 회계 시점
  압력이 *4개 경계*에서 동시에 작동하는 시스템임을 시사한다. 분기말 RDD를 포함한 다중 경계
  확장 분석은 향후 과제로 남긴다.
]
...
#callout(label: "§ 3 정리", color: ink, bg: chip-bg)[
  ...
  *자산취득형 사업의 3.42배 점프*로 흔적이 남는다. 시설 공사·자산 취득은 공정률 마감과 회계
...
12월 점프 격차(자산취득 3.42 vs 인건비 1.12)는 이 통상적 가정과 충돌한다.
...
보완적이다 — 같은 *하반기 집행형* 안에 인프라성 자산취득형(12월 절벽 점프)과 출연금형(분기·반기
사이클)이 함께 포함되어 있고, RDD 점프 강도가 둘에서 극단으로 갈린다(× 3.42 vs × 1.10).
...
#callout(label: "§ 5 정리", color: ink, bg: chip-bg)[
  ...
  *현장 풍경과의 일치*: § 1에서 인용한 "출연기관은 어차피 모기관이 메워준다"는 풍경은,
  데이터에서 *분기·반기 단위로 활동들이 일제히 정산되는 모양*(coherence 0.54)으로 흔적이 남는다.
