...
== 처방 — 회계 제도·거버넌스·데이터 인프라 권고
  ...
  *권고 2 — 출연기관 정산 시점 분산*: 본 연구가 출연금형 RDD 점프 3.42배 + phase coherence 0.54로 식별한 *동시 피크 동기화*는 모기관-위탁기관의 12월 일률 정산 일정에서 발생한다. *위탁계약별 정산 주기*를 분기 또는 반기 단위로 분산하면 12월 집중 완화 가능. 출연기관 평가 지표를 *집행률 100\%*에서 *집행 품질 + 사업 성과*로 전환하는 것이 근본 해결책.
...
= 결론
  ...
  *기여 2 — 직관 반대 발견 (Counterintuitive Findings)*: 본 연구는 게임화가 단순히 부정적 측정 왜곡이 아닌 *분야 이질적·시간 동적 현상*임을 보였다.
  ...
  - *회계연도 12월 RDD 점프*: 전체 1.91배, *출연금형 3.42배* — Liebman-Mahoney(2017) 미국 결과의 한국 확장.
...
*해석*: 출연금형은 다른 원형보다 *1개월 일찍* 피크에 도달한다. 12월 정산 마감 대비 *사전 준비 단계*가 더 길고, 위탁 기관과의 정산 일정상 10월 집중 집행이 발생함을 시사한다. 이는 본문 RDD에서 출연금형이 12월 점프 3.42x로 가장 강했던 결과와 *상보적*: 출연금은 10\~12월에 걸친 *3개월 windowing*으로 봐야 정확하다.
...
네 도구는 *시간 차원의 해상도*에서 점진적으로 강해지며, 함께 사용 시 *정상성·추세·changepoint·동적 진화*를 모두 다룬다. 본 연구의 게임화 측정은 이 4중 lens에서 *모두 일관된 출연금형 우세 + 시간 강화* 결론으로 수렴한다.
