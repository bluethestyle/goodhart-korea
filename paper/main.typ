...
== STL trend 자기 비판 — 그리고 NeuralProphet·Wavelet 중재
  ...
  *결과 — 신호 소멸*: STL 분해 후 seasonal_strength를 게임화 지표로 사용하면 사회복지 신호는 r=+0.003(p=0.991)로 *완전히 소멸한다*(@fig-stl). 두 가능성:
...
#figure(
  image("figures/h22_rdd_appendix.png", width: 100%),
  caption: [회계연도 RDD 부록 — C 분야별 12월 점프 forest plot, D 사업원형별 점프 배수],
)
