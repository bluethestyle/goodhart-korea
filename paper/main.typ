...
== NeuralProphet 중재 — 세 도구의 합의 여부
  ...
  *문제 의식*: FFT는 12월 점프 신호를 강하게 보고하지만 STL은 같은 신호를 추세 흡수로 처리한다(사회복지 r: FFT $-0.76$ → STL $+0.003$). 두 도구의 가정 차이가 결과 차이를 만들었는지, 한국 사회복지 자료의 본질적 특성인지 결정하기 위해 NeuralProphet 분해를 *세 번째 cross-check*으로 수행했다.
  ...
  *놀라운 발견*: 세 측도 사이의 상관이 모두 *0에 가깝거나 약한 음의 상관*이다. 이는 *세 도구가 서로 다른 신호 구성요소를 포착*하고 있음을 시사한다 — FFT는 *총 변동 중 12개월 주기 비중*, STL은 *추세 제거 후 잔여 계절성*, NP는 *changepoint 보정 후 yearly seasonality 진폭*. 같은 이름의 "게임화"라도 측정 도구가 보는 *측면*이 다르며, 단일 도구의 결과를 절대화하면 오해를 야기할 수 있다.

  *분야별 해석*: 14분야 outcome 상관을 세 측도로 재산출한 결과, 보건(life expectancy)·통신(broadband)에서 세 측도 모두 강한 음의 상관(보건: FFT $-0.55$, STL $+0.20$, NP $-0.71$; 통신: FFT $-0.15$, STL $+0.05$, NP $-0.75$)을 보이며, 이는 *NP가 FFT가 놓친 통신 분야의 게임화 신호를 추가로 식별*함을 의미한다. 반면 사회복지는 세 측도 모두 약한 신호(FFT $+0.03$, STL $+0.00$, NP $-0.24$)로, 본 연구의 *사회복지 자동분배 효과는 14분야 단위 STL 후 소멸한 결과와 일관*되며 활동-단위 NP에서도 강한 신호로 나타나지 않는다.

  *결론*: 세 도구의 합의는 부분적이다. FFT/STL/NP가 *동일한 한국 게임화 신호를 다른 각도*에서 측정하므로, 본 연구의 발견은 *어느 한 도구의 절대값*이 아니라 *복수 도구의 일관된 방향*에 의존해야 한다. 이는 *방법론 다원주의*의 정직한 한계 보고이자, 후속 연구가 측도 간 상관 자체를 메타-신호로 활용할 가능성을 제시한다.
