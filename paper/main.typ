...
#par(first-line-indent: 0pt, leading: 0.95em)[
  본 연구는 열린재정정보 월별 집행 11년치(2015\~2026)와 KOSIS·한국은행 ECOS·공공데이터포털·온실가스종합정보센터 14분야 결과변수를 통합해 한국 정부 재정 집행의 굿하트 효과(Goodhart 1975)를 다각도로 측정·검증한다. 활동(ACTV) 단위 임베딩(UMAP+HDBSCAN)으로 인건비형·자산취득형·출연금형·정상사업 4개 사업원형을 발견하고, Mapper와 지속 호몰로지(Persistent Homology)로 위상 안정성을 입증한다. 분야 고정효과 회귀에서 분야 라벨의 추가 설명력(ΔR²)이 0에 수렴하는 반면, 사업원형×지출진동의 상호작용은 ΔR²=+0.085\~+0.094로 유의함을 확인해 분야 단위 분석의 trivial 성격을 정량적으로 반증한다. 사회복지 자동분배 효과(r=−0.762, p=0.035; CPI 통제 후 −0.86, p=0.007)와 회계연도 12월 RDD 점프 1.91배(자산취득형 3.42배)를 통해 Liebman-Mahoney(2017)의 미국 결과를 한국으로 확장한다. STL 분해 결과 사회복지 신호의 trend 혼재 가능성을 자기 비판적으로 보고하며, 신호 처리(FFT 진폭·STL 추세분리·NeuralProphet 가법 분해·웨이블릿 시간-주파수)의 4중 도구 상보성을 통해 *방법론 트라이앵귤레이션*을 수행한다. *웨이블릿 분석은 출연금형 12개월 cycle 진폭이 2015\~2017 → 2023\~2025로 +554\% 증가*함을 새롭게 식별해, 한국 굿하트 효과가 *고정 패턴이 아닌 진행 중인 동적 강화 현상*임을 입증한다. 게임화 메커니즘은 사업원형별로 달라, *자산취득형은 12월 1일 직후 점프*에, *출연금형은 연 단위 사이클 강도*에 두드러진다.
]

#v(1em)

#par(first-line-indent: 0pt)[
  #text(weight: "bold")[주요어:] 굿하트 법칙, 정부 재정, NPM, 토폴로지 데이터 분석, 웨이블릿, 회귀불연속, 매개분석, 한국
]
...
== 본 연구의 핵심 주장
  ...
  *추가 발견 — 시간 동적 강화*: 본 연구는 FFT의 정상성 가정을 웨이블릿 분석(부록 D.4)으로 보완해, 한국 게임화 효과가 *시간이 갈수록 강화*되고 있음을 새롭게 입증한다. 출연금형 12개월 cycle 진폭은 2015\~2017 평균 대비 2023\~2025 평균이 *+554\% 증가*했으며, 정상사업도 +316.7\% 증가한 반면 인건비형은 변화 없음(−0.8\%, 통제 사례). 이는 굿하트 효과가 *고정 패턴이 아닌 진행 중인 동적 현상*임을 의미하며, 정책 점검의 시간 가중치가 최근 자료로 옮겨가야 한다는 함의를 갖는다.
...
=== 회귀불연속 설계(RDD)
  ...
  *주제 적합성*: 한국 회계연도 cutoff는 (a) 1948년 이래 *불변*, (b) 모든 부처에 *동시* 적용, (c) 개별 활동이 *조작 불가능* — RDD의 *세 핵심 가정*이 모두 성립하는 거의 이상적 자연실험 환경이다. Liebman-Mahoney(2017)가 미국에서 cutoff 며칠 차이로 5배 점프 + 품질 하락을 입증한 설계의 한국 적용은 *이론적으로 직접 가능*하며, 실제 분석에서 1.91배(전체) / 3.42배(자산취득형) 점프를 확인한다. 같은 활동의 단 며칠 차이를 비교하므로 *분야·기관·사업 특성이 자동 통제*되는 준실험이다.
...
#figure(
  ...
  caption: [견고성 검증 — (a) FE 회귀 β + 95% CI (N=128, 분야 FE 추가에도 β 유의 안 함). (b) 14분야 Permutation forest (점=관측, 띠=null 95% CI). 사회복지 obs=−0.762, p=0.035로 14분야 중 유일하게 유의.],
) <fig-h6>
...
== 회계연도 12월 점프 (한국판 Liebman-Mahoney)
  ...
  *왜 RDD가 결정적 증거인가*: 사업 본질에 따른 정상적 변동과 회계 게임화를 분리하기 위해, 같은 활동의 *11월 마지막 주 vs 12월 첫 주* 일평균 집행을 비교한다. 며칠 차이의 변동은 사업 본질로 설명할 수 없고, 12월 1일이라는 *행정적 임의 절단점*만이 작동한다. Liebman-Mahoney(2017, AER)가 미국 연방조달에서 5배 점프를 보고한 설계의 한국 적용이다.
  ...
  미국 5배 대비 한국 1.9배의 절대값 차이는 *측정 단위 granularity* 차이로 설명된다(미국 주 단위 vs 한국 월 단위 — 12월 첫째 주의 신호가 12월 전체로 분산). 사업 형태별 RDD 점프의 *순서*는 Liebman-Mahoney(2017)의 use-it-or-lose-it 메커니즘이 *공정률·자산취득 사업*에서 가장 강하게 작동함을 보여준다.
...
#figure(
  ...
  caption: [월별 활동 평균 일집행액 (2015~2025) — 색상은 연도(보라 2015 → 노랑 2025), 굵은 검정선은 11년 평균. 11~12월 영역(빨강)에서 12월 점프 가시화.],
) <fig-rdd-monthly>

#figure(
  image("figures/h22_rdd_yearly.png", width: 100%),
  caption: [연도별 12월 점프 (활동 중앙값 log 일집행액 12월 − 11월). 전체 RDD β=0.65 (1.91배, 주황 점선), Liebman-Mahoney(2017) 미국 5x 참조선(보라 점선) 대비.],
) <fig-rdd-yearly>

#figure(
  image("figures/h22_rdd_field.png", width: 100%),
  caption: [분야별 12월 점프 배수 — 국방·국토·교통이 가장 큼, 자산취득형 3.42x],
) <fig-rdd-field>

== STL trend 자기 비판 — 그리고 NeuralProphet·Wavelet 중재
...
== NeuralProphet 중재 — 세 도구의 *상보적* 측정
  ...
  *추가 발견 — 시간 동적 강화 (Wavelet)*: FFT의 정상성 가정을 보완하기 위해 부록 D.4에서 *Continuous Wavelet Transform*을 적용한 결과, *출연금형 12개월 cycle 진폭이 2015\~2017 → 2023\~2025로 +554% 증가*함을 발견했다. 정상사업도 +316.7% 증가했으나, 인건비형은 −0.8%로 변화 없음 — 12월 점프와 무관한 통제 사례다. 본 발견은 한국 굿하트 효과가 *고정 패턴이 아닌 진행 중인 동적 강화 현상*이며, FFT amp_12m_norm의 11년 평균값은 이 *증폭 추세*를 희석한다는 점을 입증한다 (정책 함의 절 권고 5 참조).
...
== 처방 — 회계 제도·거버넌스·데이터 인프라 권고

  본 연구의 RDD·웨이블릿·phase coherence 결과는 *회계연도 cutoff와 출연기관 정산 압력*이 게임화의 핵심 동인임을 시사한다. 따라서 정책 처방은 *cutoff 압력 자체를 완화*하는 방향이어야 한다.

  *권고 1 — 다년도 회계 도입 확대*: 현행 *단년도 예산 마감* 구조는 12월 점프 1.91배의 1차 동인이다. 2009년 도입된 MTEF(중기재정운용계획)를 *5년 단위 framework*에서 실제 회계 단위로 격상해 *이월/전용 자유도*를 명시화하면 12월 정산 압력이 분산될 수 있다. 미국 GPRA(Government Performance and Results Act)에서 다년도 funding 사례 참조.

  *권고 2 — 출연기관 정산 시점 분산*: 본 연구가 출연금형 phase coherence 0.54 + PSD k=1 진폭 0.332로 식별한 *동시 피크 동기화*는 모기관-위탁기관의 12월 일률 정산 일정에서 발생한다. *위탁계약별 정산 주기*를 분기 또는 반기 단위로 분산하면 12월 집중 완화 가능. 출연기관 평가 지표를 *집행률 100\%*에서 *집행 품질 + 사업 성과*로 전환하는 것이 근본 해결책.

  *권고 3 — 데이터 인프라 강화*: 본 연구는 *월별 집행*(VWFOEM)이라는 자료 한계로 12월 *주(week) 단위* 점프를 직접 관측하지 못했다. 미국 Liebman-Mahoney(2017)는 주별 자료로 5배 점프를 보고했다. 열린재정 API의 *주별·일별 granularity 추가*는 미래 RDD 분석의 식별력을 직접 향상시킨다. *공공기관 사업 단위 결산 일자 미시 자료* 공개도 핵심 보완.

  *권고 4 — 자동 감사 flagging 시스템*: 본 연구의 50건 sub05 (극단 게임화 활동) 식별 알고리즘(amp_12m_norm 상위 + RDD 점프 3배 이상)을 *상시 운영*하면 차기 회계연도 점검 자원 배분이 최적화된다. 감사원·국회 예산정책처와 협력해 *실시간 게임화 모니터링 dashboard* 구축 가능.

  *권고 5 — 시간 가중 점검*: 웨이블릿 분석에서 출연금형 게임화가 *2015\~2017 → 2023\~2025 +554%* 증가했다는 발견(부록 D.4)은, 정책 점검이 *최근 3년 자료에 가중*되어야 함을 시사한다. 11년 평균 지표는 강화 추세를 희석한다.
...
= 결론
  ...
  본 연구는 한국 중앙정부 재정 집행의 게임화 현상을 11년 시계열·14분야·1,557개 활동·11개 방법론(FFT·STL·NeuralProphet·웨이블릿·UMAP·HDBSCAN·Mapper·PH·RDD·매개분석·CPI 통제)으로 검증했다. 핵심 기여는 다음 *넷*이다.

  *기여 1 — 분석 단위 재정의*: 한국 행정학·재정학 연구가 표준으로 사용해온 *분야 단위 분석*(사회복지, 교육, 국방 등)이 게임화 현상에 대해 *trivial*하다는 정량 증거($Delta R^2 = 0.000$)를 제시하고, 진짜 분석 단위는 *사업 형태(archetype)*임을 위상 데이터 분석으로 입증했다. 4개 사업원형(출연금형/자산취득형/인건비형/정상)은 UMAP+HDBSCAN으로 발견되었고 Mapper graph 4 components, Persistent Homology 30 강건 connected components로 위상적 안정성이 확보됐다. *행정 분류와 게임화의 진짜 단위가 다르다*는 본 발견은 후속 정책 분석의 단위 선택 기준을 재고하게 한다.

  *기여 2 — 직관 반대 발견 (Counterintuitive Findings)*: 본 연구는 게임화가 단순히 부정적 측정 왜곡이 아닌 *분야 이질적·시간 동적 현상*임을 보였다.
  - *사회복지 자동분배 효과*: 게임화 강도와 빈곤 지수가 음 상관(r=−0.76), CPI 통제 후 *강화*(−0.86) — 자연 cycle 가설 강력 기각.
  - *회계연도 12월 RDD 점프*: 전체 1.91배, *자산취득형 3.42배* — Liebman-Mahoney(2017) 미국 결과의 한국 확장.
  - *같은 분야 내 사업 형태별 부호 반전*: 사회복지 출연금형(+0.53) vs 정상사업(−0.55) — 분야 cycle 가설 기각의 추가 증거.
  - *출연금형 phase coherence 0.54*: 활동 간 *동시 피크* 동기화 (다른 원형 0.08\~0.13 대비) — 외생 회계 cycle이 *집단 행위*를 유도한다는 직접 증거 (부록 D.3).

  *기여 3 — 게임화 강도의 시간 동적 강화 (Wavelet 신규 발견)*: 본 연구는 FFT 정상성 가정의 한계를 웨이블릿 분석으로 보완해, *게임화가 시간이 갈수록 강화*되고 있음을 새롭게 입증했다 (부록 D.4):
  - *출연금형*: 12개월 cycle 진폭 *2015\~2017 → 2023\~2025로 +554% 증가*
  - *정상사업*: +316.7%
  - *자산취득형*: +174.7%
  - *인건비형*: $-0.8\%$ (변화 없음, 통제 사례)

  이는 한국 굿하트 효과가 *고정 패턴이 아니라 진행 중인 동적 강화 현상*이며, 출연금형에서 가장 빠르게 심화되고 있다는 의미다. 정책 분석의 시간 가중치가 *최근 3년 자료*로 옮겨가야 한다는 함의를 갖는다.
...
#figure(
  image("figures/h27_phase.png", width: 100%),
...
원형 내 활동 쌍 $i, j$의 12개월 주파수 phase coherence $abs(angle.l e^(i Delta phi) angle.r)$를 측정 (1.0=완전 동기, 0=무관). 500쌍 무작위 표본:
