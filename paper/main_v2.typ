...
=== Archetype 차원 보조 4분면 — 진짜 설명 단위 시각화
  ...
  § 6.1의 정량 결과(분야 ΔR² ≈ 0 vs 원형×Δamp ΔR²=+0.025)에 따라, 부처 4분면은 어떤 archetype 비중이 부처별 Q 분류를 만들었는지에 대한 후속 진단을 필요로 한다. 본 절은 1,557개 사업 단위 산점도(시점 집중 노출 × 결과변수 부호)를 archetype 색으로 구분해 진짜 설명 단위에서 동일 4분면 구조를 검증한다. 출연금형(C2)·자산취득형(C1) 활동이 노출-결과 불일치 영역(Q2)에 집중되는 패턴이 시각적으로 확인된다.
