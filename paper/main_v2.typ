...
== 왜 시민이 신경 써야 하는가 — 일상 영향 4경로
  ...
  + 공공서비스 품질: 연 600조 원대 재정의 4중 평가 체제가 합리적 행위자에게 시점 조정 노력으로의 자원 집중을 유도하는 한, 사업 품질 노력 $e_q$는 측정성 격차 함수 $phi'(dot.op) < 1$ 만큼 체계적으로 희생된다 (Holmstrom-Milgrom impossibility). 시민이 일상에서 받는 공공서비스의 품질 하락은 이 측정성 격차의 직접 귀결이다.
...
== 본 연구의 핵심 기여
  ...
  + 기여 2 — 분석 단위 재정의 (§6.1, H1): 한국 행정학·재정학이 표준으로 사용해온 분야 단위 분석이 게임화에 대해 trivial($Delta R^2 = 0.000$)이고, 진짜 단위는 사업 형태(archetype)임을 위상 데이터 분석으로 다중 입증. TDA Loop 분석: C2 출연금형 내부 sub-heterogeneity 발견 — 4-archetype 분류가 완전한 분류 종점이 아니며 C2 내부 추가 sub-structure가 존재함을 PH loop가 시사.
...
= 정책 함의
  ...
  본 연구의 발견은 진단과 처방의 두 층위로 정책 권고를 도출한다.
...
== 처방 — 모형 균형점 이동의 행정 액션 매핑
  ...
  본 절은 정책 처방을 임의로 나열하지 않고, P-A 모형의 비교정역학 결과에 직접 매핑되는 3개 권고와 모형 검증 결과(H6 시간 강화·실증 발견)에서 파생된 3개 보강 권고로 구분해 제시한다. 모형은 균형 시점 조정 노력 $e_t^*$의 부분 미분으로 세 가지 정책 레버를 식별했다.
