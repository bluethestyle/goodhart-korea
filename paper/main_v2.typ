...
#par(first-line-indent: 0pt, leading: 0.95em)[
  본 연구는 한국 중앙정부 재정 집행에 *굿하트 효과(Goodhart 1975)*가 발현하는 *미시적 기제*를 Principal-Agent 균형 분석으로 도출하고, 그로부터 파생되는 6개 검증 가능 가설(H1\~H6)을 11년 시계열·14분야·1,557개 활동·11개 방법론으로 다중 검증한다.
...
== 현행 한국 평가 제도와 측정성 격차
  ...
  *측정성 격차* (Holmstrom-Milgrom 1991): 어떤 task는 정확히 측정되고 다른 task는 부분/불가 측정. 본 연구의 핵심 격차는 다음과 같다.
...
== 균형점 도출 — 굿하트 게임의 1차 조건
  ...
  $ (partial U_A) / (partial e_q) = w_q - (partial c) / (partial e_q) = 0 quad => quad w_q = (partial c) / (partial e_q) $

  *핵심 결과*: $w_t >> w_q$인 환경(현행 한국 평가 제도)에서 균형 $e_t^* >> e_q^*$ — Agent가 *합리적*으로 시점 조정에 노력 집중. 이것이 *굿하트 게임의 미시적 기제*다.
...
== 6.4.b H4 보강 — 매개경로 원형 이질성 (Baron-Kenny + Sobel + Bootstrap)
  ...
  *방법*: Baron-Kenny(1986) 4단계 + Sobel z-검정 + Bootstrap 95\% 신뢰구간(부록 C.11). 활동 단위 $X$ = 출연금 비중, $M$ = `amp_12m_norm`, $Y$ = 분야별 결과변수.
...
+ *국제 비교*: 미국 Liebman-Mahoney(2017) 5배, 한국 본 연구 1.91배(자산취득 3.42배), 일본·EU 동일 분석 시 비교 → 평가 제도 차이의 균형 효과 측정.
...
= 결론
  ...
  *기여 1 — 이론 모형: 굿하트 게임의 미시 도출*: Principal-Agent 균형 분석을 통해 *왜* 한국에서 굿하트 효과가 발현하는가에 답한다. Agent의 *비합리성*이 아닌 *합리성*이 측정성 격차 환경에서 시점 조정 노력으로 자원이 쏠리도록 유도한다 (Holmstrom-Milgrom 1991). 모형은 6개 검증 가능한 가설(H1\~H6)을 산출하며, 그중 *사업원형별 균형 패턴 차이*가 본 연구의 핵심 예측이다.
  ...
  *기여 3 — 직관 반대 발견 (모형 H2, H3, H5 검증)*: 게임화는 단순히 부정적 측정 왜곡이 아닌 *분야 이질적·시간 동적 현상*임을 보였다.
  - *자산취득형 RDD 점프 3.42배* (H2 검증): 12월 1일 직후 *이산적 spike* — Liebman-Mahoney(2017) 미국 결과의 한국 확장.
...
에서 $tilde(e)_q$는 $e_q$의 측정 가능 부분으로 $tilde(e)_q = phi(e_q)$, $phi'(\cdot) > 0$, $phi'(\cdot) < 1$ (측정성 격차). 비용 함수 $c$는 strictly convex, $c_(t t) > 0$, $c_(q q) > 0$, $c_(t q) >= 0$.
