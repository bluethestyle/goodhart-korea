...
#[
  #set par(first-line-indent: 0pt, leading: 0.95em)
  This paper derives the micro-mechanism through which Goodhart's effect (@goodhart1975) manifests in Korean central government budget execution via a Principal-Agent equilibrium model, then validates six falsifiable hypotheses (H1--H6) using 11-year monthly data, 14 expenditure fields, 1,557 spending activities, and 11 analytical methods.
...
=== 푸리에 변환(FFT) 기반 amp_12m_norm
  ...
  *직관*: 어떤 시계열이든 다양한 주기의 사인파 합으로 분해할 수 있다 (@fourier1822). 연도-내 시계열에 푸리에 변환을 적용하면 1년 주기 성분의 진폭이 추출된다. 이 진폭이 클수록 "12월 점프 + 1월 reset"이라는 회계연도 게임화 패턴이 강하다는 뜻이다. 정의(이산 푸리에 변환·`amp_12m_norm` 비율 식·Parseval 해석)는 부록 C.1.
...
=== STL 분해(Seasonal-Trend decomposition using Loess)
  ...
  *직관*: FFT amp_12m_norm 신호가 진짜 게임화일까, 아니면 *지속적으로 증가하는 추세*가 12월 결산 시점 기록 방식과 결합해 만들어내는 가짜 주기성일까? 이 의문을 검증하기 위해 STL (@cleveland1990)을 사용한다. STL은 시계열을 가법 분해 $x_t = T_t + S_t + R_t$로 추세·계절·잔차로 분리하고, 추세 제거 후 계절 성분의 분산 비율을 `seasonal_strength`로 정의해 게임화 지표로 사용한다. 두 겹 반복(inner/outer loop)·robustness weight·정의식은 부록 C.2.
...
=== NeuralProphet 신경망 분해 — 세 번째 cross-check
  ...
  *직관*: FFT(정상성 가정)와 STL(임의 추세 가정)이 서로 다른 결론을 줄 때 어느 쪽이 게임화의 진짜 신호인지 결정하기 위해 *세 번째 독립 도구*가 필요하다. NeuralProphet (@triebe2021)은 Facebook Prophet (@taylor2018)의 신경망 확장으로, 시계열을 *추세 + Fourier 계절 + AR-Net 자기회귀 + 이벤트 + 외생 회귀*의 6개 가법 성분으로 분해한다. 본 연구는 cross-check 비교 형평성을 위해 자기회귀(`n_lags=0`)·외생 회귀를 *비활성화*하고 *추세 + 계절*만 사용한다. 6항 모형식·AR-Net 신경망 식·Prophet 원판과의 차이·하이퍼파라미터 설정은 부록 C.3.
...
- *Sobel 검정* (@sobel1982): 매개효과 $a b$의 표준오차 $sqrt(b^2 sigma_a^2 + a^2 sigma_b^2)$로 z-검정. 정규성 가정 의존.
...
*Prophet 원판과의 차이*: Prophet (@taylor2018)에는 $A(t), L(t)$가 *없고* 학습이 Stan MCMC. NP는 (a) AR-Net 추가, (b) lagged regressor 비선형화, (c) PyTorch SGD 학습 — 세 가지가 실질 확장.
