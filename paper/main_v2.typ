...
== 문제 의식 — "측정되는 것이 중요해진다"
  ...
  #cite(<bevan2006>, form: "prose")는 영국 NHS 사례 연구를 통해 "측정되는 것이 중요해진다(What's measured is what matters)"는 명제 아래 평가지표 게임화의 3가지 캐논 패턴(threshold effect, ratchet effect, output distortion)을 식별했다 — "gaming"은 이 셋을 포괄하는 우산 용어다. 이는 #cite(<goodhart1975>, form: "prose")와 #cite(<campbell1979>, form: "prose")이 각각 통화정책과 사회과학 일반에서 제안한 지표가 정책 도구로 채택되는 순간 그 지표의 측정 신뢰도가 하락한다는 명제의 행정학적 구체화다.

  같은 메커니즘이 한국 중앙정부 재정 집행에서도 작동하는가? 이 질문에 답하기 위해서는 (a) 게임화를 측정 가능한 양으로 정의하고, (b) 분야·부처·사업 형태에 걸친 이질성을 분리하며, (c) 자연 경기 cycle이나 추세 효과 같은 경합 가설을 배제해야 한다. 본 연구는 11년 시계열·14분야·1,557개 활동을 다중 방법론으로 결합해 이 세 과제를 단계적으로 수행한다.
...
== 데이터에서 출발 — 세 가지 관찰
  ...
  + 관찰 1 — 12월의 봉우리는 작은 사업에서 더 강하다: 각 사업을 연 집행액 100%로 정규화하면 12월 비중 평균이 12.1%로 11월(5.5%)의 2.2배다(균등 가정 8.33%의 약 1.5배). 금액 가중 풀링(×1.52)으로는 대형 사업에 가려졌던 이 쏠림이, 사업을 동등 무게로 평균 내면 드러나며, 규모 4분위로 보면 12월 비중이 소규모(Q1) 15.4%에서 대규모(Q4) 10.9%로, 12월/11월 배율도 Q1 2.58배에서 Q4 1.86배로 단조 감소한다. → 첫 질문: 무엇이 이 시점 압력을 만드는가?
...
== 왜 시민이 신경 써야 하는가 — 일상 영향 4경로
  ...
  본 연구의 발견은 학술적 검증에 그치지 않고 시민 일상과 닿아 있다. 다만 그 영향의 상당 부분이 현 측정 체계로는 확증하기 어렵다는 사실 자체가 핵심이다 — 집행률은 집행됐는가(output)만 보고 어떤 결과를 낳았는가(outcome)는 측정하지 않으므로, 아래 네 경로 중 견고한 것은 측정 격차의 직접 귀결(품질 희생)이고 결과 측 경로는 대체로 탐색적이다.

  + 복지 수혜의 우연성: 사회복지는 14분야 중 가장 강한 음 상관($r = -0.86$, CPI 통제 후, §6.7)을 보이지만, Bonferroni 보정 미달인 데다 기초연금·기초생활보장 같은 자동 소득이전이라 12월 집중 시점과 무관하다 — 12월 쏠림이 빈곤 격차를 좁힌 것이 아니라 어차피 분배될 예산이 12월에도 분배됐을 뿐이다. 따라서 이 상관은 정책 효과로 해석하지 않으며, 이를 정책 성과로 오인하면 진짜 빈곤 정책의 자원 배분이 왜곡될 수 있다.
...
== 본 연구의 핵심 기여
  ...
  + 기여 3 — 사업원형 이질성과 정직한 outcome 검정 (§6.3\~§6.7, H2/H3/H5): 게임화는 단순한 부정적 측정 왜곡이 아니라 사업원형 이질적 현상이다. 견고한 발견은 output 측에 있다 — 자산취득형 RDD 점프 3.42배(C1 군집 풀링, n=99 활동, H2), 출연금형 사이클 우세(H3). outcome 측에서는 14분야 중 부트스트랩으로 확증된 연관이 없었다 — 농림수산만 Sobel 유의했으나 미확증, 사회복지는 최강 상관($r=-0.86$)이나 메커니즘(자동 소득이전) 부재로 기각(H5). 이 측정 공백 자체가 집행률이 결과를 검증할 기반을 갖지 못함을 드러내는 진단적 발견이다.

  + 기여 4 — 시간 동적 강화 (§6.8, H6, Wavelet 신규): FFT 정상성 가정의 한계를 웨이블릿으로 보완해 게임화가 시간에 따라 강화됨을 새롭게 식별. 출연금형 12개월 cycle 진폭 2015\~2017 → 2023\~2025로 +554\% 증가 (COVID qualifier 포함: Pre-COVID 완만한 강화 +0.068/yr → COVID 충격 증폭 → elevated plateau 유지), 인건비형 변화 없음(통제). 한국 굿하트 효과는 고정 패턴이 아닌 진행 중인 동적 현상.
...
= 이론적 토대
  ...
  §1에서 데이터가 드러낸 세 관찰 — 무엇이 시점 압력을 만드는가, 그 단위는 무엇인가, output은 outcome을 대리하는가 — 은 "왜 이런 패턴이 생기는가"라는 질문을 남긴다. 본 절과 다음 절(이론 모형)은 그 답을 구성한다. 분석은 세 갈래의 이론적 전통을 결합한다. (1) 공공관리 이론에서 굿하트-캠벨 법칙과 NPM(New Public Management) 측정 패러독스, (2) 계약경제학에서 Holmstrom-Milgrom의 다업무 모형, (3) 비교경제학에서 Kornai의 연성 예산 제약이다. 각 이론은 본 연구의 다른 발견을 설명하며, 셋이 결합될 때 사업 형태별 게임화 강도 차이가 자연스럽게 도출된다.
...
== 다업무 계약 이론(Multitasking)
  ...
  공공 사업에 적용하면, 집행률(측정 가능)과 사업 품질(측정 어려움)이 상충하는 환경에서 집행률 인센티브가 강할수록 품질이 희생된다. 본 연구의 사업 형태별 RDD 점프 차이(자산취득형 3.42배[C1 군집 풀링, n=99 활동] vs 출연금형 1.10배 vs 인건비형 1.12배)는 측정 압력에 대한 반응성이 사업 형태별로 다르다는 이 가설의 한국 실증이다.
...
== 연성 예산 제약(Soft Budget Constraint)
  ...
  공공기관·출연기관은 직접 사업과 달리 사업 시점 조정의 자유도가 높다. 정산 기일을 맞추기 위해 12월 집중 집행이 가능하고, 다음 회계연도 예산이 감액되더라도 모기관 보전 가능성이 시장 규율을 약화시킨다. 본 연구의 출연금 비중과 게임화 강도 회귀(β=+0.375, p=0.005), 그리고 출연금형의 PSD k=1 진폭 0.332 + phase coherence 0.54(자산취득형·정상사업 대비 4\~7배)가 이 메커니즘의 정량적 증거다. 한편 RDD 12월 점프 3.42배(C1 자산취득형 군집 풀링, n=99 활동)는 자산취득형에서 가장 두드러지는데, 이는 시설 공사·자산 취득 사업이 공정률 마감 + 회계연도 마감 결합으로 12월 1일 cutoff 직후 이산적 점프를 만들어내는 다른 메커니즘이다.

= 이론 모형 — Principal-Agent 굿하트 게임

  앞 절(이론적 토대)이 세 가지 이론적 전통을 소개했다면, 본 절은 그 전통을 한국 평가 제도에 specialize한 정식 모형을 제시한다. 본 모형은 agent의 비합리성이 아닌 합리적 선택이 측정성 격차 환경에서 굿하트 게임을 유발함을 보인다. 평가 제도가 정의한 목적함수 아래에서 균형 행동이 시점 조정 노력으로의 자원 집중이다.
...
== 모형 setup — Principal과 Agent의 목적함수
  ...
  Fortuitous alignment의 정식 정의: 사회복지에서 관찰된 음 상관은 — 인과적이라면 — 모형 외생적 분야 함수 $alpha$의 우연한 양수성으로 표현되나, 실증에서는 메커니즘(자동 소득이전) 부재로 기각된다(§6.7). Agent의 목적함수 $U_A$가 $Y$를 포함하지 않으므로 모든 $Y$ 변화는 fortuitous — 사회복지가 음 상관을 보이는 것은 분야의 외생 특성이지 agent의 의도된 사회 기여가 아니다.

  본 정의는 분야 trivial vs 사회복지 자동분배 양립의 모형 기반이다 — 비용 함수 $c(\cdot; theta_("archetype"))$의 단위는 사업원형이지만 (H1, 분야 trivial), 결과 함수 $alpha(theta_("field"))$의 단위는 분야이다. 두 layer는 모형의 서로 독립한 차원이며, 사회복지의 outcome alignment는 분야 layer의 우연한 한 점에 해당한다.
...
== 균형점 도출 — 굿하트 게임의 1차 조건
  ...
  핵심 결과: $w_t >> w_q$인 환경(현행 한국 평가 제도) + $phi'(\cdot) < 1$ (측정성 격차)에서 효과적 품질 가중 $w_q phi'(e_q) < w_q << w_t$이므로 균형 $e_t^* >> e_q^*$ — Agent가 합리적으로 시점 조정에 노력 집중. 측정성 격차는 두 경로로 균형을 왜곡한다: (i) 평가 가중 비대칭 ($w_t > w_q$), (ii) 품질 노력의 측정 누설 ($phi' < 1$). 이것이 굿하트 게임의 미시적 기제다.

  굿하트 법칙은 흔히 "지표가 정책 도구가 되면 측정성이 무너진다"는 명제로 표현되지만, 본 모형의 미시적 도출은 보다 강한 명제를 제공한다 — 합리적 agent의 균형 행동이 측정성 붕괴를 유발한다. Agent의 도덕적 비난은 부적절하며, 제도 설계의 결함이 본질이다.
...
== Career Concerns — 출연금형의 집단 평판 균형
  ...
  본 절은 §3.3의 정적 모형을 다년 평판 게임으로 확장하나, 본 연구의 주요 검증(H1\~H6)은 §3.3의 정적 균형으로 도출되며 본 절의 동적 결과는 §6.4 출연금형 phase coherence 분석의 메커니즘적 해석에 한정 활용된다.
  ...
  동적 게임 setup: 연도 $t = 1, 2, ..., T$에서 활동 $i in cal(I)_("모기관")$ (동일 모기관 산하 출연기관 활동)이 시점 노력 $e_(i,t)$를 선택한다. 모기관의 진정한 평가 기준 $mu^* in RR$은 시점 조정의 효과적 가중을 결정 — 즉 활동 $i$가 인지하는 시점 평가 가중은 $w_t(mu^*)$로, $mu^*$가 클수록 집행률 평가의 가중이 큼 ($w_t' (mu^*) > 0$). $mu^*$는 불확실하며 agent들은 동료 활동의 평가 결과로부터 학습한다.
  ...
  여기서 $tilde(w)_t$가 $hat(R)_(t-1)$을 통해 $hat(mu)_(i,t)$에 의존하므로 $hat(R)$이 max에 실질적으로 진입한다 (단순 가법 항이 아닌 가중 함수 자체의 인수로 진입). 1차 조건은 $tilde(w)_t (hat(mu)_(i,t)) = c_t(e_(i,t)^*, e_(q,i,t)^*)$.
  ...
  이는 공공 정보 신호 하 coordination 균형(@morris2002)의 한국 변형으로 해석된다 — 단, 원 논문은 공공 신호 정밀도 증가가 과잉 동조로 사회후생을 낮출 수도 있음을 함께 보였으며, 본 연구의 평가 제도 정밀화 처방은 이 ambiguity를 인지한 위에서 설계되어야 한다.

  우리 실증과의 정확한 대응: 출연금형 phase coherence 0.54 (다른 원형 0.08\~0.13의 4\~7배; 부록 D.3)는 이 효과적 가중 동기화의 직접 측정이다 — 활동 $i, j$가 학습된 $hat(mu)$를 통해 동시에 피크에 도달함. 인건비형 phase coherence 0.41(통제)은 구조적 균등 지급에 따른 자연 동기화이며, 자산취득형(0.08)·정상사업(0.13)은 동료 평가 신호 학습이 약한 사업 환경 — 즉 모기관-위탁기관 결속이 출연금형보다 약함 — 의 발현으로 해석된다.
...
== 실증 검증 가설
  ...
  Performative Prediction과의 이론적 연결: H6는 #cite(<hardt2016strategic>, form: "prose")과 #cite(<perdomo2020performative>, form: "prose")의 Performative Prediction — 지표가 평가 도구로 채택되는 순간 그 지표가 측정하는 분포 자체가 변화한다 — 의 한국 재정 데이터 실증 사례에 해당한다. Agent가 평가 시스템의 구조를 학습해 시간이 지날수록 게임화 강도를 점진적으로 강화하는 적응적 행동이 그 메커니즘이다. 다만 §6.8 시기 분할 검증에서 보이듯, 한국 표본의 +554% 증폭은 Pre-COVID 완만한 학습 적응이 COVID 확장재정으로 급격히 증폭된 합성 패턴이며, 순수 monotonic 학습으로 환원되지 않는다. 이론적 연결은 유효하나 실증 해석에는 시기 분할 qualifier가 함께 적용된다(상세는 §6.8).

  본 모형은 6개 가설을 조직하며, 각 가설은 독립적인 데이터 분석으로 검증된다. 다만 H2(자산취득형 비용함수 형태)는 RDD 패턴을 모형 입력 가정으로 수용한 것이므로 순수 예측이 아닌 입력-결과 정합성 확인에 가깝고(§6.3 해석 주의 참조), H1·H3는 원형 간 조건부 예측의 성격을 갖는다. 이 구분을 명시함으로써 과적합 위험을 정직하게 다룬다.

== 굿하트 효과 분류와의 매핑 #cite(<manheim2018>)

  #cite(<manheim2018>, form: "prose")는 굿하트 효과를 4가지 변형으로 분류했다 — Regressional(노이즈 포함 지표의 극값 선택 편향), Causal(비-인과적 상관), Extremal(극단 영역 관계 붕괴), Adversarial(의도적 조작). 본 연구의 가설을 이 4유형에 매핑하면 다음과 같다 — 단 H4·H5는 §6.6·§6.7에서 각각 미확증·기각되므로, 아래 표는 유형 후보 매핑이지 확증된 분류가 아니다(H2·H3·H6은 작동 사례).
...
== 결과변수 (분야별 매핑)
  ...
  분야 수 일러두기: 원자료 분야 분류는 17개(중복 라벨·예비비 포함), 1,557개 활동이 실제 분포하는 분야는 16개, 결과변수를 확보한 분야는 15개(국방 포함), 매개·상관 분석 대상은 14개다. 본문에서 14분야는 결과(outcome) 분석 기준 분야 수를 가리키며, 임베딩·군집·RDD 분석은 활동(1,557) 단위로 수행되므로 분야 수에 의존하지 않는다.
...
= 방법론
  ...
  본 연구는 "게임화"라는 추상적 개념을 정량화하기 위해 신호 처리·차원 축소·위상 데이터 분석·인과 추론을 단계적으로 결합한다. 각 방법은 고유한 가정과 실패 양태를 가지므로 단일 도구 결과는 그 가정에 의존한다. 본 연구는 가정이 직교적인 도구를 병행해 (a) 도구 간 일치에서 가정 의존성 약화 (robustness)를, (b) 도구 간 불일치에서 어느 가정이 위배되는지 진단 정보를 함께 얻는 상보적 트라이앵귤레이션(complementary triangulation)을 따른다. 일치는 충분조건이지 필요조건이 아니다 — H5(§6.6)는 FFT/STL 측도 갈림이 결론 무효화가 아니라 측도 미결의 진단으로 작동하는 사례다. 본 절은 직관과 주제 적합성에 집중하며, 상세 수식·알고리즘 골자·대안 비교는 부록 C에 별도 정리한다.

== 방법론 트라이앵귤레이션의 원칙

  단일 도구의 결과는 그 도구의 가정이 옳을 때만 신뢰할 수 있다. 도구마다 서로 다른 실패 양태를 갖기 때문에, 가정이 직교적인 도구 두셋이 같은 결론을 지지하면 가정 stress test 통과로서 robustness가 강화된다. 불일치 시에는 어느 가정이 위배되는지 추적해 향후 측도 calibration 영역을 식별한다 (cf. H5 §6.6의 FFT vs STL trend-잔재 갈림). 본 연구는 게임화 측정에서 FFT(주파수 영역, 정상성 가정)·STL(시간 영역, 가법 분해)·NeuralProphet(신경망 가법모형, 변화점 모델링)의 세 도구를 사용한다. 각 도구의 가정과 약점을 표 1에 정리한다.
...
=== STL 분해(Seasonal-Trend decomposition using Loess)
  ...
  직관: FFT amp_12m_norm 신호가 진짜 게임화일까, 아니면 지속적으로 증가하는 추세가 12월 결산 시점 기록 방식과 결합해 만들어내는 가짜 주기성일까? 이 의문을 검증하기 위해 STL #cite(<cleveland1990>)을 사용한다. STL은 시계열을 가법 분해 $x_t = T_t + S_t + R_t$로 추세·계절·잔차로 분리하고, 추세 제거 후 계절 성분의 분산 비율을 `seasonal_strength`로 정의해 게임화 지표로 사용한다. 두 겹 반복(inner/outer loop)·robustness weight·정의식은 부록 C.2.

  주제 적합성: 한국 정부 예산은 IMF 구조조정(1998), 국가재정법 도입(2007), MTEF 5년 framework 적용(2009) 등 명확한 추세 변동 동인을 가진다. 이런 추세를 분리하지 않으면 FFT는 "추세 + 12월 게임화"의 합성 신호를 측정한다. STL은 비모수 LOESS로 임의 형태의 추세를 흡수해 순수 계절 성분만 분리한다 — 한국 예산의 비선형 추세에 대해 ARIMA 차분보다 적합하다. 본 연구의 핵심 발견인 사회복지 자동분배 신호가 STL 후 소멸한다는 점은 추세 혼재 가능성을 시사하는 자기 비판 증거가 된다.

=== NeuralProphet 신경망 분해 — 세 번째 cross-check

  직관: FFT(정상성 가정)와 STL(임의 추세 가정)이 서로 다른 결론을 줄 때 어느 쪽이 게임화의 진짜 신호인지 결정하기 위해 세 번째 독립 도구가 필요하다. NeuralProphet #cite(<triebe2021>)은 Facebook Prophet #cite(<taylor2018>)의 신경망 확장으로, 시계열을 추세 + Fourier 계절 + AR-Net 자기회귀 + 이벤트 + 외생 회귀의 6개 가법 성분으로 분해한다. 본 연구는 cross-check 비교 형평성을 위해 자기회귀(`n_lags=0`)·외생 회귀를 비활성화하고 추세 + 계절만 사용한다. 6항 모형식·AR-Net 신경망 식·Prophet 원판과의 차이·하이퍼파라미터 설정은 부록 C.3.
  ...
  세 도구 합의 기준: FFT `amp_12m_norm`, STL `seasonal_strength`, NP `yearly_seasonality` 진폭이 활동×연도 패널에서 동일 방향 신호를 보이면 측정 robustness가 강화된다 — 합의는 충분조건이지 필요조건은 아니다(§ 5.1 상보적 트라이앵귤레이션 원칙). 갈릴 경우 도구별 가정 차이가 진단 정보로 활용된다 (H5 사회복지 FFT vs STL 갈림이 그 사례, § 6.7).
...
== 사업 형태(원형) 발견 — 차원 축소 + 밀도 군집
  ...
  이 12피처는 예산 구성(편성목)·시점 분포(FFT·HHI·CV)·규모·추세를 포괄하나, 12개월 시점 시퀀스 자체가 아닌 요약 통계 벡터다. 이 입력 공간의 선택은 § 6.2의 클러스터 해석 한계(silhouette 0.240 vs 시점 공간에서 −0.179, ANOVA R²=0.065)와 직접 연결된다 — 클러스터는 편성목 유형 클러스터로 해석되며, 월별 집행 패턴은 사후 서술이다.
...
=== UMAP (Uniform Manifold Approximation and Projection)
  ...
  주제 적합성: 사업 활동의 12피처(예산 구성·집행 패턴·진폭)는 강한 비선형 상관을 갖는다 — 예: 출연금 비중과 게임화 진폭 사이 sigmoid 관계. 또한 1,557개 활동이라는 작은 데이터에서 자율 임베딩 학습은 과적합되며, 재현 가능한 결정론적 임베딩이 정책 보고서에 인용 가능해야 한다. UMAP은 (a) 비선형 보존 (b) 작은 N 안정성 (c) `random_state` 재현 가능 — 세 요건을 모두 만족하는 거의 유일한 도구다.
...
=== Mapper (kmapper)
  ...
  주제 적합성: "분야 단위 trivial"이라는 본 연구의 핵심 주장은 사업 형태들이 분야와 무관하게 위상적으로 분리된 component를 형성한다는 것에 의존한다. Mapper graph에서 4개 사업원형이 분리된 components로 나타나면 위상적으로 별개 단위임을 입증할 수 있다 — 단순 임베딩 거리가 아닌 연결성 부재가 증거다.
...
== 분야 라벨이 진짜 단위인가 — 고정효과 회귀
  ...
  판정 임계 (사전 등록): 본 분석은 $Delta R^2 < 0.01$를 trivial 판정 임계로 사전 설정한다. 분야 FE가 게임화 분산을 1% 이상 설명하지 못하면 trivial로 분류하며, 사업원형 상호작용이 추가로 1% 이상의 ΔR²를 산출하면 진짜 단위 우세로 판정한다. § 6.1 결과는 이 임계를 적용한 사후 보고다 (분야 ΔR²=0.000 < 임계, 원형×Δamp ΔR²=+0.025 > 임계).
...
=== 회귀불연속 설계(RDD)
  ...
  주제 적합성: 한국 회계연도 cutoff는 (a) 1948년 이래 불변, (b) 모든 부처에 동시 적용, (c) 개별 활동이 조작 불가능 — RDD의 세 핵심 가정을 만족하는 강한 식별 환경이다. 다만 월 단위 집계 자료와 McCrary 밀도 검정 미실시로 완전한 자연실험은 아니며, 인과 해석은 §8.2 식별 한계와 함께 읽어야 한다. #cite(<liebman2017>, form: "prose")가 미국에서 cutoff 며칠 차이로 4.9배 점프 + 품질 하락을 입증한 설계(미국 회계연도 9/30 종료)의 논리를 한국 회계연도(12/31 종료)에 적용 가능하며, 실제 분석에서 1.91배(전체) / 3.42배(자산취득형, C1 군집 풀링 n=99) 점프를 확인한다. 같은 활동의 단 며칠 차이를 비교하므로 분야·기관·사업 특성이 자동 통제되는 준실험이다.
...
== 외생 통제 — 자연 경기 cycle 가설 기각
  ...
  외생성 정당화: CPI는 한국은행이 통화정책 도구로 결정하므로 (a) 한은 독립성에 의해 재정부 행정 결정에 직접 노출되지 않음, (b) 목표값(2.0\%)이 사전 공표되어 역인과 위협이 작음. 따라서 한국 거시변수 중 재정 게임화로부터 가장 외생적인 후보이며 1990\~2025년 월별 공식 시계열로 신뢰성 있게 통제 가능하다. 결과: 14/14 분야에서 부호 유지, 70% 유의성 유지. 사회복지 신호는 $r = -0.76 arrow -0.86$으로 오히려 강화 — 자연 cycle 가설 강력 기각.
...
== 견고성 검증 — Permutation·Lag/Lead·CV
  ...
  Lag/Lead 분석: 게임화-결과변수 상관에 시차 $tau in {-1, 0, +1}$년을 주어 방향성을 점검. $r(0)$ 동기 상관이 가장 강하면 즉시적 메커니즘(사회복지 자동분배 가설 정합), $r(+1)$ 강하면 지연 효과, $r(-1)$ 강하면 역인과 의심.
...
= 결과 — 6 가설 다중 검증
  ...
  본 절은 §3에서 도출된 6개 가설(H1\~H6)을 각 가설별 sub-section으로 검증한다. 각 결과는 단일 회귀 계수가 아니라 복수 도구의 합의에 의해 지지될 때만 본문에 보고하며, 마지막 sub-section에서 방법론 robustness(STL 자기 비판 + NeuralProphet cross-check 통합)를 별도로 보강한다.
...
== H1 검증 — 분야 trivial, 사업원형이 진짜 분석 단위
  ...
  귀납적 도입 — 부처와 분야 두 행정 단위의 한계 EDA: 정량 검증 전에, 데이터가 두 행정 단위에 어떻게 분포하는가를 본다. (a) 부처 단위 — 51개 중앙 부처 모두에서 12월 비중 평균이 균등 가정 8.33%(=100/12)를 초과한다 (51/51 = 100%; >15% 부처 약 35%, 10–15% 부처 약 45%, ≤10% 부처 약 20%). 즉 12월 봉우리는 모든 부처 공통 패턴으로 부처 라벨이 시점 압력의 변동을 설명하지 못한다. (b) 분야 단위 — 14개 분야의 사업×연도 단위 12월 비중 중앙값 격차가 통일·외교 100.0% vs 사회복지 13.4% = 약 7.5배, 분야 내부 IQR도 평균 약 83%p / 중앙 약 91%p로 분야 평균이 분야 내부 다양성을 가린다. 두 EDA 관찰이 합쳐져 행정 분류는 시점 압력의 단위가 아니다는 귀납적 한계를 가리키며, 이어지는 정량 ΔR² 검증이 같은 결론을 통계로 확인한다.
  ...
  모형 검증 (H1): 본 결과는 P-A 모형의 비용 함수 $c(e_t, e_q; theta)$의 단위는 사업원형 $theta$이지 분야가 아니다는 가설을 직접 입증한다(@fig-h8). 분야 라벨은 행정 분류 편의일 뿐 비용 구조와 결정 행동에 정보를 담지 않는다.
...
== H1 보강 — 4개 사업원형의 위상 안정성
  ...
  극단 대표 활동 (illustrative): 각 군집에서 핵심 지표가 극단에 위치한 활동을 예로 들면 — C1 자산취득형은 12월 집중도가 가장 큰 「부곡병원기본경비(자본)」(12월 비중 42.1%)·「도로병목지점 개선」 등, C2 출연금형은 출연금 비중 100%·연 사이클 진폭 최대이나 12월 비중은 오히려 1.4%로 낮은 「기획평가기관지원」 등, C0 인건비형은 12월 9.3%로 균등한 「경찰병원인건비」 등, C3 정상사업은 평균 부근의 「국토교통정책지원」 등이 대표적이다. 이는 개별 사업에 대한 평가가 아니라, 같은 archetype 내 활동이 동일한 시점 압력 구조를 공유함을 보이는 예시다.
  ...
  - Persistent Homology: persistence diagram에서 다수의 연결성분과 loop가 확인된다(@fig-ph-pd, @fig-ph-barcode; barcode는 상위 30개 feature 표시). 개별 feature 개수는 부트스트랩 변동이 크므로(max persistence 95% CI [0.48, 1.19]), 강건성은 개수가 아니라 전체 persistence diagram에 Wasserstein-2 거리를 적용해 검증한다 — 관측 mean 0.885 vs null(feature-shuffled) mean 1.524, $p < 0.0001$로 전체 PH 구조가 귀무가설과 유의하게 다름이 확인된다(@fig-ph-bootstrap). 이는 개수 기반 CI보다 강건한 전체-diagram 검증이다.
  ...
  세 도구(UMAP+HDBSCAN, Mapper, PH)가 일관되게 사업 형태가 분야보다 강한 단위임을 지지하며, Mapper와 PH는 각각 manifold 연결성·위상 구조의 독립적 측면을 추가로 드러낸다.
...
#rect(
  ...
  UMAP+HDBSCAN 임베딩의 입력은 12개월 시점 시퀀스가 아니라 12개 스칼라 피처다 — 편성목 구성비 4개(인건비·자산취득·출연금·경상이전), FFT 요약 통계(amp\_12m\_norm, cycle\_ratio), HHI, 규모(log\_mean\_ep), 추세. 이 피처 공간과 실제 월별 집행 시점 공간 사이의 괴리는 실루엣 점수(silhouette score)에서 정량적으로 드러난다: 원본 12-피처 공간에서 silhouette = 0.240 (편성목 유형 차원에서 클러스터 분리), 12개월 시점 벡터 공간에서 silhouette = −0.179 (음수 — 시점 차원에서 클러스터 간 분리 거의 없음). 따라서 C0/C1/C2/C3는 편성목 유형 클러스터로 해석해야 하며, 월별 집행 패턴은 클러스터 평균을 통해 사후적으로 서술되는 것으로 클러스터 내 분산이 존재한다. ANOVA R² 평균 0.065는 클러스터 라벨이 월별 비중 분산의 6.5%만 설명함을 보여준다. 특히 C2(출연금형, n=154)는 bimodal 구조다 — 연초 일괄 출연 단조 감소형과 분기말 사이클형이 한 클러스터에 묶여 있으며, 클러스터 평균의 cycle\_ratio = 0.42(연주기 우세). C3(정상사업, n=1,175 = 75.5%)는 잔류 클러스터 성격으로 내부 L2 분산이 가장 크다 (median 0.153, p95 0.363, outlier 59개). RDD 추정치(자산취득형 3.42배 등)는 편성목 클러스터 단위 풀링 추정이며, 월별 집행 이질성이 클러스터 내에 존재함을 고려하여 해석되어야 한다.
]

== H2 검증 — 자산취득형 회계연도 12월 RDD 점프

  왜 RDD가 결정적 증거인가: 사업 본질에 따른 정상적 변동과 회계 게임화를 분리하기 위해, 같은 활동의 11월 마지막 주 vs 12월 첫 주 일평균 집행을 비교한다. 며칠 차이의 변동은 사업 본질로 설명할 수 없고, 12월 1일이라는 행정적 임의 절단점만이 작동한다. #cite(<liebman2017>, form: "prose") (AER)가 미국 연방조달에서 4.9배 점프를 보고한 설계(9/30 cutoff)의 한국(12/31 cutoff) 적용이다.
  ...
  미국 4.9배 대비 한국 1.9배의 절대값 차이는 측정 단위 granularity 차이로 설명된다(미국 주 단위 vs 한국 월 단위 — 12월 첫째 주의 신호가 12월 전체로 분산). 사업 형태별 RDD 점프의 순서는 #cite(<liebman2017>, form: "prose")의 use-it-or-lose-it 메커니즘이 공정률·자산취득 사업에서 가장 강하게 작동함을 보여준다.

  측정 단위 granularity에 대한 강건성: #cite(<liebman2017>, form: "prose") 결과(4.9×)와의 차이가 월 vs 주 단위 측정 차이의 산물인지 검증하기 위해 가상 주별 시나리오 4개를 계산했다.
  ...
  분기말 집행관리 사이클의 흔적 — multi-cutoff 시스템의 가시화: @fig-rdd-monthly 의 11년 평균 시계열은 12월뿐 아니라 3월·6월·9월에도 부수 점프를 보인다. 이는 기획재정부 분기별 집행관리제도(분기마다 집행률 점검)의 직접적 흔적이며, 12월 점프가 압도적으로 큰 이유는 분기 마감 + 회계연도 마감 + 차년도 이월 차단의 3중 deadline이 12월에 중첩되기 때문이다. 분기말 부수 점프의 존재는 H2 결론을 약화하지 않고 오히려 강화한다 — 행정적 임의 절단점이 1개가 아닌 4개이며, 게임화 압력이 KPI deadline 밀도 전반에서 작동하는 multi-cutoff 시스템임이 그림에 직접 가시화된 것이다. 이는 #cite(<liebman2017>, form: "prose")의 단일 fiscal-year cutoff 분석과 한국 분기 + 연 hybrid 시스템 간의 제도 구조 차이를 시사하는 추가 단서로, 앞 단락 granularity 결론("제도 구조적 차이가 본질")의 구체적 instance에 해당한다.

  Multi-cutoff × archetype 분해: 같은 RDD 추정을 4개 분기말 cutoff × 4개 archetype으로 분해하면(@fig-rdd-cutoff-archetype; log\_daily ~ T + C(year), bw=1, cluster SE by ACTV\_CD; data/results/H22\_quarterly\_cutoffs.csv), 원형별로 어느 경계에서 점프가 작동하는가가 갈라진다. C1 자산취득형은 4개 cutoff 모두 유의 — 6월 2.50× ($p < 10^(-18)$), 9월 1.80× ($p < 10^(-10)$), 12월 3.42× ($p < 10^(-41)$), 3월 1.53× ($p < 0.001$) — 공정률 마감이 분기마다 누적되는 구조다. C2 출연금형은 4개 cutoff 모두 통계 미달 (12월 1.10× $p = 0.27$ 포함) — RDD가 잡지 못하는 연 단위 사이클이 §6.4에서 별도 도구로 드러난다. C3 정상사업은 12월 2.24× 외에 6월 1.44× ($p < 10^(-49)$)로 약한 multi-cutoff 동반. 본 분석은 12월 RDD를 일차 검증 도구로 보고하되, archetype별 cutoff 응답성 격차가 H2(점프)와 H3(사이클) 도구 선택의 경험적 정당화에 해당한다.
  ...
  모형 검증 (H2): 본 결과는 P-A 모형의 사업원형별 비용 함수 $c(e_t, e_q; theta)$ 차이가 균형 $e_t^*$의 시간 구조 차이로 발현됨을 직접 입증한다. 자산취득형은 $c(e_t)$가 12월 1일 직후 급감(공정률 마감과 회계 마감이 동시에 만료되는 임의 시점)하므로 RDD 점프 가장 큼(H2). 두 균형은 모형의 동일 FOC $w_t = (partial c) / (partial e_t)$가 원형별 비용 함수의 차이를 통해 다른 시간 구조로 발현된 것이다.
...
세 측도 모두 일관되게 출연금형이 연 단위 게임화 사이클에 가장 강하게 결속됨을 보여준다. 이는 RDD 점프(이산적 spike)는 자산취득형이 강하지만, 사이클 진폭(연속적 강도)은 출연금형이 강함이라는 사업원형별 시간 구조 차이를 입증한다.
...
== 방법론 상보성 검증 — FFT·STL·NeuralProphet 측도 비교
  ...
  본 절의 범위: 본 절은 세 게임화 측도(FFT·STL·NeuralProphet)의 상보성을 점검한다. H4(매개 경로 원형 이질성) 검증은 §6.6에서 수행된다. 본 절의 결과는 §방법론 도입에서 선언한 상보적 트라이앵귤레이션 가설의 정량 입증으로 해석된다 — 본 연구는 처음에 FFT·STL·NeuralProphet을 "서로 다른 결론을 줄 수 있는 cross-check 도구"로 설정했으나, 실제 분석 결과는 세 도구가 게임화의 다른 측면을 측정하는 상보적 lens임을 시사한다. FFT는 주파수 영역의 진폭 비중, STL은 시간 영역의 추세-잔차 분해, NP는 changepoint 보정 후 가법 분해다. 셋이 동일 신호의 직교적 표현이므로 약한 상관은 모순이 아니라 각 도구가 잡아내는 게임화 차원의 이질성을 정량화한 것이며, 이는 §방법론 표 1의 "상호 보완 관계" 설계가 의도한 결과다.
  ...
  분야별 outcome 상관 (3-way): 14분야 outcome 상관을 세 측도로 재산출한 결과, 보건(life expectancy)·통신(broadband)에서 세 측도 모두 음(보건: FFT $-0.55$, STL $+0.20$, NP $-0.71$; 통신: FFT $-0.15$, STL $+0.05$, NP $-0.75$)을 보이며, NP가 FFT가 놓친 통신 분야의 게임화 신호를 추가로 식별한다. 반면 사회복지는 세 측도 모두 약한 신호(FFT $+0.03$, STL $+0.00$, NP $-0.24$)로 14분야 단위 STL에서 신호 소멸한 결과와 활동-단위 NP에서도 일관된다.
  ...
  결론: 셋이 같은 신호를 다른 차원으로 분해하므로, 본 분석의 견고성은 한 도구의 절대값이 아닌 복수 도구가 일관되게 가리키는 분야·원형 패턴에 의존한다. 보건·통신·국방·교육의 FFT·NP 음 신호 (STL은 측도 의존), 출연금형 12월 phase coherence 0.54 (PSD/Phase 분석으로 추가 확인), Spec A·B 양쪽에서 C2 cycle 최강(@tbl-arnet-forecast)은 방법론 트라이앵귤레이션의 성공 사례다.
...
== H4 보강 — 매개경로 원형 이질성 (Baron-Kenny + Sobel + Bootstrap)
  ...
  분야별 매개 분해:
  - 농림수산: Sobel z = $-2.897$, $p = 0.004$로 14분야 중 유일하게 간접효과 z-검정이 유의했다. 그러나 부트스트랩 95% 신뢰구간이 0을 포함해(연 단위 $n = 5$ 소표본 — Sobel은 소표본에서 신뢰도가 낮다) 권장 기준으로는 미확증이다. 따라서 통계적 확증이 아닌, 출연금 비중 → 12개월 진폭 → 농가소득의 메커니즘이 그럴듯한 사례적 예시로 보고한다(수확기·재해보험처럼 결과가 집행 시점에 민감한 구조).
...
== H5 검증 — 사회복지 최강 상관의 기각 (메커니즘 규율)
  ...
  가장 강한 상관, 그러나 기각: 사회복지에서는 12월 집중 집행 강도가 클수록 순자산 지니계수가 감소(불평등 완화)하는 음의 상관이 관찰되며, 그 크기가 14분야 중 가장 강하다. 표면적으로는 게임화가 긍정적 결과와 연결되는 듯 보인다. 그러나 사회복지 급여·보조금(기초연금·기초생활보장 등)은 자동 소득이전이라 12월 집중 시점과 무관하게 분배된다 — 12월 쏠림이 빈곤 격차를 좁힌 것이 아니라, 어차피 분배될 예산이 12월에도 분배됐을 뿐이다. 따라서 본 분석은 이 최강 상관을 인과적 게임화 효과로 해석하지 않고 기각한다. 사회복지는 "상관 강도가 아니라 메커니즘으로 거른다"는 규율(falsification)의 사례로 기능하며 — 가장 강한 상관조차 메커니즘이 없어 버린다 — 이 규율이 약하지만 메커니즘이 그럴듯한 농림수산(§6.6)을 사례로 남긴 선택을 정당화한다.

  본 연구의 permutation test는 5단계 outcome 변수 검증 후 확정된 변수를 기준으로 사용한다. 교육 분야는 `imd_edu_rank`로 매핑하며, 국방 분야를 포함해 총 15개 분야를 대상으로 한다.

  사회복지 분야의 1차 차분 상관은 r=−0.762(p=0.035, permutation 1,000회)로 14분야 중 부호상 유일하게 유의했으나, 14분야 동시 검정의 Bonferroni 보정 임계값(0.05/14 = 0.0036)에는 미달한다(§8.2) — 단일 분야 유의를 strict하게 주장할 수 없다(@fig-h6). Lag/Lead 차분 상관(k=−2..+2)에서 동기 상관이 가장 강해 즉시적 분배 메커니즘과 정합하며, amp_cv 대안 지표에서도 방향이 일관된다(@fig-h6-lag). CPI 외생 통제 후 r=−0.86(p=0.007)로 부호·크기가 강화되어 자연 경기 cycle 가설과는 구분되나, 이는 상관의 부호 견고성일 뿐 인과적 게임화 효과의 확증은 아니다(@fig-h10).

  측도 의존성 — 중요 qualifier: 본 H5 결과는 FFT amp_12m_norm 측도에 의존한다. STL seasonal_strength 측도로 전환 시 r=−0.762 → +0.003으로 신호가 완전히 소멸한다(§6.9 상세). 이는 (a) FFT가 사회복지의 지속 증가 추세를 흡수하여 가짜 주기성이 생성된 것이거나, (b) STL이 진짜 게임화 점프를 추세로 흡수하여 신호를 놓친 것이거나의 미해결 ambiguity를 의미한다. 본 연구는 두 측도를 모두 보고하며 결론을 단일 측도에 귀속하지 않는다. NeuralProphet(changepoint 명시 모델링)의 활동-단위 결과는 r=−0.24로 음 신호를 부분 회복하여 FFT 해석을 지지하나, STL-FFT 불일치는 측정 도구 선택에 따른 본질적 불확실성으로 남는다.
...
== H6 검증 — 시간 동적 강화 (Wavelet)
  ...
  $w_q$ 측면은 거의 변화 없음 (사업 성과 측정 기술의 본질적 어려움 그대로). 따라서 $w_t / w_q$ 비율 증가는 합리적 추론이며, wavelet 결과(+554\%)는 이 추론을 직접 검증한다.
...
== 시간 진화 TDA — 공간 위상 × 시간 위상 dual framework
  ...
  해석: TDA와 wavelet은 서로 다른 메커니즘을 측정한다. Wavelet은 주기 진폭(timing intensity)을, TDA는 공간 형태(structural composition)를 잡는다. 두 측도가 직교한다는 사실은 중복 측정이 아닌 상보적 관점이라는 framework의 정당성을 강화한다 — H1의 archetype 구조는 시간에 안정적이고, H6의 cycle 진폭은 시간에 변화한다는 분리된 두 발견이 본 연구의 dual-위상 framework로 통합된다.
...
== 부분 structural calibration — $w_t / w_q$ 비율 외부 추정
  ...
  H6 시간 강화 메커니즘 — 가중 채널 vs 비용 채널: 재정사업자율평가의 비율은 2005년 이래 stable 0.75다. 따라서 H6의 +554% 진폭 강화는 가중 채널($w_t / w_q$ 상승)으로 설명되지 않는다. 두 가지 대안 메커니즘:
...
== 거시 outcome 보완 — ALIO 공공기관 경영평가
  ...
  매칭 전략: ALIO 기관명 → 1,557개 활동(ACTV_NM)과 직접 매칭은 0% (다른 추상 단위). 그러나 주관 부처(OFFC_NM) 기준 매칭 97% — 부처 단위에서 평가 등급과 H3 wavelet amp_12m 간 상관 분석 가능.

  향후 활용: ALIO 기관 수준 평가 등급이 낮은 부처가 H3 amp_12m이 높은가 (부처 단위 spurious correlation? mediation through archetype?)를 검증하는 보조 분석을 별도 절에 추가할 수 있다. 본 분석은 데이터 수집 + 매칭 인프라까지로 한정하며, 본격 분석은 공공데이터포털 다운로드 권한 확보 후 추가 분석 예정이다 (현재 PDF 장벽으로 자동 수집 한계).

  데이터 한계 명시: 자율평가 결과의 사업 단위 등급은 공식 공개되어 있으나 PDF/이미지 형태로만 제공되어 자동 수집이 어렵다. 본 연구는 이를 재현 가능성 인프라 부재로 간주하며, 향후 정부의 기계 가독 결과 공개가 학술 후속 연구의 전제 조건임을 명시한다.
...
== 방법론 Robustness — STL 자기 비판 + NeuralProphet·Wavelet 중재
  ...
  - Wavelet 시간 진화: 사회복지 활동의 12개월 cycle 진폭이 시간에 따라 변화하는지 추적. 변화가 있다면 정상성·trend 분해 가정 자체가 한국 자료에 부분 부적합(부록 D.4 참조).
...
= 정책 함의
  ...
  앞 절들의 발견을 종합하면 세 가지다 — (1) 게임화의 실질 단위는 분야가 아니라 사업원형이고(H1), (2) 견고한 신호는 시점 집중(output) 측에 있어 자산취득형 RDD 점프·출연금형 사이클·+554\% 시간 강화로 나타나며(H2·H3·H6), (3) 결과(outcome) 측 연관은 미확증으로 남아 집행률이 정책 결과를 검증할 기반을 갖추지 못함이 드러났다(H5). 이 진단을 바탕으로, 본 절은 정책 권고를 "어디를 점검할 것인가"(진단)와 "무엇을 바꿀 것인가"(처방)의 두 층위로 도출한다.

== 진단 도구 — 부처-결과변수 4분면

  왜 4분면인가: 시점 집중 노출과 결과변수 부호의 조합으로 분석 단위를 4사분면에 배치하면, 같은 노출 강도라도 결과 부호가 다른 단위를 분리할 수 있다. 점검 자원을 배분할 때 일률적으로 노출이 강한 곳을 보지 말고, 시점 집중 + 결과변수 악화가 동시에 나타나는 단위를 우선한다는 의사결정 도구다. § 6.1·§ 6.2 발견에 따라 분야 라벨은 trivial이고 사업원형이 진짜 분석 단위이므로, 본 도구는 부처 차원과 사업원형 차원을 이중으로 적용해 해석한다 (archetype 차원 4분면은 § 7.1.1).
  ...
  해석 주의: 부처 라벨 단위에서 Q2로 분류된 부처가 왜 측정-결과 불일치를 보이는지의 원인 변수는 부처 내부 archetype 구성에 있다 (§ 6.1: 분야 ΔR²=0.000 vs 원형×Δamp ΔR²=+0.025). 따라서 부처 4분면은 1차 진단, archetype 4분면 (예: 출연금형 + 결과 음 상관 분야 식별)이 2차 진단으로 함께 적용되어야 한다. 한편 결과변수 4분면은 결과변수 확보가 가능한 12개 분야에 한정된다(국방·공공질서·예비비 등 제외, §4 결과변수 매핑 참조) — 결과 미측정 분야는 본 진단의 사각지대로 남으며, 이 측정 공백 자체가 권고 4(데이터 인프라)의 직접 근거다.
...
== 처방 — 모형 균형점 이동의 행정 액션 매핑
  ...
  권고 1 — 다년도 회계 도입 확대 ($w_t$ ↓): 현행 단년도 예산 마감 구조는 $w_t$ 가중치를 극단적으로 크게 만드는 제도적 장치다 — 미사용 예산이 자동 소멸되므로 12월 31일 직전이 실질적인 절단점이 된다. 행정 경로로는 현행 「국가재정법」 제46조(불용액 이월 제한)·제7조(국가재정운용계획)를 개정해 자산취득성 사업의 자동 이월·다년도 세출 배정을 허용하는 방안이 있다. MTEF(2009 도입)를 5년 framework에서 실제 회계 단위로 격상하면 $w_t$가 크게 감소하고 $e_t^*$도 따라 감소할 것으로 예측된다. 미국 GPRA(Government Performance and Results Act) 다년도 funding 사례 참조.

  권고 2 — 출연기관 평가 지표 전환 ($w_t$ ↓ + $w_q$ ↑): 출연금형 phase coherence 0.54 + PSD k=1 진폭 0.332는 모기관-위탁기관의 12월 일률 정산 압력의 직접 증거다. 출연기관 경영평가에서 집행률 100\%의 비중 축소($w_t$ ↓) + 사업 품질 평가(피인용·인용·산출물 평가) 가중 확대($w_q$ ↑)가 두 레버를 동시에 움직인다. 다만 $w_q$ 증가의 효과는 측정성 격차의 본질적 한계(Holmstrom-Milgrom impossibility)로 일부에 그친다.
  ...
  권고 5 — 자동 모니터링 시스템 ($c_(t t)$ ↑): 본 분석의 극단 게임화 활동 식별 알고리즘(amp_12m_norm 상위 + RDD 점프 3배 이상)을 상시 운영하면 시점 집중 패턴의 조기 식별 가시성이 높아져 $(partial c) / (partial e_t)$ 증가 → $e_t^*$ 감소가 기대된다. 감사원·국회 예산정책처와 협력해 실시간 패턴 모니터링 dashboard 구축 가능.
...
+ 다중 비교 보정 한계 (정직 명시): H5 사회복지 음 상관(1차 차분 $r = -0.762$, $p = 0.035$; §6.7서 메커니즘 부재로 기각)은 14개 분야 동시 permutation test 결과 중 하나다. 이를 14개 가설에 대한 다중 비교로 간주하면 Bonferroni 보정 임계값은 $0.05 / 14 = 0.0036$이 되며, 사회복지의 $p = 0.035$는 이 보정 후 임계값을 통과하지 못한다.

  이 상관의 부호는 견고하나(CPI 통제 후 $r = -0.86$, Spearman·Kendall·LOO 일관), 그것이 인과적 게임화 효과를 뜻하지는 않는다: (a) Bonferroni/FDR 보정 후 단일 분야 통계 유의를 주장할 수 없고, (b) 자동 소득이전이라 12월 시점과 무관해 메커니즘이 부재하다. 모형 §3이 alignment 함수 $alpha$의 분야별 이질성을 사전 인정한 것은 왜 음 상관이 나타나도 인과로 읽지 않는지의 이론적 근거이지 연관의 견고성 근거가 아니다. 따라서 본 상관은 기각된 사례로 보고하며, 후속 연구에서 다른 국가의 자동분배 환경에서 동일 패턴이 재현되는지 검증이 필요하다.
...
= 결론
  ...
  기여 3 — 직관 반대 발견 (모형 H2·H3·H5 검증): 게임화는 단순히 부정적 측정 왜곡이 아닌 분야 이질적·시간 동적 현상임을 보였다. PH loop 분석에서 C2 출연금형 내부 sub-heterogeneity가 추가로 식별되어 4-archetype 분류의 post-archetype 분석 여지도 명시한다.
  ...
  추가 강건성·구조 분해·calibration 기여: 추가 분석은 본 연구의 핵심 발견을 세 방향으로 보강한다. 첫째, H5 상관의 부호는 Spearman $rho = -0.833$, Kendall $tau = -0.714$, 8회 Leave-One-Out 전 부호 보존으로 측도·표본에 걸쳐 견고하다(단, 이는 부호 견고성일 뿐 인과 확증이 아니며, 사회복지는 §6.7에서 메커니즘 부재로 기각). 둘째, 시간 진화 TDA(PH)와 wavelet의 직교성($r = 0.242$, $p = 0.473$)은 두 도구가 중복이 아닌 상보적 차원 — 공간 위상(구조) vs 시간 위상(주기) — 을 측정함을 입증한다. 셋째, 공식 평가편람에서 추출한 $w_t / w_q$ 비율이 2005년 이래 stable 0.75임을 확인, H6의 +554% 진폭 강화가 가중 채널이 아닌 비용 채널 + COVID 충격으로 메커니즘적으로 재해석되었다.

  시민 일상영향과의 매듭짓기 — § 1.2 4경로 재정리: 서론에서 제기한 시민 일상 영향 4경로는 본 분석 결과에 비추어 다음과 같이 정리된다 — 견고한 것은 측정 격차의 직접 귀결(공공서비스 품질)이고, 결과 측 경로는 대체로 탐색적·미확증이다.
  - 복지 수혜의 우연성: 사회복지 분야 $r = -0.86$은 최강 음 상관이나 Bonferroni 미달이고 자동 소득이전이라 12월 시점과 무관해 기각된다 (§ 6.7). 이 상관을 정책 성과로 오인하면 빈곤 정책 자원 배분이 왜곡된다.
  - 농가 실수령: 농림수산만 Sobel $z = -2.897$, $p = 0.004$ (§ 6.6)로 유의했으나 부트스트랩 미확증($n = 5$)이라 사례적 예시다. 12월 쏠림이 농가 자금 흐름에 영향을 줄 수 있으나 인과는 확증되지 않았다.
  ...
  광역 framing: 한국 재정 집행의 굿하트 게임이 고정 패턴이 아닌 진행 중인 동적 현상이라는 본 발견은, 평가 제도 자체가 측정 대상 행동의 분포를 변화시킨다는 Performative Prediction (@hardt2016strategic; @perdomo2020performative)의 한국 인접 사례로 해석될 수 있다 — 단, 원 framework이 다루는 distribution shift의 stable equilibrium 수렴과 본 분석의 +554\% 동적 강화는 직접 예측 관계가 아니라 학습 적응 + COVID 충격 합성 메커니즘으로 §6.8에서 분리·재해석된다. 후속 연구는 국제 비교(미국·일본·EU)와 자연 실험을 통해 어떤 제도 조건에서 이 동적 강화가 억제되는가를 밝힐 수 있을 것이다.
...
세 도구는 동일 신호를 직교 차원으로 분해한다. "출연금형 12월 게임화"라는 본 연구의 핵심 발견은 세 도구가 (가정 직교성에도 불구하고) 동일 archetype을 신호 후보로 식별한다는 점에서 — FFT k=1 진폭 33\%, NP yearly seasonality 진폭 최대, STL trend 후 잔여 신호도 출연금 우세 — 가정 stress test 통과로 해석된다. 이는 상보적 트라이앵귤레이션(§4)의 robustness 형태이지 단일 도구 결과의 기계적 재확인이 아니다 — 한 도구의 가정이 위배되어도(예: STL이 H5 신호를 추세로 흡수, §6.6) 신호 후보 식별 자체는 잔존한다.
...
네 도구는 시간 차원의 해상도에서 점진적으로 강해지며, 함께 사용 시 정상성·추세·changepoint·동적 진화를 모두 다룬다. 본 연구의 게임화 측정은 이 4중 lens에서 출연금형 사이클 우세 + 시간 강화 결론으로 수렴한다(자산취득형은 RDD 점프에서 별도로 우세 — 부록 E 참조).
