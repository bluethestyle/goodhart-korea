...
=== NeuralProphet 신경망 분해 — 세 번째 cross-check
  ...
  세 측도(`amp_12m_norm`·`seasonal_strength`·`yearly_seasonality`)의 합의·발산 해석 기준은 § 방법론 도입·표 1과 같다(합의=robustness, 발산=진단; H5 사회복지 FFT vs STL 갈림이 그 사례, § 6.7).
...
= 결과 — 6 가설 다중 검증
  ...
  본 절은 §3에서 도출된 6개 가설(H1\~H6)을 가설별 sub-section으로 검증한다. 각 결과는 복수 도구의 합의로 지지될 때만 보고하며(트라이앵귤레이션 원칙은 § 방법론), 마지막 sub-section에서 STL 자기 비판으로 방법론 robustness를 보강한다.
...
== 방법론 상보성 검증 — FFT·STL·NeuralProphet 측도 비교
  ...
  본 절의 범위: 본 절은 세 게임화 측도(FFT·STL·NeuralProphet)의 상보성을 점검한다(H4 매개 검증은 §6.6). FFT는 주파수 진폭 비중, STL은 추세-잔차 분해, NP는 changepoint 보정 가법 분해로 동일 신호를 직교 분해하므로, 약한 상관은 모순이 아니라 §방법론 표 1이 설계한 상보성의 정량 입증이다.
...
== H5 검증 — 사회복지 최강 상관의 기각 (메커니즘 규율)
  ...
  측도 의존성 — 중요 qualifier: 본 H5 결과는 FFT amp_12m_norm 측도에 의존하며, STL seasonal_strength로 전환 시 r=−0.762 → +0.003으로 신호가 소멸한다. 이 측도 의존성(STL 자기 비판 + NeuralProphet·Wavelet 중재)은 §6.9에서 별도 절로 상세 분석하며, 본 연구는 결론을 단일 측도에 귀속하지 않는다.
...
== 거시 outcome 보완 — ALIO 공공기관 경영평가
  ...
  기존 한계와 보완 시도: 본 연구의 분야별 outcome 변수는 거시 지표로 signal-to-noise가 약하다. 이를 보완하고자 미시 outcome인 ALIO 공공기관 경영평가 등급(2022–2024, 105개 기관, S/A/B/C/D/E)을 수집했다. 기관명↔활동(ACTV_NM) 직접 매칭은 0%이나 주관 부처(OFFC_NM) 기준 97% 연결되어, 부처 단위에서 평가 등급과 H3 amp_12m 상관을 분석할 기반은 마련된다.
...
== 방법론 Robustness — STL 자기 비판 + NeuralProphet·Wavelet 중재
  ...
  (a)와 (b)를 어떻게 구분하는가: 두 보조 검증이 응답한다 — NeuralProphet 중재(§6.5)는 changepoint 명시 모델링으로 사회복지 r=−0.24 음 신호를 부분 회복(STL의 0.003보다 강함)하고, Wavelet 시간 진화(부록 D.4)는 정상성·trend 분해 가정이 한국 자료에 적합한지를 점검한다.
...
= 결론
  ...
  추가 강건성: H5 상관 부호는 Spearman·Kendall·LOO 전반에서 견고하고(부호 견고성일 뿐 인과 확증 아님; 사회복지는 §6.7서 기각), 시간 진화 TDA와 wavelet의 직교성($r = 0.242$)은 두 도구의 상보성을, $w_t / w_q$ 비율 stable 0.75 확인은 H6의 +554% 강화가 가중 채널이 아닌 비용 채널 + COVID 충격임을 보강한다(상세는 §6.7·§6.8).
