...
#[
  ...
  본 연구는 한국 중앙정부 재정 집행에 *굿하트 효과@goodhart1975*가 발현하는 *미시적 기제*를 Principal-Agent 균형 분석으로 도출하고, 그로부터 파생되는 6개 검증 가능 가설(H1\~H6)을 11년 시계열·14분야·1,557개 활동·11개 방법론으로 다중 검증한다.

  *이론*: 평가 가중 $w_t$(시점 조정, 측정 가능)와 $w_q$(사업 품질, 측정 어려움)의 *측정성 격차*가 큰 한국 평가 제도(집행률 평가·출연기관 경영평가·감사) 환경에서 *합리적 행위자*의 균형 행동이 *시점 조정 노력 $e_t$로 자원이 쏠리는 굿하트 게임*임을 도출한다(Holmstrom-Milgrom 1991의 한국 적용).

  *실증*: 활동 단위 임베딩(UMAP+HDBSCAN)으로 *4개 사업원형*(인건비형·자산취득형·출연금형·정상사업)을 발견(Mapper·Persistent Homology로 위상 안정성 입증, H1). 분야 고정효과 회귀에서 분야 라벨의 추가 설명력 $Delta R^2 = 0.000$이나 *사업원형×지출진동* 상호작용은 $Delta R^2 = +0.085 \~ +0.094$로 유의(H1, *분야 trivial*). 회계연도 12월 RDD 점프는 *자산취득형 3.42배*(H2), *출연금형*은 PSD k=1 진폭 0.332 + phase coherence 0.54 + wavelet $+554\%$로 *시간 동적 강화* 우세(H3, H6) — #cite(<liebman2017>, form: "prose") 미국 결과의 *질적·동적* 한국 확장. 사회복지 자동분배 효과 ($r = -0.86$, CPI 통제 후 강화)는 $(partial Y) / (partial e_t) > 0$인 *우연한 alignment* 분야의 발현으로 모형이 명시적으로 frame한다(H5).

  *정책*: 모형의 비교정역학 ($(partial e_t^*) / (partial w_t) > 0$ 등)에서 6개 정책 권고를 *직접 도출* — 다년도 회계, 출연기관 평가 지표 전환, 정산 시점 분산, 데이터 인프라, 자동 flagging, 시간 가중 점검. Holmstrom-Milgrom impossibility의 본질적 한계를 정직하게 명시한다.
]
...
== 미국 선행연구의 한국적 확장
  ...
  본 연구는 동일한 RDD 설계를 한국 데이터에 적용한다. 한국 자료가 월 단위 granularity인 점을 고려해 11월 vs 12월 일평균 집행을 비교한 결과 *전체 1.91배, 자산취득형 3.42배*의 점프를 확인한다. 미국 5배 대비 절대값은 작지만 사업 형태별 차이가 뚜렷하다는 점에서 *질적으로 같은 메커니즘이 한국에서도 작동*함을 시사한다.

== 본 연구의 핵심 기여 (5축)

  본 연구의 기여는 *원리·실증·정책*의 3-축에 *방법론*과 *정직한 한계*를 더한 5-축이다. 본문 각 섹션에서 정식 검증되며, 결론에서 다시 5-축으로 요약한다.
  ...
  + *기여 3 — 직관 반대 발견 (§6.2\~6.5, H2/H3/H5)*: 게임화는 단순한 부정적 측정 왜곡이 아니라 *분야 이질적·사업원형 이질적* 현상이다. 자산취득형 RDD 점프 3.42배(H2), 출연금형 사이클 우세(H3), 사회복지 *fortuitous alignment*에 의한 자동분배 효과(H5)가 핵심.
...
== 다업무 계약 이론(Multitasking)
  ...
  *한국 공공기관 평가 게임화 문헌*: 한국 행정학·재정학 KCI 학술지에서도 출연기관 PBS(Project-Based Funding System)와 공공기관 경영평가의 게임화·ratchet effect를 다룬 연구가 축적되어 있다. 김근세·이만형 @kim2009pbs, 박정수 @park2014pbs, 임도빈 외 @lim2020kpi 등이 PBS 도입 이후 R&D 성과 지표의 *수치 inflation*과 *전략적 보고*를 보고했다. 본 연구는 이를 *11년 시계열 활동 단위 데이터*로 확장해 *원형별 정량 차이*까지 식별한 첫 시도다.
...
= 이론 모형 — Principal-Agent 굿하트 게임
  ...
  앞 절(이론적 토대)이 *세 가지 이론적 전통*을 소개했다면, 본 절은 그 전통을 *한국 평가 제도*에 specialize한 정식 모형을 제시한다. 핵심 질문: *왜 합리적인 부처·출연기관이 12월에 집중 집행하고, 사업 형태별로 다른 패턴을 보이는가?* 답은 단순하다 — agent의 *비합리성*이 아니라 *합리성*이 이를 유발한다. 평가 제도가 정의한 목적함수 아래에서 균형 행동이 "굿하트 게임"이다.
...
== Career Concerns — 출연금형의 집단 평판 균형
  ...
  *동적 균형 (effective weight 매개)*: 활동 $i$는 *효과적 가중* $tilde(w)_t equiv w_t(hat(mu)_(i,t))$를 사용해 stage problem을 푼다.
...
== 실증 검증 가설
  ...
  + *H1 (분야 vs 원형)*: 비용 함수 $c$의 단위는 *사업원형*이지 분야가 아니다. → 분야 더미 $Delta R^2 approx 0$, 원형 상호작용 $Delta R^2 > 0$ 예측. (검증: 결과 5.1)
  + *H2 (자산취득형 RDD 점프)*: $c(\cdot)$이 12월 1일 직후 급감하는 원형. → RDD 점프 가장 큼. (검증: 결과 5.4 — 3.42배)
  ...
  + *H6 (시간 강화)*: $w_t / w_q$ 비율의 시간 증가 → $e_t^*$ 시계열 강화. (검증: 결과 5.7 + 부록 D.4 — wavelet +554%). 본 가설은 *Performative Prediction* (@hardt2016strategic; @perdomo2020performative)의 한국 실증 사례 — *지표가 평가 도구가 되는 순간 분포가 변화*하며 agent 행동이 *학습된 적응*을 통해 점진 강화된다.

  본 모형은 *6개 가설을 동시에 예측*하며, 각 가설은 독립적인 데이터 분석으로 검증된다. 이는 모형의 *과적합 위험을 낮추는* 다중 검증 구조다.

== 굿하트 효과 분류와의 매핑 @manheim2018
...
=== STL 분해(Seasonal-Trend decomposition using Loess)
  ...
  *직관*: FFT amp_12m_norm 신호가 진짜 게임화일까, 아니면 *지속적으로 증가하는 추세*가 12월 결산 시점 기록 방식과 결합해 만들어내는 가짜 주기성일까? 이 의문을 검증하기 위해 STL@cleveland1990을 사용한다. STL은 시계열을 가법 분해 $x_t = T_t + S_t + R_t$로 추세·계절·잔차로 분리하고, 추세 제거 후 계절 성분의 분산 비율을 `seasonal_strength`로 정의해 게임화 지표로 사용한다. 두 겹 반복(inner/outer loop)·robustness weight·정의식은 부록 C.2.
...
=== NeuralProphet 신경망 분해 — 세 번째 cross-check
  ...
  *직관*: FFT(정상성 가정)와 STL(임의 추세 가정)이 서로 다른 결론을 줄 때 어느 쪽이 게임화의 진짜 신호인지 결정하기 위해 *세 번째 독립 도구*가 필요하다. NeuralProphet@triebe2021은 Facebook Prophet@taylor2018의 신경망 확장으로, 시계열을 *추세 + Fourier 계절 + AR-Net 자기회귀 + 이벤트 + 외생 회귀*의 6개 가법 성분으로 분해한다. 본 연구는 cross-check 비교 형평성을 위해 자기회귀(`n_lags=0`)·외생 회귀를 *비활성화*하고 *추세 + 계절*만 사용한다. 6항 모형식·AR-Net 신경망 식·Prophet 원판과의 차이·하이퍼파라미터 설정은 부록 C.3.
...
= 결과 — 6 가설 다중 검증
  ...
  #figure(
    table(
      ...
      [H1], [분야 trivial, 원형이 진짜 분석 단위], [§6.1],
      [H2], [자산취득형 RDD 점프 가장 큼 (이산적 spike)], [§6.2],
      [H3], [출연금형 사이클 우세 (PSD/coherence)], [§6.3],
      [H4], [매개 경로의 원형 이질성 (pooled 미유의)], [§6.4],
      [H5], [사회복지 fortuitous alignment ($(partial Y) / (partial e_t) > 0$)], [§6.5],
      [H6], [시간 동적 강화 ($w_t / w_q$ 비율 증가)], [§6.6],
    ),
    caption: [P-A 모형의 6 가설과 결과 절의 검증 매핑],
  )

== 6.1 H1 검증 — 분야 trivial, 사업원형이 진짜 분석 단위
...
== 6.1.b H1 보강 — 4개 사업원형의 위상 안정성
  ...
  *위상 안정성 검증*: UMAP+HDBSCAN의 4개 군집이 알고리즘 우연이 아닌지 확인하기 위해 두 가지 위상 도구를 병행 적용한다.
  ...
  - *Mapper 그래프* (@fig-mapper-amp, @fig-mapper-cluster)는 32 노드 / 38 엣지 / 10 components / 7 loops로 군집 구분이 위상적으로 분리되어 있음을 보인다.
  - *Persistent Homology*는 30개 강건 component와 15개 강건 loop를 보고하며(@fig-ph-pd, @fig-ph-barcode), 50회 부트스트랩에서 H1 max persistence 95% CI [0.48, 1.19]로 위상 구조의 우연성을 배제한다(@fig-ph-bootstrap).
...
#figure(
  image("figures/h4_mapper_amp.png", width: 100%),
  caption: [Mapper graph — 평균 amp_12m_norm 색상. 32 nodes / 38 edges / 10 components / 7 loops. 노드 크기는 활동 수에 비례.],
) <fig-mapper-amp>

#figure(
  image("figures/h4_mapper_cluster.png", width: 100%),
  caption: [Mapper graph — 동일 그래프, H3 사업원형(4 archetype) 색상. 위상적으로 분리된 4 archetype 검증.],
) <fig-mapper-cluster>

#figure(
  image("figures/h9_pd.png", width: 100%),
  caption: [Persistence Diagram — Vietoris-Rips, N=300, max thresh=8.0. H0(파랑)·H1(빨강) birth-death 쌍.],
) <fig-ph-pd>

#figure(
  image("figures/h9_barcode.png", width: 100%),
  caption: [H1 Barcode — 가장 오래 지속되는 30개 loop의 birth-death 막대. max persistence = 0.671.],
) <fig-ph-barcode>

#figure(
  image("figures/h9_bootstrap.png", width: 100%),
  caption: [Bootstrap PH 50회 (n=200) — H1 max persistence 95% CI [0.48, 1.19]. 5y 참조(median 0.65) 대비 표본 안정성 검증.],
) <fig-ph-bootstrap>

== 6.5 H5 검증 — 사회복지 자동분배 (Fortuitous Alignment)
...
#figure(
  ...
  caption: [\[H1 검증\] 분야 라벨 trivial 검정 — 분야 FE ΔR²=0.000 vs 원형×Δamp ΔR²=+0.024],
) <fig-h8>

== 6.2 H2 검증 — 자산취득형 회계연도 12월 RDD 점프
  ...
  *왜 RDD가 결정적 증거인가*: 사업 본질에 따른 정상적 변동과 회계 게임화를 분리하기 위해, 같은 활동의 *11월 마지막 주 vs 12월 첫 주* 일평균 집행을 비교한다. 며칠 차이의 변동은 사업 본질로 설명할 수 없고, 12월 1일이라는 *행정적 임의 절단점*만이 작동한다. #cite(<liebman2017>, form: "prose") (AER)가 미국 연방조달에서 5배 점프를 보고한 설계의 한국 적용이다.
  ...
  활동 단위 일평균 집행을 11월\~12월 RDD로 추정한 결과, 12월 점프는 *전체 평균 1.91배*($p < 10^(-124)$)이다(@fig-rdd-monthly, @fig-rdd-yearly). 사업 형태별 분해(@fig-rdd-field):
  ...
  *RDD vs 스펙트럼 측도 — 사업원형별 메커니즘 차이*: RDD가 12월 1일 직후 *이산적 점프*를 측정하는 반면, FFT/STL/NeuralProphet/Wavelet은 *연 단위 사이클 진폭*을 측정한다. 자산취득형은 RDD 점프가 강하고(3.42배), 출연금형은 사이클 강도가 강하다(PSD 0.332, phase coherence 0.54, wavelet +554\%; 부록 D 참조). 두 사업원형 모두 *외생 회계 cycle에 결속*되어 있으나, 자산취득형은 *공정률 마감 직후 단발적 spike*로, 출연금형은 *위탁기관 정산 일정에 따른 다회 분산 + 12월 누적*으로 발현된다. 게임화 메커니즘이 사업원형별로 다르다는 점은 본 연구의 정책 함의(권고 1·2)에 직접 반영된다.

  *모형 검증 (H2, H3)*: 본 결과는 P-A 모형의 *사업원형별 비용 함수 $c(e_t, e_q; theta)$ 차이*가 균형 $e_t^*$의 *시간 구조 차이*로 발현됨을 직접 입증한다. 자산취득형은 $c(e_t)$가 12월 1일 직후 *급감*(공정률 마감과 회계 마감이 동시에 만료되는 임의 시점)하므로 RDD 점프 가장 큼(H2). 출연금형은 $c(e_t)$가 *분산형*(위탁기관 정산이 연중 다회)이므로 분산형 사이클 강도 가장 큼(H3). 두 균형은 모형의 *동일 FOC* $w_t = (partial c) / (partial e_t)$가 *원형별 비용 함수의 차이*를 통해 다른 시간 구조로 발현된 것이다.
...
#figure(
  image("figures/h22_rdd_yearly.png", width: 100%),
  caption: [연도별 12월 점프 (활동 중앙값 log 일집행액 12월 − 11월). 전체 RDD β=0.65 (1.91배, 주황 점선), #cite(<liebman2017>, form: "prose") 미국 5x 참조선(보라 점선) 대비.],
) <fig-rdd-yearly>
...
== 6.7 방법론 Robustness — STL 자기 비판 + NeuralProphet·Wavelet 중재
  ...
  *결과 — 신호 소멸*: STL 분해 후 seasonal_strength를 게임화 지표로 사용하면 사회복지 신호는 r=+0.003(p=0.991)로 *완전히 소멸한다*(@fig-stl-bars, @fig-stl-scatter). 두 가능성:
  ...
  *(a)와 (b)를 어떻게 구분하는가*: 본 연구는 두 가지 보조 검증으로 응답한다.
  - *NeuralProphet 중재* (다음 절): changepoint 명시적 모델링으로 STL이 추세로 흡수했을 가능성을 우회. 활동-단위 결과는 사회복지 r=−0.24로 음 신호 회복(약하지만 STL의 0.003보다 강함).
...
== 6.3 H3 검증 — 출연금형 사이클 우세 (PSD·Phase·Coherence·Wavelet)

본 절은 H3(출연금형의 PSD k=1 진폭·phase coherence·wavelet 진폭이 다른 원형보다 우세) 검증 핵심을 본문에 통합한다 (상세 도출은 부록 D.1\~D.4).
...
*모형 검증 (H3)*: P-A 모형의 사업원형별 비용 함수 차이 — 출연금형의 *분산형 $c(e_t)$* (위탁기관 정산이 연중 다회 발생) → 균형 $e_t^*$가 *연 사이클 전반에 걸쳐 분산 누적*되는 패턴 → PSD/coherence/wavelet 우세. 자산취득형의 *step function $c(e_t)$* (12월 1일 직후 한계비용 급감)와 *대조적*.

== 6.4 H4 검증 — NeuralProphet 매개 경로 cross-check

  *재정의*: 본 연구는 처음에 FFT·STL·NeuralProphet을 "서로 다른 결론을 줄 수 있는 cross-check 도구"로 설정했으나, 실제 분석 결과는 *세 도구가 게임화의 다른 측면을 측정하는 상보적 lens*임을 시사한다. FFT는 *주파수 영역의 진폭 비중*, STL은 *시간 영역의 추세-잔차 분해*, NP는 *changepoint 보정 후 가법 분해*다. 셋이 *동일 신호의 직교적 표현*이므로 약한 상관은 모순이 아니라 *각 도구가 잡아내는 게임화 차원의 이질성*을 정량화한 것이다.
...
== 6.4.b H4 보강 — 매개경로 원형 이질성 (Baron-Kenny + Sobel + Bootstrap)
  ...
  *방법*: #cite(<baron1986>, form: "prose") 4단계 + Sobel z-검정 + Bootstrap 95\% 신뢰구간(부록 C.11). 활동 단위 $X$ = 출연금 비중, $M$ = `amp_12m_norm`, $Y$ = 분야별 결과변수.
...
== 6.6 H6 검증 — 시간 동적 강화 (Wavelet)
  ...
  *결과 — 12개월 cycle 진폭의 시간 진화* (사업원형 평균 시계열, 2015\~2017 → 2023\~2025 평균 비교):
  ...
  #figure(
    ...
    caption: [원형별 12m wavelet power 시간 진화 — 출연금형 5.5배 강화],
  )
...
#figure(
  image("figures/h28_evolution.png", width: 95%),
...
== 처방 — 모형 균형점 이동의 행정 액션 매핑
  ...
  #figure(
    table(
      ...
      [$partial^2 c / partial e_t^2$ ↑], [$e_t$ 한계 비용 ↑ → 시점 조정 ↓], [출연기관 정산 시점 분산 (분기/반기)·자동 감사 flagging·실시간 모니터링·예외 정산 사전 신고 의무],
    ),
...
+ *DID 통제군 부재*: 한국 단일 정부 + KPI 도입 시점 점진적 → 명확한 사전·사후 비교군 부재. RDD가 가장 강한 인과 식별이나 외부 충격 자연 실험 부재.
...
= 결론
  ...
  *기여 1 — 이론 모형: 굿하트 게임의 미시 도출*: Principal-Agent 균형 분석을 통해 *왜* 한국에서 굿하트 효과가 발현하는가에 답한다. Agent의 *비합리성*이 아닌 *합리성*이 측정성 격차 환경에서 시점 조정 노력으로 자원이 쏠리도록 유도한다 @holmstrom1991. 모형은 6개 검증 가능한 가설(H1\~H6)을 산출하며, 그중 *사업원형별 균형 패턴 차이*가 본 연구의 핵심 예측이다.
  ...
  *기여 4 — 방법론 트라이앵귤레이션과 정직한 한계 보고*: FFT·STL·NeuralProphet 세 도구의 활동-연도 상관이 모두 0에 가까운 점은 모순이 아니라 *각 도구가 직교 차원으로 분해*함을 의미하며, 본 부록에서 PSD·phase·coherence·wavelet으로 확장해 *4개 도구 시간 해상도*에서 모두 *출연금형 우세 + 시간 강화* 결론으로 수렴한다. STL 분해 후 사회복지 신호 소멸은 *추세 혼재 가능성*을 자기 비판적으로 명시하는 근거로 활용했다. 모든 코드·데이터·결과는 GitHub repository(`bluethestyle/goodhart-korea`)와 Zenodo에 공개되어 *재현 가능한 연구*를 지향한다.
...
// =============================================================
#bibliography("refs.bib", title: "참고문헌", style: "apa", full: true)
...
#figure(
  text(size: 9.5pt)[
    #table(
      ...
      [AER], [American Economic Review], [미국경제학회 학술지. #cite(<liebman2017>, form: "prose") 게재.],
...
- *Strategic Classification* (@hardt2016strategic): 분류기가 평가 도구가 되는 순간 입력 분포가 *전략적 적응*된다는 ML 이론. Performative Prediction의 정적 버전.
...
*Prophet 원판과의 차이*: Prophet@taylor2018에는 $A(t), L(t)$가 *없고* 학습이 Stan MCMC. NP는 (a) AR-Net 추가, (b) lagged regressor 비선형화, (c) PyTorch SGD 학습 — 세 가지가 실질 확장.
...
#figure(
  image("figures/h27_psd.png", width: 95%),
...
#figure(
  image("figures/h27_phase.png", width: 85%),
...
#figure(
  image("figures/h27_coherence.png", width: 95%),
  caption: [원형별 phase coherence heatmap — 출연금형의 12m 동기화가 압도적],
) <fig-coh>

*결정적 발견*: *출연금형 활동들은 12개월 cycle에서 phase coherence 0.54로 가장 강하게 동기화*되어 있다. 즉 *서로 다른 출연금 활동들이 *같은 월*에 함께 피크*한다는 의미이며, 이는 외생적 회계 cycle이 *집단 행위*를 유도한다는 본 연구의 핵심 가설을 직접 입증한다. 자산취득형(0.08)·정상사업(0.13)은 동기화가 약함 — 사업별 자체 일정에 따라 분산된 시기에 피크.
...
*방법*: 각 사업원형의 *전 활동 평균 시계열*(2015\~2025, 132개월, 활동별 연 단위 정규화 후 원형 평균)에 CWT 적용. scales = 2\~40개월, complex Morlet 1.5-1.0. 12개월 scale의 power를 시계열로 추출해 *연도별 평균*으로 진화 추적.

#figure(
  image("figures/h28_scaleogram.png", width: 65%),
...
#figure(
  ...
  caption: [원형별 12m wavelet power 시간 진화 — 출연금형의 5배 이상 강화],
)
...
#figure(
  image("figures/h22_rdd_appendix.png", width: 90%),
...
2. *Dynamic 균형 미반영*: 시간 강화 (H6, +554\%) 검증은 *비교정역학*으로 했으나 stationary 모형의 본질적 한계다. #cite(<sannikov2008>, form: "prose") (ECMA)의 *연속시간 PA 모형*에서 doint optimization 동적 균형, 또는 Performative Prediction (@perdomo2020performative)의 *분포 shift 학습* 모형으로 확장하면 wavelet 시계열의 *변화점 정량 매핑* 가능.
...
4. *Heterogeneous Treatment Effects 미사용*: 본 연구의 archetype heterogeneity 발견은 Athey-Imbens (@athey2017, @athey2019)의 *causal forest* paradigm으로 보강 가능. 사업원형별 ($e_t arrow Y$) treatment effect의 *조건부 추정*이 핵심 후속 작업이며, 본 연구의 *분석 단위 재정의(H1)*는 이 후속 추정의 데이터 출발점을 제공한다.
