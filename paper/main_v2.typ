...
#[
  ...
  *실증*: 활동 단위 임베딩(UMAP+HDBSCAN)으로 *4개 사업원형*(인건비형·자산취득형·출연금형·정상사업)을 발견하고, Mapper·Persistent Homology로 위상 안정성을 입증한다(H1). 분야 라벨은 게임화 강도 분산에 *trivial한 추가 설명력*에 그치는 반면, 사업원형×지출진동 상호작용은 유의한 추가 설명력을 가진다. 회계연도 12월 RDD 분석에서 *자산취득형이 여타 사업원형 대비 가장 강한 연말 집중 점프*를 보이며(H2, 자산취득형 3.42$times$ (C1 군집 풀링, n=99) vs 전체 평균 1.91$times$), 출연금형은 여타 원형 대비 현저히 강한 *연간 사이클 우세* 패턴을 나타낸다(H3). Baron-Kenny + Sobel 매개분석은 *농림수산 분야에서 집행률 → archetype → 농가소득 매개 경로가 통계적으로 유의*함을 식별한다($z = -2.897$, $p = 0.004$, H4) — 거시 outcome 평균이 0에 수렴해도 *극단 분야에서 강한 발현*을 보이는 Manheim-Garrabrant Type 3 Extremal 굿하트의 정량 흔적. 사회복지 분야는 자동분배 특성에 의한 *fortuitous alignment* 효과를 보이며($r = -0.86$, CPI 통제 후 강화), 이는 모형의 분야 alignment 함수로 명시적으로 포착된다(H5). H5 결과는 Spearman/Kendall과 Leave-One-Out 검증으로 부호 견고성을 추가 입증한다. 웨이블릿 분석은 *출연금형 게임화 강도가 표본 기간 내 시간에 따라 강화*됨을 새롭게 보인다(H6, $+554%$) — #cite(<liebman2017>, form: "prose") 미국 결과의 *질적·동적* 한국 확장. +554%는 가중 채널이 아닌 *비용 채널 + COVID 충격* 합성으로 메커니즘적으로 재해석된다.
...
== 왜 시민이 신경 써야 하는가 — 일상 영향 4경로
  ...
  본 연구는 학술적 검증 외에도 *시민 일상*에 직접 영향을 미친다. 굿하트 게임의 결과는 추상적 효율 손실이 아닌 *생활 체감 비용*으로 발현한다.

  + *복지 수혜의 우연성*: 사회복지 분야 12월 집중 분배는 빈곤 격차 ↓에 우연히 기여한다($r = -0.86$, CPI 통제 후 강화, §6.7). 그러나 이는 *의도된 정책 성과*가 아닌 *분야 alignment 함수의 우연한 양수성*이다. 이 우연성을 정책 성과로 오인하면 진짜 빈곤 정책의 자원 배분이 왜곡되어 복지 수혜자의 실질 혜택이 감소할 수 있다.

  + *농가 실수령*: 농림수산 분야는 14분야 중 *유일하게* 강한 매개 경로를 보인다 (Sobel $z = -2.897$, $p = 0.004$, §6.5). 출연금 비중 → 12개월 진폭 → 농가소득 outcome 매개가 통계적으로 견고하게 식별된다. 12월 쏠림은 농가 가계의 자금 흐름·시점·총량에 직접 영향을 미친다.

  + *환경 결과 악화*: 12월 집중 집행은 행정 절차 부실로 이어져 환경 분야에서 음의 outcome 부호를 보인다. 기후 위기 시대에 환경 정책의 시점 왜곡은 시민 생활환경(대기질·온실가스 누적)에 직접 비용을 발생시킨다.
...
== 본 연구의 핵심 기여
  ...
  + *기여 3 — 직관 반대 발견 (§6.3\~§6.7, H2/H3/H5)*: 게임화는 단순한 부정적 측정 왜곡이 아니라 *분야 이질적·사업원형 이질적* 현상이다. 자산취득형 RDD 점프 3.42배(C1 군집 풀링, n=99 활동, H2), 출연금형 사이클 우세(H3), 사회복지 *fortuitous alignment*에 의한 자동분배 효과(H5)가 핵심.
...
== 다업무 계약 이론(Multitasking)
  ...
  한국 공공기관 평가 게임화 문헌: 한국 행정학·재정학 KCI 학술지에서도 출연기관 PBS(Project-Based Funding System)와 공공기관 경영평가의 게임화·ratchet effect를 다룬 연구가 축적되어 있다. 김근세·이만형 #cite(<kim2009pbs>)은 PBS·BSC 결합 평가의 multitasking 문제를 분석했고, 박정수 #cite(<park2014pbs>)과 임도빈 외 #cite(<lim2020kpi>)는 PBS 도입 이후 R&D·KPI 성과지표의 *수치 inflation*과 *전략적 보고*를 실증 보고했다.
...
== 굿하트 효과 분류와의 매핑 #cite(<manheim2018>)
  ...
  #cite(<manheim2018>, form: "prose")는 굿하트 효과를 *4가지 변형*으로 분류했다 — Regressional(노이즈 포함 지표의 극값 선택 편향), Causal(비-인과적 상관), Extremal(극단 영역 관계 붕괴), Adversarial(의도적 조작). 본 연구의 6 가설은 이 4유형에 다음과 같이 매핑된다.
...
=== 회귀불연속 설계(RDD)
  ...
  *주제 적합성*: 한국 회계연도 cutoff는 (a) 1948년 이래 *불변*, (b) 모든 부처에 *동시* 적용, (c) 개별 활동이 *조작 불가능* — RDD의 *세 핵심 가정*이 모두 성립하는 거의 이상적 자연실험 환경이다. #cite(<liebman2017>, form: "prose")가 미국에서 cutoff 며칠 차이로 4.9배 점프 + 품질 하락을 입증한 설계(미국 회계연도 9/30 종료)의 논리를 한국 회계연도(12/31 종료)에 적용 가능하며, 실제 분석에서 1.91배(전체) / 3.42배(자산취득형, C1 군집 풀링 n=99) 점프를 확인한다. 같은 활동의 단 며칠 차이를 비교하므로 *분야·기관·사업 특성이 자동 통제*되는 준실험이다.
...
== H1 검증 — 분야 trivial, 사업원형이 진짜 분석 단위
  ...
  *귀납적 도입 — 부처와 분야 두 행정 단위의 한계 EDA*: 정량 검증 전에, 데이터가 두 행정 단위에 어떻게 분포하는가를 본다. (a) *부처 단위* — 51개 중앙 부처 모두에서 12월 비중 평균이 균등 가정 8.33%(=100/12)를 초과한다 (51/51 = 100%; >15% 부처 약 35%, 10~15% 부처 약 45%, ≤10% 부처 약 20%). 즉 12월 봉우리는 *모든 부처 공통 패턴*으로 부처 라벨이 시점 압력의 변동을 설명하지 못한다. (b) *분야 단위* — 14개 분야의 사업×연도 단위 12월 비중 중앙값 격차가 *통일·외교 100.0% vs 사회복지 13.4% = 약 7.5배*, 분야 내부 IQR도 평균 ~83%p / 중앙 ~91%p로 분야 평균이 분야 내부 다양성을 가린다. 두 EDA 관찰이 합쳐져 *행정 분류는 시점 압력의 단위가 아니다*는 귀납적 한계를 가리키며, 이어지는 정량 ΔR² 검증이 같은 결론을 통계로 확인한다.
...
== H1 보강 — 4개 사업원형의 위상 안정성
  ...
  *구조 안정성 + 발견*: UMAP+HDBSCAN의 4개 군집이 알고리즘 우연이 아닌지 확인하기 위해 두 가지 위상 도구를 병행 적용한다. TDA는 (a) HDBSCAN 4-archetype 안정성 검증, (b) Mapper의 *manifold connectivity* 측면(점들이 어떻게 연결되어 있는가)과 HDBSCAN의 *density cluster* 측면(점들이 어디에 밀집하는가)이 *상보적* 측정임을 명시, (c) PH가 C2 출연금형 내부 sub-heterogeneity를 발견하는 역할을 수행한다.
  ...
  - *PH Loop 해석*: 상위 5개 robust loop 분석 결과, 4개는 C2 출연금형 (불규칙·고변동 지출 활동) 내부 sub-heterogeneity를 추적하며, 1개(Loop 3, 2-vertex)만 archetype 경계 saddle로 확인된다(4× enrichment, p=0.004). *해석*: 이는 4-archetype 분류가 완벽한 partitioning이 아니라 C2 출연금형 내부에 추가 분류 가능한 sub-structure가 있음을 시사한다. H1 결론('archetype이 진짜 단위')을 약화하지 않으나 *post-archetype 추가 분석 여지*를 명시한다.
...
== 방법론 상보성 검증 — FFT·STL·NeuralProphet 측도 비교
  ...
  *해석*: cycle은 *과거 회고가 아니라 진행형 패턴*이다. Spec B는 (a) Spec A의 archetype 순서를 유지(C2 > C3 > C1 > C0), (b) 2026 미래로 외삽해도 *cycle이 사라지지 않음*을 시사. 단, AR-Net 예측은 *현행 평가 제도의 stationary 가정*에 의존하므로, 본 권고 1~3이 실현되면 forecast는 *반사실적*(counterfactual)으로 재산출되어야 한다. 본 결과는 H3 가설의 *시간 외삽 robustness*에 해당하며, 부록 D.5에 추가 robustness check를 정리한다.
  ...
  === 3-tool × 14분야 outcome heatmap — 분야별 측도 발산 가시화

  분야 outcome과 게임화 측도(FFT·STL·NP) 셋의 상관을 *14분야 × 3도구* 매트릭스로 정리하면, 도구 간 *부호 일관* 분야와 *측도 의존* 분야가 시각적으로 분리된다 (`scripts/build_slide30_triangulation.py` → `figures/h30_triangulation.png`).

  *부호 일관 분야 (3 도구 모두 음 상관)*: 보건 (FFT $-0.55$ / STL $-0.20$ / NP $-0.71$), 통신 ($-0.15$ / $+0.05$ / $-0.75$ — STL 약함), 국방, 교육, 공공질서·안전 — 측정 적응 가설 *robust*. 이들 분야에서는 시점 집중과 outcome 악화의 음 상관이 *어느 측도로도 일관*된다.

  *부호 반전 분야 (측도 의존)*: 사회복지 (FFT $-0.86$ / STL $+0.003$ / NP $-0.24$). STL이 사회복지의 지속 증가 추세를 흡수해 신호 소멸한 것으로 추정되며 (§6.9), 단일 측도 결론을 *피해야 한다*. 본 결과는 *합의 트라이앵귤레이션*이 아니라 *상보적 트라이앵귤레이션*(불일치도 진단 정보)의 정량 사례다.

  *결론*: 셋이 같은 신호를 *다른 차원*으로 분해하므로, 본 분석의 견고성은 *한 도구의 절대값*이 아닌 *복수 도구가 일관되게 가리키는 분야·원형 패턴*에 의존한다. 보건·통신·국방·교육의 음 신호 (모든 측도 일관), 출연금형 12월 phase coherence 0.54 (PSD/Phase 분석으로 추가 확인), Spec A·B 양쪽에서 C2 cycle 최강(@tbl-arnet-forecast)은 *방법론 트라이앵귤레이션*의 성공 사례다.
...
== H5 검증 — 사회복지 자동분배 (Fortuitous Alignment)
  ...
  *직관 반대 발견*: 게임화는 부정적 측정 왜곡으로 통상 인식되지만, 본 분석은 사회복지 분야에서 *오히려 긍정적 결과*와 연결되는 사례를 발견한다. 사회복지 정상사업의 12월 집중 집행 강도가 클수록 *순자산 지니계수가 감소(불평등 완화)*하는 음의 상관이다. 이는 회계연도 마감 직전 사회복지 급여·보조금이 일괄 지급되면서 빈곤층에 자원이 자동 분배되는 메커니즘으로 해석된다.
...
== H6 검증 — 시간 동적 강화 (Wavelet)
  ...
  *결과 — 12개월 cycle 진폭의 시간 진화* (사업원형 평균 시계열, 2015\~2017 → 2023\~2025 평균 비교):
  ...
  #figure(
    ...
    caption: [원형별 12m wavelet power 시간 진화 — 출연금형 5.5배 강화. 출연금형 변화율 정밀값 +553.5%, 본문 인용 시 반올림 +554%.],
  )
...
== 진단 도구 — 부처-결과변수 4분면
  ...
  *왜 4분면인가*: 시점 집중 노출과 결과변수 부호의 조합으로 분석 단위를 4사분면에 배치하면, *같은 노출 강도라도 결과 부호가 다른 단위*를 분리할 수 있다. 점검 자원을 배분할 때 일률적으로 노출이 강한 곳을 보지 말고, *시점 집중 + 결과변수 악화*가 동시에 나타나는 단위를 우선한다는 의사결정 도구다. § 6.1·§ 6.2 발견에 따라 *분야 라벨은 trivial*이고 *사업원형이 진짜 분석 단위*이므로, 본 도구는 부처 차원과 사업원형 차원을 *이중*으로 적용해 해석한다 (archetype 차원 4분면은 § 7.1.1).
  ...
  - *Q1 (자동분배 동조, 녹색)*: 행정중심복합도시건설청 등 — 시점 집중 노출은 있으나 결과변수와 음 상관, 사회복지형 자동분배 alignment 가능성
  - *Q3, Q4*: 시점 집중 노출이 낮아 본 분석 우선순위 외

  *해석 주의*: 부처 라벨 단위에서 Q2로 분류된 부처가 *왜* 측정-결과 불일치를 보이는지의 *원인 변수*는 부처 내부 archetype 구성에 있다 (§ 6.1: 분야 ΔR²=0.000 vs 원형×Δamp ΔR²=+0.025). 따라서 부처 4분면은 1차 진단, archetype 4분면 (예: 출연금형 + 결과 음 상관 분야 식별)이 2차 진단으로 함께 적용되어야 한다.
...
== 처방 — 모형 균형점 이동의 행정 액션 매핑
  ...
  *권고 4 — 데이터 인프라 강화 (모형 $w_q$ ↑의 *전제 조건*)*: 본 분석은 *월별 집행*(VWFOEM) 자료 한계로 12월 *주(week) 단위* 점프를 직접 관측하지 못했다. 미국 #cite(<liebman2017>, form: "prose")는 주별 자료로 4.9배 점프를 보고했다. 열린재정 API의 *주별·일별 granularity 추가*는 RDD 식별력 향상 외에도 $w_q$ 증가에 필요한 *사업 성과 측정 인프라*의 1차 자료가 된다.
  ...
  *예상 효과 비교*: 권고 1·2·3은 모형 비교정역학에서 직접 $e_t^*$ 감소를, 권고 4·5는 측정 인프라 강화로 *$c$ 함수 상승*을 가능하게 하는 *전제 및 운영 보강*이며, 권고 6은 H6 실증 발견에 기반한 *시간 가중 조정*을 제안한다. 가장 큰 효과는 *권고 1*(다년도 회계)이 예측되며($w_t$ 변화 폭이 크기 때문), 가장 빠른 도입 가능성은 *권고 5*(모니터링)이다. 두 권고를 *우선순위*로 한다.
...
= 결론
  ...
  *기여 3 — 직관 반대 발견 (모형 H2·H3·H5 검증)*: 게임화는 단순히 부정적 측정 왜곡이 아닌 *분야 이질적·시간 동적 현상*임을 보였다. PH loop 분석에서 C2 출연금형 내부 sub-heterogeneity가 추가로 식별되어 4-archetype 분류의 post-archetype 분석 여지도 명시한다.
  - *자산취득형 RDD 점프 3.42배* (C1 군집 풀링, n=99 활동, H2 검증): 12월 1일 직후 *이산적 spike* — #cite(<liebman2017>, form: "prose") 미국 결과의 한국 확장.
  - *출연금형 사이클 우세* (H3 검증): PSD 0.332 + phase coherence 0.54 + wavelet +554\% — 다른 메커니즘으로 동일 회계 cycle에 결속.
  - *사회복지 자동분배 효과 (fortuitous, H5 명제)*: 게임화 ↔ 빈곤 격차(순자산 지니계수 대리) 음 상관 ($r = -0.86$, CPI 통제 후 강화) — *우연한* outcome alignment, 정책 정당화 아님.
  ...
  *시민 일상영향과의 매듭짓기 — § 1.2 4경로 재해소*: 서론에서 제기한 *시민 일상 영향 4경로*는 본 연구의 정량 결과로 다음과 같이 해소된다.
  - *복지 수혜의 우연성*: 사회복지 분야 $r = -0.86$ 음 상관은 fortuitous alignment (§ 6.7) — *의도된 정책 성과가 아닌* 자동분배 alignment의 우연. 이 우연을 정책 성과로 오인하면 빈곤 정책 자원 배분이 왜곡된다.
  - *농가 실수령*: 농림수산 Sobel $z = -2.897$, $p = 0.004$ (§ 6.6) — *14분야 중 유일하게* 매개 경로가 통계 유의. 출연금 비중 → 12개월 진폭 → 농가소득 매개가 견고하게 식별된다.
  - *환경 결과*: 환경 분야 outcome 음 부호 (§ 6.5) — 12월 집중 집행이 행정 절차 부실로 이어져 시민 생활환경(대기질·온실가스 누적)에 직접 비용 발생.
...
#figure(
  ...
  caption: [원형별 12m wavelet power 시간 진화 — 출연금형의 5배 이상 강화. 출연금형 변화율 정밀값 +553.5%, 본문 인용 시 반올림 +554%.],
)
...
#figure(
  table(
    ...
    [Pre-COVID], [2015\~2019], [평균 $approx$ 0.30], [완만한 강화 +0.068/yr],
    [COVID 충격], [2020\~2022], [평균 $approx$ 1.30], [급증 (확장재정)],
    [Plateau], [2023\~2025], [평균 $approx$ 1.32], [유지 (baseline 미회귀)],
  ),
...
12월 첫 주에 신호 비중을 시나리오별로 가정해 정규화:
- S1 (보수, 12월 첫 주 = 12월 전체의 25%, 균등): 한국 추정 ≈ ×1.85 — 거의 변화 없음
- S2 (중간, 12월 첫 주 = 35%): 한국 추정 ≈ ×2.20
- S3 (적극, 12월 첫 주 = 45%): 한국 추정 ≈ ×2.60
- S4 (최대, 12월 첫 주 = 50%): 한국 추정 ≈ ×2.87

따라서 *granularity 보정 시나리오 범위 = ×1.85 ~ ×2.87*. 미국 ×4.9에 *어떤 시나리오로도 도달하지 못함*. 양국 차이는 단순 측정 단위가 아니라 *제도 구조 차이* (조달 유형, 회계 분권화)가 본질적 요인임을 시사. 권고 4(주별·일별 granularity 추가)의 직접 동기.
