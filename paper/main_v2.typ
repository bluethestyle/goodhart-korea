...
=== 회귀불연속 설계(RDD)
  ...
  *주제 적합성*: 한국 회계연도 cutoff는 (a) 1948년 이래 *불변*, (b) 모든 부처에 *동시* 적용, (c) 개별 활동이 *조작 불가능* — RDD의 *세 핵심 가정*이 모두 성립하는 거의 이상적 자연실험 환경이다. #cite(<liebman2017>, form: "prose")가 미국에서 cutoff 며칠 차이로 4.9배 점프 + 품질 하락을 입증한 설계(미국 회계연도 9/30 종료)의 논리를 한국 회계연도(12/31 종료)에 적용 가능하며, 실제 분석에서 1.91배(전체) / 3.42배(자산취득형) 점프를 확인한다. 같은 활동의 단 며칠 차이를 비교하므로 *분야·기관·사업 특성이 자동 통제*되는 준실험이다.
...
+ *국제 비교*: 미국 #cite(<liebman2017>, form: "prose") 4.9배, 한국 본 연구 1.91배(자산취득 3.42배), 일본·EU 동일 분석 시 비교 → 평가 제도 차이의 균형 효과 측정.
