...
#[
  ...
  *실증*: 활동 단위 임베딩(UMAP+HDBSCAN)으로 *4개 사업원형*(인건비형·자산취득형·출연금형·정상사업)을 발견하고, Mapper·Persistent Homology로 위상 안정성을 입증한다(H1). 분야 라벨은 게임화 강도 분산에 *trivial한 추가 설명력*에 그치는 반면, 사업원형×지출진동 상호작용은 유의한 추가 설명력을 가진다. 회계연도 12월 RDD 분석에서 *자산취득형이 여타 사업원형 대비 가장 강한 연말 집중 점프*를 보이며(H2, 자산취득형 3.42$times$ (C1 군집 풀링, n=99) vs 전체 평균 1.91$times$), 출연금형은 여타 원형 대비 현저히 강한 *연간 사이클 우세* 패턴을 나타낸다(H3). Baron-Kenny + Sobel 매개분석에서 농림수산만 Sobel 간접효과가 유의했으나($z = -2.897$, $p = 0.004$, H4), 부트스트랩 95% CI가 0을 포함해 미확증($n = 5$, 연 단위 소표본)이라 *통계적 확증이 아닌 사례적 예시*로 보고한다. 사회복지는 14분야 중 *가장 강한* 음 상관($r = -0.86$, CPI 통제 후)이나, Bonferroni 보정($0.05/14 = 0.0036$) 미달인 데다 기초연금·기초생활보장 등 *자동 소득이전*이라 12월 집중 시점과 무관해 — *강한 상관이라도 메커니즘이 없으면 기각*하는 규율 사례다(H5). 결국 14분야에서 부트스트랩으로 확증된 outcome 연관은 없으며, 이는 *집행률이 정책 결과를 검증할 측정 기반 자체를 갖지 못함*(결과지표는 분야 단위로만 존재하고 사업 단위 미시 결과는 데이터 부재)을 드러낸다. 반면 본 분석의 *견고한 발견은 output 측*에 있다 — RDD 자산취득형 점프·웨이블릿 진폭 강화는 다중 검증을 통과한다. 웨이블릿 분석은 *출연금형 게임화 강도가 표본 기간 내 시간에 따라 강화*됨을 새롭게 보인다(H6, $+554%$) — #cite(<liebman2017>, form: "prose") 미국 결과의 *질적·동적* 한국 확장. +554%는 가중 채널이 아닌 *비용 채널 + COVID 충격* 합성으로 메커니즘적으로 재해석된다.
