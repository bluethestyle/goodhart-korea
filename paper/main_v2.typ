...
== 왜 시민이 신경 써야 하는가 — 일상 영향 4경로
  ...
  + *공공서비스 품질*: 연 600조 원대 재정의 4중 평가 체제가 *합리적 행위자*에게 시점 조정 노력으로의 자원 집중을 유도하는 한, 사업 품질 노력 $e_q$는 측정성 격차 함수 $\phi'(\cdot) < 1$ 만큼 체계적으로 희생된다 (Holmstrom-Milgrom impossibility). 시민이 일상에서 받는 공공서비스의 품질 하락은 이 측정성 격차의 직접 귀결이다.
...
== 본 연구의 핵심 기여
  ...
  + *기여 2 — 분석 단위 재정의 (§6.1, H1)*: 한국 행정학·재정학이 표준으로 사용해온 *분야 단위 분석*이 게임화에 대해 *trivial*($Delta R^2 = 0.000$)이고, 진짜 단위는 *사업 형태(archetype)*임을 위상 데이터 분석으로 다중 입증.. TDA Loop 분석: 출연금형(Archetype 2) 내부 sub-heterogeneity 발견 — 4-archetype 분류가 완전한 분류 종점이 아니며 Archetype 2 내부 추가 sub-structure가 존재함을 PH loop가 시사.
...
#rect(
  ...
  *[해석 한계 — H3 클러스터 임베딩 공간]* #h(1fr) #text(size: 8.5pt, fill: rgb("#666"))[audit 사항]
...
== 처방 — 모형 균형점 이동의 행정 액션 매핑
  ...
  *권고 4 — 데이터 인프라 강화 (모형 $w_q$ ↑의 *전제 조건*)*: 본 연구는 *월별 집행*(VWFOEM) 자료 한계로 12월 *주(week) 단위* 점프를 직접 관측하지 못했다. 미국 #cite(<liebman2017>, form: "prose")는 주별 자료로 4.9배 점프를 보고했다. 열린재정 API의 *주별·일별 granularity 추가*는 RDD 식별력 향상 외에도 $w_q$ 증가에 필요한 *사업 성과 측정 인프라*의 1차 자료가 된다.
...
= 결론
  ...
  *기여 3 — 직관 반대 발견 (모형 H2, H3, 추가로, PH loop 분석 결과 Archetype 2(출연금형) 내부에 sub-heterogeneity가 존재함이 발견되어 4-archetype 분류의 post-archetype 추가 분석 여지를 명시한다. H5 검증)*: 게임화는 단순히 부정적 측정 왜곡이 아닌 *분야 이질적·시간 동적 현상*임을 보였다.
  ...
  *추가 강건성·구조 분해·calibration 기여*: 추가 분석은 본 연구의 핵심 발견을 세 방향으로 보강한다. 첫째, H5 결과는 Spearman $rho = -0.833$, Kendall $tau = -0.714$, 8회 Leave-One-Out 전 부호 보존으로 측도·표본 이중 강건성을 확인했다. 둘째, 시간 진화 TDA(PH)와 wavelet의 직교성($r = 0.242$, $p = 0.473$)은 두 도구가 중복이 아닌 상보적 차원 — 공간 위상(구조) vs 시간 위상(주기) — 을 측정함을 입증한다. 셋째, 공식 평가편람에서 추출한 $w_t / w_q$ 비율이 2005년 이래 stable 0.75임을 확인, H6의 +554% 진폭 강화가 *가중 채널이 아닌 비용 채널 + COVID 충격*으로 메커니즘적으로 재해석되었다.
  ...
  한국 재정 집행의 굿하트 게임이 *고정 패턴이 아닌 진행 중인 동적 현상*이라는 본 발견은, 평가 제도 자체가 측정 대상 행동의 분포를 변화시킨다는 *Performative Prediction* (@hardt2016strategic; @perdomo2020performative)의 한국 인접 사례로 해석될 수 있다 — 단, 원 framework이 다루는 distribution shift의 stable equilibrium 수렴과 본 연구의 +554\% 동적 강화는 직접 예측 관계가 아니라 *학습 적응 + COVID 충격 합성* 메커니즘으로 §6.8에서 분리·재해석된다. 후속 연구는 국제 비교(미국·일본·EU)와 자연 실험을 통해 *어떤 제도 조건에서 이 동적 강화가 억제되는가*를 밝힐 수 있을 것이다.
