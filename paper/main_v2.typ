...
= 결과 — 6 가설 다중 검증
  ...
  #figure(
    table(
      ...
      [H4], [매개 경로의 원형 이질성 (pooled 미유의)], [§6.5·§6.6],
...
#figure(
  image("figures/h3_umap.png", width: 70%),
  caption: [\[H1 검증\] 활동 임베딩 UMAP — 4개 사업원형 (1,557 활동 × 12 피처)],
) <fig-umap>

#figure(
  image("figures/h4_mapper_amp.png", width: 50%),
  caption: [\[H1.b 검증\] Mapper graph — 평균 amp_12m_norm 색상. \ 32 nodes / 38 edges / 10 components / 7 loops. 노드 크기는 활동 수에 비례.],
) <fig-mapper-amp>

#figure(
  image("figures/h4_mapper_cluster.png", width: 50%),
  caption: [\[H1.b 검증\] Mapper graph — 동일 그래프, H3 사업원형(4 archetype) 색상.\ 위상적으로 분리된 4 archetype 검증.],
) <fig-mapper-cluster>

#figure(
  image("figures/h9_pd.png", width: 60%),
  caption: [\[H1.b 검증\] Persistence Diagram — Vietoris-Rips, N=300, max thresh=8.0.\ H0(파랑)·H1(빨강) birth-death 쌍.],
) <fig-ph-pd>

#figure(
  image("figures/h9_barcode.png", width: 60%),
...
#figure(
  image("figures/h22_rdd_field.png", width: 70%),
  caption: [\[H2 검증\] 분야별 12월 점프 배수 — 국방·국토·교통이 가장 큼, *자산취득형 3.42배 (가장 강함)*],
) <fig-rdd-field>

== 6.4 H3 검증 — 출연금형 사이클 우세 (PSD·Phase·Coherence·Wavelet)
...
== 6.5 H4 검증 — NeuralProphet 매개 경로 cross-check
...
== 6.6 H4 보강 — 매개경로 원형 이질성 (Baron-Kenny + Sobel + Bootstrap)
...
== 6.7 H5 검증 — 사회복지 자동분배 (Fortuitous Alignment)
  ...
  *직관 반대 발견*: 게임화는 부정적 측정 왜곡으로 통상 인식되지만, 본 연구는 사회복지 분야에서 *오히려 긍정적 결과*와 연결되는 사례를 발견한다. 사회복지 정상사업의 12월 집중 집행 강도가 클수록 *순자산 지니계수가 감소(불평등 완화)*하는 음의 상관이다. 이는 회계연도 마감 직전 사회복지 급여·보조금이 일괄 지급되면서 빈곤층에 자원이 자동 분배되는 메커니즘으로 해석된다.
  ...
  사회복지 분야의 1차 차분 상관은 r=−0.762(p=0.035, permutation 1,000회)로 14분야 중 유일하게 통계적으로 유의했다(@fig-h6). CPI 외생 통제 후 r=−0.86(p=0.007)로 *신호가 강화*되어, 자연 경기 cycle 가설을 기각하였다(@fig-h10).
...
#figure(
  image("figures/h6_lag_amp.png", width: 80%),
...
#figure(
  image("figures/h24_stl_bars.png", width: 70%),
...
#figure(
  image("figures/h24_stl_scatter.png", width: 70%),
...
#figure(
  image("figures/h27_psd.png", width: 70%),
...
#figure(
  image("figures/h27_phase.png", width: 70%),
...
#figure(
  image("figures/h28_scaleogram.png", width: 70%),
...
#figure(
  image("figures/h22_rdd_appendix.png", width: 70%),
