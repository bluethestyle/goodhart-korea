...
== H2: 자산취득형 회계연도 12월 RDD 점프
  ...
  미국 4.9배 대 한국 1.91배 격차의 일부는 집계 단위 차이(미국 주별 대 한국 월별)에서 온다 — 12월 첫째 주의 집중 신호가 월 단위 집계에서 한 달 전체로 희석되기 때문이다. 이를 따지려고 한국 자료를 가상 주별로 쪼갠 네 시나리오를 계산했다(@tbl-granularity). 가장 적극적인 전반 집중(front-load) 가정(S4, 12월 첫 7일 = 50%)에서도 한국 추정은 4.10배로 미국 4.9배에 미달한다. 양국 차이는 측정 단위만의 산물이 아니라 조달 유형·회계 분권화 등 제도 구조 차이가 본질적 요인이다.
