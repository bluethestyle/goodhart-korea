...
#align(center)[
  #v(6pt)
  #text(size: 17pt, weight: "bold", font: head-font)[한국 정부 재정 집행의 굿하트 게임#footnote[익명 심사용 원고. 저자명·소속·직위·이메일과 논문투고일·연구비 지원·재현 저장소(GitHub/Zenodo) 정보는 NABO 편집부 양식에 따라 최종 게재 시 기재한다.]]
  #v(5pt)
...
#pad(x: 4mm)[
  ...
  본 연구는 한국 중앙정부 재정 집행의 굿하트 효과를 주인-대리인 균형 모형으로 도출하고, 11년(2015–2025) 월별 패널 1,557개 활동으로 검증한다. 집행률은 완벽히 측정되나 사업 품질은 측정되지 않는 측정성 격차가 합리적 대리인을 시점 조정 노력으로 편향시킨다. 활동 단위 임베딩·위상 분석으로 4개 사업원형을 발견하며, 분야 고정효과의 게임화 설명력은 0인 반면 사업원형은 유의해 분산의 단위가 사업원형임을 확인한다. 12월 회귀불연속에서 자산취득형은 전체(1.91배) 대비 3.42배의 연말 점프를 보이고, 출연금형 게임화는 표본 기간 +554% 강화된다. 견고한 발견은 집행 산출 측에 있으며 결과 측 연관은 미확증으로, 집행률이 정책 결과를 검증할 미시 데이터를 갖추지 못함을 보고한다.
...
= 서론
  ...
  본 연구의 기여는 셋이다. 첫째(이론), 주인-대리인 균형 분석으로 한국에서 굿하트 효과가 발현하는 미시 기제를 도출하고 6개 검증 가능 가설을 산출한다 — 대리인의 비합리성이 아니라 합리성이 측정성 격차 환경에서 자원을 시점 조정으로 쏠리게 한다. 둘째(분석 단위), 한국 행정학·재정학이 표준으로 써온 분야 단위 분석이 게임화에 대해 무효($Delta R^2 = 0.000$)이고 진짜 단위는 사업 형태(사업원형)임을 위상 데이터 분석으로 입증한다. 셋째(인과 식별과 정직한 한계), 자산취득형 RDD 점프 3.42배·출연금형 사이클 우세·+554% 시간 강화라는 견고한 산출 측 발견을 보이는 한편, 14개 분야 결과 연관은 어느 것도 확증되지 않음을 정직하게 보고하고, 그 측정 공백 자체를 정책 진단으로 전환한다.
...
= 방법론
  ...
  본 연구는 가정이 직교하는 도구를 병행하는 상보적 트라이앵귤레이션을 따른다. 도구 간 일치는 robustness를, 불일치는 어느 가정이 위배되는지의 진단 정보를 준다 — 일치는 충분조건이지 필요조건이 아니다. 핵심 인과 도구는 RDD·매개분석·CPI 통제이고, 사이클 측정 도구는 FFT·STL·NeuralProphet·웨이블릿이다. 게임화는 직접 관측되지 않는 잠재 행동이므로, 집행 시계열의 모양 — 외생적 회계 일정에 종속된 1년 주기 패턴 — 에서 그 흔적을 추출한다. 세 사이클 측정 도구의 가정·약점·상호 보완 관계는 아래와 같다(@tbl-triangulation).
  ...
  #figure(
    table(
      ...
      table.hline(y: 1, stroke: 1.0pt + black),
      [*도구*], [*가정*], [*약점*], [*보완 도구*],
      [FFT], [정상성·주기 일정], [추세 흡수·경계 효과], [STL, NP],
      [STL], [가법 분해·LOESS 평활], [점프를 추세로 흡수], [FFT, NP],
      [NeuralProphet], [구간 선형 추세 + 푸리에 계절], [소표본 과적합], [FFT, STL],
    ),
    caption: [세 게임화 측정 도구의 가정·약점·상호 보완 관계],
  ) <tbl-triangulation>

  *인과 식별.* 회계연도 12월 1일은 연속적 시간에 그어진 행정적 절단점이다. 사업의 본질적 필요와 무관하게 이 선 직전·직후로 집행이 점프하면 외생적 회계 주기에 의한 것으로 식별된다(Imbens and Lemieux, 2008; Lee and Lemieux, 2010). 본 연구는 활동 단위 11월 대 12월 일평균 비율로 점프 배수를 보고한다(Liebman and Mahoney(2017)의 4.9배 형식과 비교 가능). 절단점은 1948년 이래 불변이고 모든 부처에 동시 적용되며 개별 활동이 조작할 수 없어 RDD 가정을 만족한다. 매개분석은 출연금 비중($X$)→시점 조정 노력($M$)→분야 결과($Y$) 경로를 Baron and Kenny(1986) 회귀·Sobel z-검정·부트스트랩 1,000회로 분해한다. CPI는 한국은행이 통화정책으로 결정해 재정 행정에 외생적이므로, Frisch-Waugh-Lovell 잔차회귀로 자연 경기 순환 가설을 기각하는 통제변수로 쓴다.

  *사이클 측정.* FFT는 연도-내 시계열에서 12개월 주기 진폭 비중(amp_12m_norm)을 추출한다 — 게임화 신호의 주파수가 사전에 알려진 1년이므로 주파수 영역 분해가 적합하다(Bevan and Hood, 2006). 진폭이 추세 혼재의 산물인지 가리려고 STL(추세를 LOESS로 흡수한 뒤 계절강도 측정)과 NeuralProphet(구간별 선형 추세 + 변화점 보정 + 푸리에 계절)을 교차 적용하며, FFT의 정상성 가정을 넘어 진폭의 시간 변화를 보려고 연속 웨이블릿 변환(complex Morlet)을 쓴다. 견고성은 순열검정 1,000회·시차/선행 분석·변동계수 대안 지표로 점검한다.

  *사업원형 발견·검증.* 사업원형은 사전 분류가 아니라 데이터에서 발견·검증한 단위다. 1,557개 활동을 12개 스칼라 피처(편성목 구성비 4개·FFT 요약 2개·집중도·규모·추세 등)로 표현하고, UMAP(McInnes et al., 2018)으로 2차원에 사영한 뒤 HDBSCAN(Campello et al., 2013) 밀도 군집으로 4개 원형을 도출했다. 군집이 알고리즘 우연이 아님을 Mapper 그래프(연결성분 10개)와 지속적 호몰로지(부트스트랩 50회; 다이어그램 거리 검증에서 관측 Wasserstein-2 평균 0.885 대 피처 치환 귀무 1.524, $p < 0.0001$)로 확인했다(Singh et al., 2007; Edelsbrunner and Harer, 2008). 단 군집은 12개월 시점 시퀀스가 아니라 편성목 유형 공간에서 정의되므로(원본 피처 공간 실루엣 0.240 대 시점 공간 $-0.179$, 군집 라벨의 월별 비중 ANOVA $R^2 = 0.065$) "편성목 유형 군집"으로 해석하며, RDD 등 원형별 추정치는 군집 내부의 월별 이질성을 감안해 읽는다. 차원 축소·위상 분석의 알고리즘 정의·매개변수 민감도·PH 루프의 출연금형 내부 세부 이질성은 기술보고서에 둔다.#footnote[모형의 전체 도출(1차 조건·헤세 행렬·Cramer's rule 비교정역학), 위상·신호처리 도구의 상세 알고리즘·강건성, 확장 분석(시간 진화 TDA·구조 보정·ALIO)은 풀버전 기술보고서(Zenodo, DOI: 게재 확정 후 공개)에 수록한다.]
...
== H1: 분야는 무효, 사업원형이 분석 단위
  ...
  Pooled 고정효과 회귀에서 분야 고정효과만 추가하면 $R^2$는 0.014에서 변하지 않는다(분야 FE 단독 $Delta R^2 = 0.000$). 사업원형×진폭변화 상호작용을 더하면 $R^2$는 0.038로 증가한다($Delta R^2 = +0.025$). 탐색적으로도, 51개 중앙 부처 전부에서 12월 집행 비중 평균이 균등 가정 8.33%를 초과하고(51/51), 14개 분야 내부의 12월 비중 사분위범위가 평균 약 83%p에 달해, 부처·분야 라벨은 시점 압력 변동의 단위가 아니다.

  진짜 단위를 데이터에 물으면 UMAP+HDBSCAN이 4개 안정 군집을 산출한다(@fig-umap). 각 군집의 z-score 프로파일은 실무 사업 형태와 일치한다(부록 B) — 인건비형 C0(n=129, 인건비 비중 z=+3.07, 게임화 진폭 z=$-1.30$), 자산취득형 C1(n=99, 자산취득 비중 z=+3.28), 출연금형 C2(n=154, 출연금 비중 z=+2.89, 진폭 z=+0.83), 정상사업 C3(n=1,175, 평균 부근). Mapper 그래프는 노드 32·엣지 38·연결성분 10으로 군집들이 위상적으로 분리됨을 보이고, 지속적 호몰로지 부트스트랩 50회는 다이어그램 전체 거리 검증(관측 Wasserstein-2 평균 0.885 대 피처 치환 귀무 1.524, $p < 0.0001$)으로 표본 변동에 대한 안정성을 확인한다. 세 도구가 일관되게 수렴해, 사업 형태가 분야보다 강한 분석 단위임이 지지된다. 본 결과는 비용 함수 $c(dot; theta)$의 단위가 분야가 아니라 사업원형이라는 모형 가설을 직접 입증한다.

  단 군집 입력은 12개월 시점 시퀀스가 아니라 활동마다 뽑은 12개 요약 피처(편성목 구성비·FFT 진폭·집중도·규모·추세)이므로, 군집은 "편성목 유형 군집"으로 해석해야 한다. 실루엣 점수가 원본 피처 공간에서는 0.240인 반면 12개월 시점 벡터 공간에서는 $-0.179$로, 시점만 놓고 보면 군집이 사실상 갈리지 않으며, 군집 라벨이 월별 비중 변동을 설명하는 정도도 ANOVA $R^2 = 0.065$에 그친다. 따라서 RDD 등 원형별 추정치는 편성목 군집 단위로 묶어 낸 평균이며 같은 군집 안에도 월별 집행의 이질성이 남아 있음을 감안해 읽는다. 또한 PH 루프 분석은 C2 출연금형 내부에 더 세분할 여지가 있음을 시사해, 4개 원형이 분류의 종점이 아님을 드러낸다.
...
== H2: 자산취득형 회계연도 12월 RDD 점프
  ...
  사업 본질에 따른 정상 변동과 회계 게임화를 분리하기 위해 같은 활동의 11월 대 12월 일평균 집행을 비교한다. 며칠 차이의 변동은 사업 본질로 설명할 수 없고 12월 1일이라는 행정적 절단점만 작동하므로, 분야·기관·사업 특성이 자동 통제되는 준실험이다. 전체 평균 12월 점프는 1.91배다($p < 10^(-124)$). 사업원형별 분해(@tbl-cutoff-archetype, @fig-rdd-field)에서 자산취득형은 3.42배(C1 군집 풀링, n=99)로 가장 크고 — 공정률 마감과 회계연도 마감이 12월 1일 직후에 결합 — 정상사업 2.24배, 인건비형 1.12배(구조상 평탄), 출연금형 1.10배(통계 미달)가 뒤따른다. 점프 순서는 use-it-or-lose-it 압력이 공정률·자산취득 사업에서 가장 강하게 작동함을 보인다.
  ...
  미국 4.9배 대 한국 1.91배 격차의 일부는 집계 단위 차이(미국 주별 대 한국 월별)에서 온다 — 12월 첫째 주의 집중 신호가 월 단위 집계에서 한 달 전체로 묽어지기 때문이다. 이를 따지려고 한국 자료를 가상 주별로 쪼갠 네 시나리오를 계산했다(@tbl-granularity). 가장 적극적인 front-load 가정(S4, 12월 첫 7일 = 50%)에서도 한국 추정은 4.10배로 미국 4.9배에 미달한다. 양국 차이는 측정 단위만의 산물이 아니라 조달 유형·회계 분권화 등 제도 구조 차이가 본질적 요인이다.
  ...
  #figure(
    table(
      ...
      [S2 가상 주별 균등], [1.85×], [12월 신호를 4주에 균등 배분],
      [S3 약한 전반 집중], [2.87×], [12월 첫 7일 = 35% 가정],
      [S4 강한 집중], [4.10× [3.88, 4.32]], [12월 첫 7일 = 50% 가정],
      [미국(Liebman-Mahoney)], [4.9×], [주별 자료 직접 관측],
    ),
    caption: [자료 세분화(granularity) 강건성 — 가상 주별 시나리오],
  ) <tbl-granularity>
...
== H6: 시간 동적 강화
  ...
  시간 진화 위상분석(공간 위상)과 웨이블릿(시간 위상)은 서로 무관하며($r = 0.242$, $p = 0.473$) — 원형 구조는 시간에 안정적이고 주기 진폭은 변한다는 두 발견을 보완적으로 묶는다. 부분 구조 보정과 ALIO 거시지표 보완 등 확장 분석은 기술보고서에 둔다.
...
== 진단: 부처×결과변수 4분면
  ...
  가로축에 시점 집중 노출, 세로축에 결과변수 상관 부호를 놓고 분석 단위를 네 칸에 배치하면(@fig-quadrant), 노출이 같아도 결과 부호가 다른 단위를 갈라낼 수 있다. 시점 집중과 결과 악화가 동시에 나타나는 Q2(추가 분석 우선)에 국무조정실·과학기술정보통신부 등이 분류된다. 부처 라벨이 Q 분류를 만든 원인은 부처 내부의 사업원형 구성에 있으므로($Delta R^2 = 0.000$ 대 원형×$Delta$amp $Delta R^2 = +0.025$), 부처 4분면은 1차 진단, 사업원형 4분면(출연금형·자산취득형이 고노출 영역에 몰림)이 2차 진단으로 결합된다. 결과 미측정 분야(국방·공공질서 등)는 본 진단의 사각지대로 남으며, 이 측정 공백 자체가 데이터 인프라 권고의 직접 근거다.
...
== 처방: 모형 레버의 행정 액션 매핑
  ...
  *권고 1 — 다년도 회계 도입 확대($w_t$↓).* 단년도 예산 마감은 미사용 예산을 자동 소멸시켜 12월 31일을 실질 절단점으로 만든다. 「국가재정법」의 이월 제한 조항을 개정해 자산취득성 사업의 자동 이월·다년도 세출 배정을 허용하고, 중기재정계획을 실제 회계 집행 단위로 격상하면 $w_t$가 크게 줄어 $e_t^*$도 따라 준다(미국 GPRA 다년도 예산배정 참조).
...
= 한계와 결론
  ...
  본 연구의 한계는 모형과 실증으로 나뉜다. 모형은 비교정역학 방향만 식별하고 절대값을 보정하지 못하며(자연 실험 필요), 정적 균형 가정으로 반복 게임의 동적 균형을 담지 못하고, 부처 간 파급을 무시한다. 실증은 외부 통제군 부재(단일 정부 체제), STL 추세 혼재, 분야별 소표본($N = 8$–$12$), 국방·예비비 결측, 월별 자료의 세분도 부재, 다중 비교 보정의 한계를 안는다. 특히 사회복지 음 상관($r = -0.86$)은 부호가 견고하나 Bonferroni 보정 후 단일 분야의 유의를 주장할 수 없고 메커니즘이 부재하므로 기각된 사례로 보고한다 — 모형이 정렬 함수 $alpha$의 분야별 이질성을 미리 인정한 것이 그 이론적 근거다.

  본 연구는 한국 중앙정부 재정 집행의 게임화를 원리·실증·정책의 3축으로 통합 분석했다. 이론적으로 합리적 대리인의 균형 행동이 측정성 격차 환경에서 자원을 시점 조정으로 쏠리게 함을 도출했고, 실증적으로 분야 단위 분석이 무효($Delta R^2 = 0.000$)이며 진짜 단위가 사업원형임을 위상 데이터 분석으로 입증했다. 견고한 발견은 산출 측에 있다 — 자산취득형 RDD 점프 3.42배(H2), 출연금형 사이클 우세(PSD 0.332·phase coherence 0.54, H3), +554% 시간 강화(H6). 결과 측에서는 부트스트랩으로 확증된 분야 연관이 없었으며(H4·H5), 이 측정 공백 자체가 집행률이 정책 결과를 검증할 기반을 갖추지 못했음을 보여주는 진단이다. 정책 처방은 모형의 세 레버에서 직접 도출되며, 불가능성 정리의 본질적 한계를 정직하게 명시한다. 한국 재정 집행의 굿하트 게임이 고정 패턴이 아니라 진행 중인 동적 현상이라는 발견은 평가 제도가 측정 대상 행동의 분포를 바꾼다는 수행적 예측(performative prediction)의 인접 사례로 해석되며(Hardt et al., 2016; Perdomo et al., 2020), 후속 자연 실험·국제 비교가 동적 강화를 억제하는 제도 조건을 밝힐 과제로 남는다.
...
#[
  ...
  #text(size: 9.5pt, fill: rgb("#555"))[
    *저자 기여 및 AI 도구 사용 명시(익명화).* 저자 기여는 CRediT 표준에 따라 개념화·이론 도출, 모형·수학 검증, 데이터 정비·재현 점검, 고급 방법론(위상·신호처리) 감수로 분담되었으며 구체적 매핑은 게재 확정 후 공개한다. 데이터 수집·정제·분석·시각화는 생성형 AI(Claude)가 보조했고, 가설 설정·결과 해석·정책 함의는 저자가 직접 결정·검토했다. 코드·데이터·결과 및 풀버전 기술보고서는 게재 확정 후 공개 저장소와 Zenodo(DOI 부여 예정)에 공개한다.
  ]
]
...
= 부록 A. 사업원형별·분야별 RDD forest plot

본문 Ⅴ.2는 RDD 결과를 전체 1.91배 + 자산취득형 3.42배(C1 군집 풀링, n=99)로 요약했다. 3.42배는 C1 자산취득형 사업군 전체(99개 활동, 1,979 관측)의 풀링 추정치($exp(1.2283)$)로 개별 활동에 귀속되지 않는다. 사업원형별 점프 순서는 자산취득형(3.42배) > 정상사업(2.24배) > 인건비형(1.12배) > 출연금형(1.10배 ns)이며, 자산취득형은 12월 1일 직후 이산적 점프로, 출연금형은 연 사이클 전반의 분산형 강도로 외생 회계 주기에 결속된다.

#figure(
  image("figures/h22_rdd_archetype_forest.png", width: 94%),
  caption: [사업원형별 회계연도 12월 점프 효과크기(forest plot)],
) <fig-rdd-archetype-forest>

#figure(
  image("figures/h22_rdd_field_forest.png", width: 94%),
  caption: [분야별 회계연도 12월 점프 효과크기(forest plot)],
) <fig-rdd-field-forest>
#tnote[주: 국방·국토·교통이 분야별로 가장 강하나 분야 구분의 추가 설명력은 $Delta R^2 = 0.000$(Ⅴ.1)으로 무효이며, 사업원형 분해(@fig-rdd-archetype-forest)가 진짜 설명 단위다.]

= 부록 B. 사업원형 z-score 프로파일

UMAP+HDBSCAN으로 도출한 4개 군집의 핵심 피처 z-score는 행정 실무 사업 형태와 일치한다. 군집은 12개월 시점 시퀀스가 아니라 편성목 유형 공간에서 정의되므로(실루엣 0.240 대 시점 공간 $-0.179$) "편성목 유형 군집"으로 해석하며, 월별 집행 패턴은 군집 평균으로 사후 서술된다(군집 라벨의 월별 비중 ANOVA $R^2 = 0.065$).
...
#figure(
  table(
    ...
    [C2 출연금형], [154], [출연금 비중 +2.89], [+0.83], [위탁 정산, 연 사이클·12월 몰림],
...
#[
  ...
  Holmstrom, B. and P. Milgrom, “Multitask Principal-Agent Analyses: Incentive Contracts, Asset Ownership, and Job Design,” #emph[Journal of Law, Economics, and Organization], Vol. 7, 1991, pp.24–52.
