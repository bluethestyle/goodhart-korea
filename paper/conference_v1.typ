...
== 균형과 비교정역학
  ...
  부호의 방향이 곧 처방의 방향이다 — 시점 평가를 덜 보상하라($w_t$ 축소), 품질을 더 보상하라($w_q$ 확대), 시점 맞추기를 더 비싸게 만들라($c_(t t)$ 인상). 이 세 레버는 정책 함의(Ⅵ장)의 권고 1–3과 1:1 대응한다. 한편 출연금형에서 두드러지는 기관 간 동조(여러 기관이 같은 시점에 함께 몰림)는 정적 모형을 집단 평판 반복 게임으로 확장하면 사회적 학습 — 동료 기관 점수로 숨은 평가 기준을 추론 — 의 결과로 설명되나(Holmström, 1999; Morris and Shin, 2002), 본 연구의 H1–H6 검증에는 정적 균형으로 충분하므로 동적 확장의 정식 도출은 지면 관계상 생략한다.
