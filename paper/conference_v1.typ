...
#set document(
  title: "한국 정부 재정 집행의 굿하트 게임 — Principal-Agent 균형 분석과 사업원형별 집행 측 인과 식별",
)
...
#align(center)[
  ...
  #v(5pt)
  #text(size: 12.5pt, weight: "bold", font: head-font)[— 주인-대리인(Principal-Agent) 균형 분석과 사업원형별 집행 측 인과 식별 —]
  #v(8pt)
...
== 균형과 비교정역학
  ...
  비교정역학은 정책 레버를 식별한다(전체 미분 전개·헤세 행렬은 기술보고서에 수록).
...
= 데이터
  ...
  본 연구는 게임화를 측정할 원인 측 데이터(재정 집행 패턴)와 그 효과를 검증할 결과 측 데이터(분야별 성과지표)를 분야 코드를 공통 키로 결합한다. 원인 측은 열린재정정보 월별 집행 테이블(VWFOEM, 2015–2025, 활동 1,557건)과 편성목 테이블(2020–2025, 활동 단위 출연금·자산취득·인건비 비중)이며, 외생 통제변수로 한국은행 ECOS 소비자물가지수(CPI)를 쓴다. 결과 측은 14개 분야 성과지표다. 원자료 분야 분류는 17개이나 활동이 분포하는 분야는 16개이며, 그중 측정 가능한 결과변수를 매핑한 14개 분야가 매개·상관 분석 대상이다 — 국방·예비비는 분야 목표에 부합하는 측정 가능한 결과변수가 없어 제외했다. 임베딩·군집·RDD는 활동(1,557) 단위로 수행되므로 분야 수에 의존하지 않는다. 분석 대상 활동(세부사업) 1,557건은 12개 특성변수를 산출할 월별 집행·편성목 자료를 갖춘 사업으로 한정된다. 결과변수 선정 기준은 분야 목표 부합성과 시계열 길이이며, 초기 후보 중 6개를 이 기준으로 교체했다(@tbl-data). 제거된 4개는 입력과 동일한 자기 인과 지표이거나 역지표로 상관을 부풀리는 방향이어서, 교체는 결과를 키우기는커녕 보수화한다 — 사후 선택(researcher degrees of freedom)과 정반대다. 교체 전후 전체 목록·사유는 기술보고서와 공개 저장소에 제시한다.
...
== H2: 자산취득형 회계연도 12월 RDD 점프
  ...
  단 이 점프가 전략적 시점 조정(게임화)인지, 공정률 준공검사·「국가계약법」 준공기한에 따른 정상적 대금 지급인지는 월 단위 비교로 완전히 분리되지 않는다. placebo와 비용 급락의 제도적 근거는 일반 계절성 가설을 약화하나 이 두 해석을 가르지는 못하며, 준공일 대비 지급일 격차·일별 집행의 추가 분석이 후속 과제다 — 정상 회계처리 비중이 크다면 시점 분산 처방(권고 3)의 실효도 그만큼 제한된다.
...
== H6: 시간 동적 강화
  ...
  다만 시기 3분할에서 +554%는 단조적 학습 적응이 아니라 코로나 확장재정이 촉발한 것이나, 일시적 충격은 아니다 — 12개월 주기강도 평균이 코로나 이전(2015–2018) 0.265에서 코로나기(2019–2021) 1.049로 급증한 뒤, 코로나 이후(2022–2025)에는 오히려 가장 높은 1.375로(코로나 이전의 약 5배) 고착됐다(구간 전환 +295.8%; 최근 구간 연 $-0.215$의 완만한 하강은 있으나 코로나 이전 수준으로 미회귀). 즉 코로나가 끌어올린 게임화 강도가 회수되지 않고 새 기준선이 된 ratchet(비가역) 패턴으로, 평가 제도가 행동 분포를 바꾼다는 수행적 예측과 정합한다. 외부 평가편람에서 역산한 가중 비율 $w_t\/w_q$는 재정사업자율평가 기준 2005년 이래 0.75로 안정적이어서, +554% 강화는 가중치 경로가 아니라 비용 경로($c_(t t)$ 감소; 2018년 부처 자체평가 전환으로 중앙 감독 약화)와 외생 충격으로 재해석된다. 원형 간 순서는 어느 시기로 잘라도 보존되어 H6의 비용 함수 이질성 예측과 일치한다.
  ...
  시간 진화 위상분석(공간 위상)과 웨이블릿(시간 위상)은 서로 무관하며($r = 0.242$, $p = 0.473$) — 원형 구조는 시간에 안정적이고 주기 진폭은 변한다는 두 발견을 보완적으로 묶는다. 부분 구조 보정과 ALIO 거시지표 보완 등 확장 분석은 기술보고서에 수록한다.
...
#align(center)[
  #v(4pt)
  #text(size: 14pt, weight: "bold", font: head-font)[The Goodhart Game in Korean Government Budget Execution: A Principal-Agent Equilibrium Analysis and Execution-Side Causal Identification by Archetype]
  #v(8pt)
