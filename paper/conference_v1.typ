...
= 한계와 결론
  ...
  본 연구의 한계는 모형과 실증으로 나뉜다. 모형은 비교정역학 방향만 식별하고 절대값을 보정하지 못하며(자연 실험 필요), 정적 균형 가정으로 반복 게임의 동적 균형을 담지 못하고, 부처 간 파급을 무시한다. 실증은 외부 통제군 부재(단일 정부 체제), STL 추세 혼재, 분야별 소표본(결과 상관 $N = 8$–$12$, 매개 $n = 4$–$6$), 국방·예비비 결측, 월별 자료의 세분도 부재, 다중 비교 보정의 한계를 안는다. 특히 사회복지 음 상관($r = -0.86$)은 부호가 견고하나 Bonferroni 보정 후 단일 분야의 유의를 주장할 수 없고 메커니즘이 부재하므로 기각된 사례로 보고한다 — 모형이 정렬 함수 $alpha$의 분야별 이질성을 미리 인정한 것이 그 이론적 근거다.
