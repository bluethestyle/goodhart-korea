...
= 한계와 결론
  ...
  본 연구의 한계는 모형과 실증으로 나뉜다. 모형은 비교정역학 방향만 식별하고 절대값을 보정하지 못하며(자연 실험 필요), 정적 균형 가정으로 반복 게임의 동적 균형을 담지 못하고, 부처 간 파급을 무시한다. 실증은 외부 통제군 부재(단일 정부 체제), STL 추세 혼재, 분야별 소표본(결과 상관 $N = 8$–$12$, 매개 $n = 4$–$6$), 국방·예비비 결측, 월별 자료의 세분도 부재, 다중 비교 보정의 한계를 안는다. 특히 사회복지 음 상관($r = -0.86$)은 부호가 견고하나 Bonferroni 보정 후 단일 분야의 유의를 주장할 수 없고 메커니즘이 부재하므로 기각된 사례로 보고한다 — 모형이 정렬 함수 $alpha$의 분야별 이질성을 미리 인정한 것이 그 이론적 근거다. 끝으로 분석 대상은 중앙정부이며, 본 연구의 use-it-or-lose-it 압력은 「국가재정법」의 단년도 이월 차단에 기인한다. 지방자치단체에서는 불용이 차기 예산 삭감으로 직결되지 않고 오히려 증가와 연관되기도 해(김용수·노희천, 2021), 이 메커니즘의 지방 일반화에는 지방 예산정치에 대한 별도 고려가 필요하다.
