...
= 서론
  ...
  Bevan and Hood(2006)는 영국 NHS 사례에서 "측정되는 것이 중요해진다"는 명제 아래 평가지표 게임화의 세 패턴(threshold·ratchet·output distortion)을 식별했다. 이는 Goodhart(1975)와 Campbell(1979)이 통화정책과 사회과학 일반에서 제시한 명제, 즉 지표가 정책 도구로 채택되는 순간 측정 신뢰도가 하락한다는 명제의 행정학적 구체화다. 같은 기제가 한국 중앙정부 재정 집행에서도 작동하는가. 이에 답하려면 게임화를 측정 가능한 양으로 정의하고, 분야·부처·사업 형태에 걸친 이질성을 분리하며, 자연 경기 순환이나 추세 같은 경합 가설을 배제해야 한다.

  본 연구는 이 세 과제를 단계적으로 수행한다. 분석은 이론이 아니라 11년 월별 집행 패널이 드러낸 세 관찰에서 출발한다. 첫째, 각 사업을 연 집행액 100%로 정규화하면 12월 비중 평균이 12.1%로 11월(5.5%)의 2.2배이며, 이 쏠림은 소규모 사업(Q1 15.4%)에서 대규모(Q4 10.9%)로 단조 감소한다. 둘째, 분기말 사이클의 강도와 위상은 같은 부처·분야 안에서도 사업별로 상이하다. 셋째, 집행률은 90% 이상에 압축되어 분야 간 거의 같으나(천장 효과) 결과지표의 분산은 훨씬 크며, 천장(99–100%)에 도달한 사업의 12월 비중은 균등 가정(8.33%)에 가깝지만 95% 미달 사업은 약 19%에 쏠린다(Welch $t > 20$, $p < 10^(-100)$). 세 관찰은 각각 시점 압력의 원인·단위·산출–결과(output–outcome) 대리 가능성을 묻는 질문으로 이어지며, 본 연구의 이론과 실증 골격을 결정한다.

  본 연구의 직접 선례는 Liebman and Mahoney(2017)다. 이들은 미국 연방조달에서 회계연도 마감(9월 30일) 직전 주의 지출이 4.9배 증가하고 동시에 품질 점수가 하락함을 회귀불연속(RDD)으로 입증했다. 이는 미사용 예산이 이월되지 않는 use-it-or-lose-it 구조가 만드는 측정 왜곡의 인과 식별 사례다. 본 연구는 동일한 RDD 설계를 한국 회계연도(12월 31일 종료)에 적용하되, 월 단위 집계 자료를 고려해 11월 대 12월 일평균 집행을 비교하고, 점프 규모의 사업원형별 이질성을 새로 식별한다.
  ...
  본 연구의 기여는 셋이다. 첫째(이론), 주인-대리인 균형 분석으로 한국에서 굿하트 효과가 발현하는 미시 기제를 도출하고 6개 검증 가능 가설을 산출한다. 즉 대리인의 비합리성이 아니라 합리성이 측정성 격차 환경에서 자원을 시점 조정으로 집중시킨다. 둘째(분석 단위), 한국 행정학·재정학이 표준으로 써온 분야 단위 분석이 게임화에 무효($Delta R^2 = 0.000$)임을 보이고, 데이터에 단위를 묻는 비지도 군집으로 게임화가 갈리는 축이 분야가 아니라 정부 회계의 편성목 구성(form)임을 특정한다. 이는 숨은 새 분류의 발명이 아니다. 군집은 편성목 구성비만으로 재현되므로(재군집 ARI 0.895), 회계상 이미 존재하는 편성목 구조의 재확인에 가깝다. 기여는 표준 분석이 분야에 가려 놓친 이 편성목 축이 게임화의 단위임을 지목하고, 그 축이 시점이 아니라 구성에 있음(디커플링)을 활동 임베딩으로, 구조의 표본 안정성을 위상 분석으로 확립한 데 있다. 셋째(인과 식별과 정직한 한계), 자산취득형 RDD 점프 3.42배·출연금형 사이클 우세·+554% 시간 강화라는 견고한 산출 측 발견을 보이는 한편, 14개 분야 결과 연관은 어느 것도 확증되지 않음을 정직하게 보고하고, 그 측정 공백 자체를 정책 진단으로 전환한다.

  이 진단은 시민 후생과 직결된다. 견고하게 확인되는 것은 측정성 격차의 직접 귀결, 즉 측정이 어려운 사업 품질 노력이 측정이 쉬운 시점 조정에 체계적으로 밀려 공공서비스 품질이 희생되는 현상이며, 복지 수혜·농가 실수령·환경 결과 같은 결과 측 경로는 본 분석으로 확증되지 않아 탐색적으로 남는다. 어느 쪽이든, 600조 원대 재정의 평가 체제가 자원 배분을 어느 방향으로 비트는지를 데이터로 특정하는 것이 본 연구의 실천적 동기다.
...
== 균형과 비교정역학
  ...
  부호의 방향이 곧 처방의 방향이다. 시점 평가를 덜 보상하라($w_t$ 축소), 품질을 더 보상하라($w_q$ 확대), 시점 맞추기를 더 비싸게 만들라($c_(t t)$ 인상). 이 세 레버는 정책 함의(Ⅵ장)의 권고 1–3과 1:1 대응한다. 한편 출연금형에서 두드러지는 기관 간 동조(여러 기관이 같은 시점에 함께 몰림)는 정적 모형을 집단 평판 반복 게임으로 확장하면 사회적 학습, 즉 동료 기관 점수로 숨은 평가 기준을 추론한 결과로 설명되나(Holmström, 1999; Morris and Shin, 2002), 본 연구의 H1–H6 검증에는 정적 균형으로 충분하므로 동적 확장의 정식 도출은 지면 관계상 생략한다.

== 사업원형별 균형 예측

  비용 함수 $c(e_t, e_q; theta)$와 평가 가중 $(w_t, w_q)$가 원형 $theta$별로 다르면 균형 시점 노력 $e_t^*$의 양상도 상이하게 나타난다. 다음 예측(@tbl-archetype-pred)이 Ⅴ장에서 데이터로 확인할 대상이다.
...
== H2: 자산취득형 회계연도 12월 RDD 점프
  ...
  12월 점프가 마감의 제도효과인지 일반 계절성인지는 비(非)마감 월 전이를 placebo로 두어 가른다(전체 활동×연도 월별 일평균의 로그비, 활동코드 클러스터). 마감이 아닌 7개 월 전이 중 6개가 배수 1.0 미만(중앙값 0.82)으로 점프가 없는 반면, 마감월(3·6·9·12)은 모두 1을 넘고 12월이 가장 크다. 이는 분기말 분해(@tbl-cutoff-archetype)의 전체 12월 배수 1.91과 정합한다. 12월 집중이 계절적 추세가 아니라 회계연도·분기 마감에 결속됨을 보이는 결과다. 유일한 예외인 1→2월 전이(1.62배)는 회계연도 초 1월 집행이 구조적으로 낮은 데 따른 기저효과로, 마감과 무관하다.
...
== H6: 시간 동적 강화
  ...
  #figure(
    table(
      ...
      table.hline(y: 1, stroke: 1.0pt + black),
      [*원형*], [*이른 시기 (2015–2017)*], [*늦은 시기 (2023–2025)*], [*변화율*],
...
== 처방: 모형 레버의 행정 액션 매핑
  ...
  *권고 1: 다년도 회계 도입 확대($w_t$ 축소).* 단년도 예산 마감은 미사용 예산을 자동 소멸시켜 12월 31일을 실질 절단점으로 만든다. 「국가재정법」의 이월 제한 조항을 개정해 자산취득성 사업의 자동 이월·다년도 세출 배정을 허용하고, 중기재정계획을 실제 회계 집행 단위로 격상하면 $w_t$가 크게 줄어 $e_t^*$도 따라 준다(미국 GPRA 다년도 예산배정 참조). 이는 집행률을 높여 불용을 줄이려는 조기집행·신속집행 기조와 정반대 방향이다. 후자는 모형상 $w_t$ 확대로 게임화를 오히려 강화하기 때문이다. 핵심은 예산을 "다 쓰느냐"가 아니라 "언제·어떻게 쓰느냐"다.
  ...
  *권고 3: 출연기관 정산 시점 분산($c_(t t)$ 인상).* 위탁계약별 정산 주기를 분기·반기로 분산하면 12월 직후 시점 조정의 한계 비용이 올라 $e_t^*$가 분산된다. 「공공기관운영법 시행령」의 연 1회 출연금 정산 주기를 개정하는 방안이 있다.

  *권고 4–6.* 데이터 인프라 강화(주별·일별 세분도로 RDD 식별력 + $w_q$ 측정 인프라 동시 확보), 자동 모니터링($c_(t t)$ 인상; 극단 게임화 활동 상시 식별), 시간 가중 점검(H6 직접 적용; 정책 점검을 최근 3년 자료에 가중)이다. 모형은 $w_t$ 변화 폭이 큰 권고 1의 효과가 가장 크다고 방향상 예측하나, 절대 효과 크기는 자연 실험 보정 없이는 추정할 수 없다. 가장 빠른 도입 가능성은 권고 5다. 측정성 격차가 본질적이므로 이 처방들은 균형점을 옮길 뿐 완전한 해결을 약속하지 않는다(불가능성 정리).
...
기존 재정 연구는 분야·부처라는 행정 분류를 분석 단위로 전제했으나, 본 연구는 활동 단위 임베딩으로 게임화가 갈리는 축이 분야가 아니라 편성목 구성임을 특정한다(@fig-umap). 위상 데이터 분석(TDA)의 결정적 기여는 *새 구조의 발견*이 아니다. 그 구조는 회계상 이미 존재하는 편성목 축이다. 기여는 셋이다: ① 군집 구조가 표본 재추출에 *불변임을 다이어그램 전체 수준에서 증명*(지속적 호몰로지 PH; 덩어리·구멍을 모든 거리 척도에서 추적, 관측 Wasserstein 0.885 대 귀무 1.524)해 "알고리즘 우연" 반론을 차단하고, ② 밀도가 아니라 점들의 연결 구조를 보는 Mapper로 군집의 위상적 분리를 독립 확인하며, ③ 회귀로는 보이지 않는 구성·시점 디커플링과 출연금형 내부 하위구조를 기하로 드러낸다. 단순 군집 분석을 넘어 *구조의 견고성과 기하를 증명*하는, 재정 집행 연구에서 시도된 바 없는 검증 파이프라인이다.
...
#tnote[자료: 저자 산출(활동 임베딩, 지속적 호몰로지).  주: 피처 무작위 치환(귀무) 분포 대비 Wasserstein-2 거리(관측 평균 0.885 대 귀무 1.524, $p < 0.0001$). 구조의 개수가 아니라 다이어그램 전체 수준에서 4개 원형의 표본 변동 강건성을 입증한다.]
...
#tnote[자료: 저자 산출(활동 임베딩, 1,557개 활동).]
